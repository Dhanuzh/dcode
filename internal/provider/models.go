...
func (mr *ModelRegistry) Refresh() error {
	...
	// Parse providers from models.dev format
	if providers, ok := modelsDevData["providers"].(map[string]interface{}); ok {
		for provID, provData := range providers {
			...
			if models, ok := provMap["models"].([]interface{}); ok {
...
func parseModelsDevModel(providerID string, data map[string]interface{}) ModelInfo {
	...
	if caps, ok := data["capabilities"].(map[string]interface{}); ok {
		if v, ok := caps["temperature"].(bool); ok {
			model.Capabilities.Temperature = v
		}
		if v, ok := caps["reasoning"].(bool); ok {
			model.Capabilities.Reasoning = v
		}
		if v, ok := caps["attachment"].(bool); ok {
			model.Capabilities.Attachment = v
		}
		if v, ok := caps["toolcall"].(bool); ok {
			model.Capabilities.ToolCall = v
		}
		if input, ok := caps["input"].(map[string]interface{}); ok {
			model.Capabilities.Input = parseModalityFlags(input)
		}
		if output, ok := caps["output"].(map[string]interface{}); ok {
			model.Capabilities.Output = parseModalityFlags(output)
		}
	}
	...
	if cost, ok := data["cost"].(map[string]interface{}); ok {
		...
		if cache, ok := cost["cache"].(map[string]interface{}); ok {
...
func parseModalityFlags(data map[string]interface{}) ModalityFlags {
	flags := ModalityFlags{}
	if v, ok := data["text"].(bool); ok {
		flags.Text = v
	}
	if v, ok := data["audio"].(bool); ok {
		flags.Audio = v
	}
	if v, ok := data["image"].(bool); ok {
		flags.Image = v
	}
	if v, ok := data["video"].(bool); ok {
		flags.Video = v
	}
	if v, ok := data["pdf"].(bool); ok {
		flags.PDF = v
	}
	return flags
}

// loadFromCache loads cached model data
