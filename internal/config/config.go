...
var DefaultModels = map[string]ModelInfo{
	...
	"copilot": {
		ID: "claude-sonnet-4.5", Name: "Claude Sonnet 4.5 (Copilot)", Provider: "copilot",
		ContextWindow: 200000, MaxOutput: 16384, SupportsTools: true,
	},
	"google": {
...
func (c *Config) GetAPIKey(provider string) string {
	// Check provider overrides first
	if po, ok := c.Providers[provider]; ok && po.APIKey != "" {
		return po.APIKey
	}
	if po, ok := c.Providers[provider]; ok && po.Options != nil && po.Options.APIKey != "" {
...
func (c *Config) ListAvailableProviders() []string {
	...
	allProviders := []providerCheck{
		{"anthropic", []string{"ANTHROPIC_API_KEY"}, func(cr *Credentials) string { return cr.AnthropicAPIKey }},
		{"openai", []string{"OPENAI_API_KEY"}, func(cr *Credentials) string { return cr.OpenAIAPIKey }},
		{"copilot", []string{"GITHUB_TOKEN"}, func(cr *Credentials) string { return cr.GitHubToken }},
	...
	for _, pc := range allProviders {
		...
		// Special: copilot auto-detect from gh CLI (with timeout)
		if !found && pc.name == "copilot" {
			ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
			if cmd, err := exec.CommandContext(ctx, "gh", "auth", "token").Output(); err == nil && strings.TrimSpace(string(cmd)) != "" {
				found = true
			}
			cancel()
		}
