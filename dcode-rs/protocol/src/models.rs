...
use dcode_utils_image::PromptImageMode;
use dcode_utils_image::load_for_prompt_bytes;
...
use dcode_git::GhostCommit;
use dcode_utils_absolute_path::AbsolutePathBuf;
use dcode_utils_image::error::ImageProcessingError;
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Dcode could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Dcode cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
// (Moved event mapping logic into dcode-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__dcode_apps__gmail_get_recent_emails",
            "namespace": "mcp__dcode_apps__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__dcode_apps__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__dcode_apps__gmail".to_string()),
    ...
    fn includes_request_rule_instructions_for_on_request() {
        ...
        exec_policy
            .add_prefix_rule(
                ...
                dcode_execpolicy::Decision::Allow,
            )
            .expect("add rule");
        let instructions = DeveloperInstructions::from_permissions_with_network(
            SandboxMode::WorkspaceWrite,
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__dcode_apps__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Dcode cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
