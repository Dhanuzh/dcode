//! Defines the protocol for a Dcode session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Dcode:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::dcode_thread::DcodeThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by dcode-core; it does not
    /// involve the model.
    SetThreadName { name: String },

    /// Request Dcode to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request Dcode to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down dcode instance.
    Shutdown,
...
/// running the command proposed by Dcode.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.dcode`, `.git`, notably `.git/hooks`) under
...
fn default_read_only_subpaths_for_writable_root(
    ...
    // Make .agents/skills and .dcode/config.toml and related files read-only
    // to the agent, by default.
    for subdir in &[".agents", ".dcode"] {
        #[allow(clippy::expect_used)]
        let top_level_dcode = writable_root.join(subdir).expect("valid relative path");
        if top_level_dcode.as_path().is_dir() {
            subpaths.push(top_level_dcode);
        }
    }
...
/// Dcode errors that we expose to clients.
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.dcode_error_info
            .as_ref()
            .is_none_or(DcodeErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Dcode),
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_dcode() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("dcode".to_string()).restriction_product(),
            Some(Product::Dcode)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Dcode])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Dcode]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".dcode")).expect("create .dcode");
        let canonical_cwd = cwd.path().canonicalize().expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_dcode = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".dcode"))
            .expect("canonical .dcode");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_dcode.as_path())
        );
    }
