//! Shared model metadata types exchanged between Dcode services and clients.
//!
...
/// Metadata describing a Dcode-supported model.
#[derive(Debug, Clone, Deserialize, Serialize, TS, JsonSchema, PartialEq)]
...
impl ModelInfo {
    /// Create a minimal `ModelInfo` for a model returned by a third-party
    /// OpenAI-compatible provider (e.g. GitHub Copilot, Groq).  All metadata
    /// fields are set to safe defaults; crucially `upgrade` is `None` so that
    /// no model-migration prompts are triggered.
    pub fn for_custom_provider(slug: String) -> Self {
        ...
        Self {
            ...
            default_reasoning_summary: ReasoningSummary::default(),
