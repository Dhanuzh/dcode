...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Dcode degrades to warn-and-ignore
    /// instead of failing to load. Dcode 0.112.0 rejected unknown values here,
...
impl FileSystemSandboxPolicy {
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.dcode -> <root>/decoy`, bwrap must still see
            // `<root>/.dcode`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.dcode` so downstream sandboxes can still detect and mask
...
mod tests {
    ...
    fn effective_runtime_roots_canonicalize_symlinked_paths() {
        ...
        let dcode_dir = real_root.join(".dcode");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&dcode_dir).expect("create .dcode");
        ...
        let expected_dcode = AbsolutePathBuf::from_absolute_path(
            dcode_dir.canonicalize().expect("canonicalize .dcode"),
        )
        .expect("absolute canonical .dcode");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-dcode");
        let dot_dcode = root.join(".dcode");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_dcode).expect("create .dcode symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_dcode = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".dcode"),
        )
        .expect("absolute .dcode symlink");
    ...
    fn tmpdir_special_path_canonicalizes_symlinked_tmpdir() {
        ...
        let dcode_dir = real_tmpdir.join(".dcode");
