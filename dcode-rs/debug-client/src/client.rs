...
impl AppServerClient {
    pub fn spawn(
        ...
        let mut child = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{dcode_bin}` app-server"))?;
    ...
    pub fn initialize(&mut self) -> Result<()> {
        ...
        let request = ClientRequest::Initialize {
            ...
            params: dcode_app_server_protocol::InitializeParams {
        ...
        let _parsed: dcode_app_server_protocol::InitializeResponse =
...
fn handle_server_request(
    ...
    let Ok(server_request) = dcode_app_server_protocol::ServerRequest::try_from(request) else {
        return Ok(());
    };

    match server_request {
        dcode_app_server_protocol::ServerRequest::CommandExecutionRequestApproval {
            request_id,
            ..
        } => {
            let response = dcode_app_server_protocol::CommandExecutionRequestApprovalResponse {
                decision: CommandExecutionApprovalDecision::Decline,
            };
            send_jsonrpc_response(stdin, request_id, response)
        }
        dcode_app_server_protocol::ServerRequest::FileChangeRequestApproval {
            request_id, ..
        } => {
            let response = dcode_app_server_protocol::FileChangeRequestApprovalResponse {
