...
Typed clients for Dcode/OpenAI APIs built on top of the generic transport in `dcode-client`.
...
- Serves as the wire-level layer consumed by `dcode-core`; higher layers handle auth refresh and business logic.
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `dcode_api::CompactionInput`):
    - `model: &str`.
...
- **Memory summarize endpoint**
  - Input: `MemorySummarizeInput` (re-exported as `dcode_api::MemorySummarizeInput`):
    - `model: String`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `dcode-api` and `dcode-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
