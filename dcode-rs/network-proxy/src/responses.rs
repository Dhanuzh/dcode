...
pub fn blocked_message(reason: &str) -> &'static str {
    match reason {
        REASON_NOT_ALLOWED => {
            "Dcode blocked this request: domain not in allowlist (this is not a denylist block)."
        }
        REASON_NOT_ALLOWED_LOCAL => {
            "Dcode blocked this request: local/private addresses not allowed."
        }
        REASON_DENIED => "Dcode blocked this request: domain denied by policy.",
        REASON_METHOD_NOT_ALLOWED => {
            "Dcode blocked this request: method not allowed in limited mode."
        }
        REASON_MITM_REQUIRED => "Dcode blocked this request: MITM required for limited HTTPS.",
        _ => "Dcode blocked this request by network policy.",
    }
}
