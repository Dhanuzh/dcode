...
`dcode-network-proxy` is Dcode's local network policy enforcement proxy. It runs:
...
`dcode-network-proxy` reads from Dcode's merged `config.toml` (via `dcode-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $DCODE_HOME/proxy/ (ca.pem + ca.key).
...
```bash
cargo run -p dcode-network-proxy --
```
...
`dcode-network-proxy` can be embedded as a library with a thin API:

```rust
use dcode_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `dcode-network-proxy` is embedded in managed Dcode runtime, policy decisions emit structured
OTEL-compatible events with `target=dcode_otel.network_proxy`.

Event name:

- `dcode.network_proxy.policy_decision`
...
This section documents the protections implemented by `dcode-network-proxy`, and the boundaries of
