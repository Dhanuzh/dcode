...
This repository uses Bazel to build the Rust workspace under `dcode-rs`.
...
- `rules_rs` imports third-party crates from `dcode-rs/Cargo.toml` and
  `dcode-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `dcode_rust_crate`, which wraps `rust_library`,
...
- Each crate in `dcode-rs/*/BUILD.bazel` typically uses `dcode_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `dcode_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `dcode_rust_crate` to configure it.
