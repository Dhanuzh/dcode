...
These are entities exit on the dcode backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `Dcode` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `Dcode`
   - The core engine of dcode
...
2. `Session`
   - The `Dcode`'s current configuration and state
   - `Dcode` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `Dcode` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `Dcode` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `Dcode`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `Dcode`, as `Dcode` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `Dcode` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `Dcode`)
...
- `Event`
  - These are messages sent on the `EQ` (`Dcode` -> UI)
...
sequenceDiagram
    ...
    user->>dcode: Op::ConfigureSession
    dcode-->>session: create session
    dcode->>user: Event::SessionConfigured
