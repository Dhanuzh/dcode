...
impl McpProcess {
    pub async fn new(dcode_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(dcode_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = dcode_utils_cargo_bin::cargo_bin("dcode-mcp-server")
            .context("should find binary for dcode-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = dcode_core::default_client::originator().value;
    ...
    pub async fn send_dcode_tool_call(
        &mut self,
        params: DcodeToolCallParam,
    ) -> anyhow::Result<i64> {
        let dcode_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "dcode".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(dcode_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "dcode/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "dcode/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `dcode-mcp-server` child process.
        //
