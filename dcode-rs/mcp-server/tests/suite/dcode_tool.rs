...
use dcode_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use dcode_mcp_server::DcodeToolCallParam;
use dcode_mcp_server::ExecApprovalElicitRequestParams;
use dcode_mcp_server::ExecApprovalResponse;
use dcode_mcp_server::PatchApprovalElicitRequestParams;
use dcode_mcp_server::PatchApprovalResponse;
use dcode_protocol::protocol::FileChange;
use dcode_protocol::protocol::ReviewDecision;
use dcode_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Dcode sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "dcode" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let dcode_request_id = mcp_process
        .send_dcode_tool_call(DcodeToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            dcode_request_id.to_string(),
            params.dcode_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `dcode` tool call completes and that the file was created.
    let dcode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(dcode_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(dcode_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    dcode_mcp_tool_call_id: String,
    dcode_event_id: String,
    thread_id: dcode_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Dcode to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let dcode_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        dcode_elicitation: "exec-approval".to_string(),
        dcode_mcp_tool_call_id,
        dcode_event_id,
        dcode_command: command,
        dcode_cwd: workdir.to_path_buf(),
        dcode_call_id: "call1234".to_string(),
        dcode_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "dcode" tool request that will trigger the apply_patch command
    let dcode_request_id = mcp_process
        .send_dcode_tool_call(DcodeToolCallParam {
    ...
    // Verify the original `dcode` tool call completes
    let dcode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(dcode_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_dcode_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = dcode_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn dcode_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `dcode mcp` with a specific config.toml.
    let dcode_home = TempDir::new()?;
    create_config_toml(dcode_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(dcode_home.path()).await?;
    ...
    .await??;
    assert_eq!(dcode_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(dcode_response.id, RequestId::Number(dcode_request_id));
    assert_eq!(
        dcode_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": dcode_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("dcode tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
/// Create a Dcode config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(dcode_home: &Path, server_uri: &str) -> std::io::Result<()> {
