...
use crate::dcode_tool_config::DcodeToolCallParam;
use crate::dcode_tool_config::DcodeToolCallReplyParam;
use crate::dcode_tool_config::create_tool_for_dcode_tool_call_param;
use crate::dcode_tool_config::create_tool_for_dcode_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_dcode_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_dcode_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation {
            name: "dcode-mcp-server".to_string(),
            title: Some("Dcode".to_string()),
        ...
        // Preserve Dcode's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_dcode_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_dcode_tool_call_param(),
                create_tool_for_dcode_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "dcode" => self.handle_tool_call_dcode(id, arguments).await,
            "dcode-reply" => {
                self.handle_tool_call_dcode_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_dcode(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<DcodeToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![rmcp::model::Content::text(format!(
                                "Failed to load Dcode configuration from overrides: {e}"
                            ))],
                            structured_content: None,
                            is_error: Some(true),
                            meta: None,
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Dcode tool: {e}"
                        ))],
                        structured_content: None,
                        is_error: Some(true),
                        meta: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for dcode tool-call; the `prompt` field is required.",
                    )],
                    structured_content: None,
                    is_error: Some(true),
                    meta: None,
        ...
        let running_requests_id_to_dcode_uuid = self.running_requests_id_to_dcode_uuid.clone();

        // Spawn an async task to handle the Dcode session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Dcode session and stream events back to the client.
            crate::dcode_tool_runner::run_dcode_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_dcode_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_dcode_session_reply(
        &self,
        ...
        // parse arguments
        let dcode_tool_call_reply_param: DcodeToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<DcodeToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Dcode tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Dcode tool: {e}"
                        ))],
            ...
            None => {
                tracing::error!(
                    "Missing arguments for dcode-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for dcode-reply tool-call; the `thread_id` and `prompt` fields are required.",
        ...
        let thread_id = match dcode_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let dcode = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::dcode_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = dcode_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_dcode_uuid = running_requests_id_to_dcode_uuid.clone();

            async move {
                crate::dcode_tool_runner::run_dcode_tool_session_reply(
                    thread_id,
                    dcode,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_dcode_uuid.lock().await;
        ...
        // Obtain the Dcode thread from the server.
        let dcode_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Dcode.
        if let Err(e) = dcode_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: dcode_protocol::protocol::Op::Interrupt,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to Dcode: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_dcode_uuid
            .lock()
            .await
            .remove(&request_id);
    }
