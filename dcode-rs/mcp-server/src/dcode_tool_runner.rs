//! Asynchronous worker that executes a **Dcode** tool-call inside a spawned
...
use dcode_core::config::Config as DcodeConfig;
...
/// To adhere to MCP `tools/call` response format, include the Dcode
...
/// Run a complete Dcode session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_dcode_tool_session(
    id: RequestId,
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!("Failed to start Dcode session: {e}"))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the Dcode submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_dcode_uuid.lock().await.remove(&id);
        return;
    }

    run_dcode_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_dcode_tool_session_reply(
    thread_id: ThreadId,
    thread: Arc<DcodeThread>,
    outgoing: Arc<OutgoingMessageSender>,
...
async fn run_dcode_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<DcodeThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(dcode_event_to_notification(&event)) above has
