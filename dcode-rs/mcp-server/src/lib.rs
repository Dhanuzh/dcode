...
mod dcode_tool_config;
mod dcode_tool_runner;
mod exec_approval;
...
pub use crate::dcode_tool_config::DcodeToolCallParam;
pub use crate::dcode_tool_config::DcodeToolCallReplyParam;
...
const OTEL_SERVICE_NAME: &str = "dcode_mcp_server";
...
mod tests {
    ...
    use dcode_core::config::types::OtelExporterKind;
    ...
    async fn mcp_server_builds_otel_provider_with_logs_traces_and_metrics() -> anyhow::Result<()> {
        ...
        let provider = dcode_core::otel_init::build_provider(
            &config,
            "0.0.0-test",
            Some(OTEL_SERVICE_NAME),
            DEFAULT_ANALYTICS_ENABLED,
