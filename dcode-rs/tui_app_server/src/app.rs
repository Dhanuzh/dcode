...
use dcode_app_server_client::AppServerRequestHandle;
use dcode_app_server_client::TypedRequestError;
...
use dcode_core::message_history;
...
use dcode_protocol::approvals::ExecApprovalRequestEvent;
...
use dcode_protocol::protocol::GetHistoryEntryResponseEvent;
...
fn command_execution_decision_to_review_decision(
    decision: dcode_app_server_protocol::CommandExecutionApprovalDecision,
) -> dcode_protocol::protocol::ReviewDecision {
    match decision {
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&dcode_protocol::protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&dcode_protocol::approvals::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[dcode_protocol::approvals::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&dcode_protocol::models::PermissionProfile>,
) -> Vec<dcode_protocol::protocol::ReviewDecision> {
...
fn list_skills_response_to_core(response: SkillsListResponse) -> ListSkillsResponseEvent {
    ListSkillsResponseEvent {
        skills: response
            .data
            .into_iter()
            .map(|entry| dcode_protocol::protocol::SkillsListEntry {
                cwd: entry.cwd,
                skills: entry
                    .skills
                    .into_iter()
                    .map(|skill| dcode_protocol::protocol::SkillMetadata {
                        name: skill.name,
                        description: skill.description,
                        short_description: skill.short_description,
                        interface: skill.interface.map(|interface| {
                            dcode_protocol::protocol::SkillInterface {
                        ...
                        dependencies: skill.dependencies.map(|dependencies| {
                            dcode_protocol::protocol::SkillDependencies {
                                tools: dependencies
                                    .tools
                                    .into_iter()
                                    .map(|tool| dcode_protocol::protocol::SkillToolDependency {
                                        r#type: tool.r#type,
                                        value: tool.value,
                        ...
                        scope: match skill.scope {
                            dcode_app_server_protocol::SkillScope::User => {
                                dcode_protocol::protocol::SkillScope::User
                            }
                            dcode_app_server_protocol::SkillScope::Repo => {
                                dcode_protocol::protocol::SkillScope::Repo
                            }
                            dcode_app_server_protocol::SkillScope::System => {
                                dcode_protocol::protocol::SkillScope::System
                            }
                            dcode_app_server_protocol::SkillScope::Admin => {
                                dcode_protocol::protocol::SkillScope::Admin
                            }
                        },
                ...
                errors: entry
                    .errors
                    .into_iter()
                    .map(|error| dcode_protocol::protocol::SkillErrorInfo {
                        path: error.path,
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.dcode_error_info,
        Some(AppServerDcodeErrorInfo::ActiveTurnNotSteerable { .. })
    )
                    ...
                    app_event_tx.send(AppEvent::ThreadHistoryEntryResponse {
                        ...
                        event: GetHistoryEntryResponseEvent {
                            ...
                            entry: entry_opt.map(|entry| {
                                dcode_protocol::message_history::HistoryEntry {
        ...
        match op.view() {
            ...
            AppCommandView::ListSkills { cwds, force_reload } => {
                let response = app_server
                    .skills_list(dcode_app_server_protocol::SkillsListParams {
                        cwds: cwds.to_vec(),
                        force_reload,
...
type McpInventoryMaps = (
    HashMap<String, dcode_protocol::mcp::Tool>,
    HashMap<String, Vec<dcode_protocol::mcp::Resource>>,
    HashMap<String, Vec<dcode_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                dcode_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                dcode_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                dcode_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                dcode_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    use dcode_app_server_protocol::AdditionalNetworkPermissions;
    use dcode_app_server_protocol::AdditionalPermissionProfile;
    ...
    use dcode_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
    use dcode_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
    use dcode_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
    use dcode_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
    use dcode_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
    use dcode_app_server_protocol::RequestId as AppServerRequestId;
    use dcode_app_server_protocol::ServerNotification;
    use dcode_app_server_protocol::ServerRequest;
    ...
    use dcode_app_server_protocol::TokenUsageBreakdown;
    use dcode_app_server_protocol::Turn;
    ...
    use dcode_protocol::models::NetworkPermissions;
    ...
    use dcode_protocol::protocol::McpAuthStatus;
    use dcode_protocol::protocol::NetworkApprovalContext;
    use dcode_protocol::protocol::NetworkApprovalProtocol;
    ...
    fn mcp_inventory_maps_prefix_tool_names_by_server() {
        let statuses = vec![
            McpServerStatus {
                ...
                auth_status: dcode_app_server_protocol::McpAuthStatus::Unsupported,
            },
            McpServerStatus {
    ...
    -> Result<()> {
        ...
        while let Ok(event) = app_event_rx.try_recv() {
            match event {
                ...
                AppEvent::DcodeOp(Op::UserTurn { items, .. }) => {
                    submitted_items = Some(items);
                }
                _ => {}
            }
...
"#;
    ...
    async fn inactive_thread_exec_approval_preserves_context() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                dcode_protocol::protocol::ReviewDecision::ApprovedForSession,
                dcode_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                    network_policy_amendment: dcode_protocol::approvals::NetworkPolicyAmendment {
                        host: "example.com".to_string(),
                        action: dcode_protocol::approvals::NetworkPolicyRuleAction::Allow,
                    },
                },
                dcode_protocol::protocol::ReviewDecision::Abort,
            ]
        ...
        app.enqueue_thread_notification(
            ...
            ServerNotification::ThreadStarted(ThreadStartedNotification {
                thread: Thread {
                    ...
                    created_at: 1,
                    updated_at: 2,
                    status: dcode_app_server_protocol::ThreadStatus::Idle,
                    path: Some(rollout_path.clone()),
                    cwd: PathBuf::from("/tmp/agent"),
                    cli_version: "0.0.0".to_string(),
                    source: dcode_app_server_protocol::SessionSource::Unknown,
                    agent_nickname: Some("Robie".to_string()),
                    agent_role: Some("explorer".to_string()),
                    git_info: None,
    ...
    async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()>
    {
        ...
        assert_eq!(
            app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
                dcode_app_server_protocol::SkillsChangedNotification {},
            )),
            None
        );
        Ok(())
    ...
    fn thread_event_store_rebase_preserves_resolved_request_state() {
        ...
        store.push_notification(ServerNotification::ServerRequestResolved(
            dcode_app_server_protocol::ServerRequestResolvedNotification {
                request_id: AppServerRequestId::Integer(1),
                thread_id: thread_id.to_string(),
            },
        ));
    ...
    fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
        let turn_error = AppServerTurnError {
            ...
            dcode_error_info: Some(AppServerDcodeErrorInfo::ActiveTurnNotSteerable {
