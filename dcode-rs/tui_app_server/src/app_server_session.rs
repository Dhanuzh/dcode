use dcode_app_server_client::AppServerClient;
use dcode_app_server_client::AppServerEvent;
...
use dcode_app_server_protocol::Model as ApiModel;
...
use dcode_app_server_protocol::ReviewDelivery;
...
use dcode_protocol::protocol::SessionNetworkProxyRuntime;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<dcode_protocol::account::PlanType>,
...
pub(crate) struct ThreadSessionState {
    ...
    pub(crate) service_tier: Option<dcode_protocol::config_types::ServiceTier>,
    pub(crate) approval_policy: AskForApproval,
    pub(crate) approvals_reviewer: dcode_protocol::config_types::ApprovalsReviewer,
    pub(crate) sandbox_policy: SandboxPolicy,
    pub(crate) cwd: PathBuf,
    pub(crate) reasoning_effort: Option<dcode_protocol::openai_models::ReasoningEffort>,
...
impl AppServerSession {
    ...
    pub(crate) async fn turn_start(
        &mut self,
        thread_id: ThreadId,
        items: Vec<dcode_protocol::user_input::UserInput>,
        cwd: PathBuf,
        approval_policy: AskForApproval,
        approvals_reviewer: dcode_protocol::config_types::ApprovalsReviewer,
        sandbox_policy: SandboxPolicy,
        model: String,
        effort: Option<dcode_protocol::openai_models::ReasoningEffort>,
        summary: Option<dcode_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<dcode_protocol::config_types::ServiceTier>>,
        collaboration_mode: Option<dcode_protocol::config_types::CollaborationMode>,
        personality: Option<dcode_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<dcode_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<dcode_app_server_protocol::ApprovalsReviewer> {
...
) -> Option<dcode_app_server_protocol::SandboxMode> {
    match policy {
        SandboxPolicy::DangerFullAccess => {
            Some(dcode_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        SandboxPolicy::ReadOnly { .. } => Some(dcode_app_server_protocol::SandboxMode::ReadOnly),
        SandboxPolicy::WorkspaceWrite { .. } => {
            Some(dcode_app_server_protocol::SandboxMode::WorkspaceWrite)
        }
...
) -> dcode_app_server_protocol::ReviewTarget {
    match target {
        CoreReviewTarget::UncommittedChanges => {
            dcode_app_server_protocol::ReviewTarget::UncommittedChanges
        }
        CoreReviewTarget::BaseBranch { branch } => {
            dcode_app_server_protocol::ReviewTarget::BaseBranch { branch }
        }
        CoreReviewTarget::Commit { sha, title } => {
            dcode_app_server_protocol::ReviewTarget::Commit { sha, title }
        }
        CoreReviewTarget::Custom { instructions } => {
            dcode_app_server_protocol::ReviewTarget::Custom { instructions }
        }
    }
}
...
async fn thread_session_state_from_thread_response(
    ...
    service_tier: Option<dcode_protocol::config_types::ServiceTier>,
    approval_policy: AskForApproval,
    approvals_reviewer: dcode_protocol::config_types::ApprovalsReviewer,
    sandbox_policy: SandboxPolicy,
    cwd: PathBuf,
    reasoning_effort: Option<dcode_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
...
pub(crate) fn app_server_rate_limit_snapshot_to_core(
    snapshot: dcode_app_server_protocol::RateLimitSnapshot,
) -> RateLimitSnapshot {
    RateLimitSnapshot {
...
fn app_server_rate_limit_window_to_core(
    window: dcode_app_server_protocol::RateLimitWindow,
) -> RateLimitWindow {
    RateLimitWindow {
...
fn app_server_credits_snapshot_to_core(
    snapshot: dcode_app_server_protocol::CreditsSnapshot,
) -> CreditsSnapshot {
    CreditsSnapshot {
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> Config {
        ConfigBuilder::default()
            .dcode_home(temp_dir.path().to_path_buf())
            .build()
            .await
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: dcode_app_server_protocol::Thread {
                ...
                source: dcode_protocol::protocol::SessionSource::Cli.into(),
                agent_nickname: None,
                agent_role: None,
                git_info: None,
                name: None,
                turns: vec![Turn {
                    id: "turn-1".to_string(),
                    items: vec![
                        dcode_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            content: vec![dcode_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        dcode_app_server_protocol::ThreadItem::AgentMessage {
            ...
            service_tier: None,
            cwd: PathBuf::from("/tmp/project"),
            approval_policy: dcode_protocol::protocol::AskForApproval::Never.into(),
            approvals_reviewer: dcode_app_server_protocol::ApprovalsReviewer::User,
            sandbox: dcode_protocol::protocol::SandboxPolicy::new_read_only_policy().into(),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        let session = thread_session_state_from_thread_response(
            ...
            dcode_protocol::config_types::ApprovalsReviewer::User,
