...
use dcode_app_server_protocol::CollabAgentState as AppServerCollabAgentState;
use dcode_app_server_protocol::CollabAgentStatus as AppServerCollabAgentStatus;
use dcode_app_server_protocol::CollabAgentTool;
use dcode_app_server_protocol::CollabAgentToolCallStatus;
...
use dcode_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use dcode_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
#[cfg(test)]
...
use dcode_protocol::protocol::CollabAgentStatusEntry;
...
use dcode_protocol::request_user_input::RequestUserInputQuestionOption;
...
#[cfg(test)]
fn core_rate_limit_error_kind(info: &CoreDcodeErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        CoreDcodeErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        CoreDcodeErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        CoreDcodeErrorInfo::ResponseTooManyFailedAttempts {
...
fn app_server_rate_limit_error_kind(info: &AppServerDcodeErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        AppServerDcodeErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerDcodeErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerDcodeErrorInfo::ResponseTooManyFailedAttempts {
            http_status_code: Some(429),
        } => Some(RateLimitErrorKind::Generic),
        _ => None,
    }
...
) -> dcode_protocol::protocol::SessionConfiguredEvent {
    dcode_protocol::protocol::SessionConfiguredEvent {
...
fn app_server_request_id_to_mcp_request_id(
    request_id: &dcode_app_server_protocol::RequestId,
) -> dcode_protocol::mcp::RequestId {
    match request_id {
        dcode_app_server_protocol::RequestId::String(value) => {
            dcode_protocol::mcp::RequestId::String(value.clone())
        }
        dcode_app_server_protocol::RequestId::Integer(value) => {
...
fn exec_approval_request_from_params(
    ...
    ExecApprovalRequestEvent {
        ...
        proposed_execpolicy_amendment: params
            ...
            .map(dcode_app_server_protocol::ExecPolicyAmendment::into_core),
        proposed_network_policy_amendments: params.proposed_network_policy_amendments.map(
            |amendments| {
                amendments
                    .into_iter()
                    .map(dcode_app_server_protocol::NetworkPolicyAmendment::into_core)
                    .collect()
            },
        ),
        skill_metadata: params.skill_metadata.map(|metadata| {
            dcode_protocol::approvals::ExecApprovalRequestSkillMetadata {
        ...
        available_decisions: params.available_decisions.map(|decisions| {
            decisions
                ...
                .map(|decision| match decision {
                    dcode_app_server_protocol::CommandExecutionApprovalDecision::Accept => {
                        dcode_protocol::protocol::ReviewDecision::Approved
                    }
                    dcode_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession => {
                        dcode_protocol::protocol::ReviewDecision::ApprovedForSession
                    }
                    dcode_app_server_protocol::CommandExecutionApprovalDecision::AcceptWithExecpolicyAmendment {
                        execpolicy_amendment,
                    } => dcode_protocol::protocol::ReviewDecision::ApprovedExecpolicyAmendment {
                        proposed_execpolicy_amendment: execpolicy_amendment.into_core(),
                    },
                    dcode_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                        network_policy_amendment,
                    } => dcode_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                        network_policy_amendment: network_policy_amendment.into_core(),
                    },
                    dcode_app_server_protocol::CommandExecutionApprovalDecision::Decline => {
                        dcode_protocol::protocol::ReviewDecision::Denied
                    }
                    dcode_app_server_protocol::CommandExecutionApprovalDecision::Cancel => {
                        dcode_protocol::protocol::ReviewDecision::Abort
                    }
                })
                .collect()
        }),
        parsed_cmd: params
            ...
            .unwrap_or_default()
            .into_iter()
            .map(dcode_app_server_protocol::CommandAction::into_core)
            .collect(),
    }
}
...
fn app_server_patch_changes_to_core(
    changes: Vec<dcode_app_server_protocol::FileUpdateChange>,
) -> HashMap<PathBuf, dcode_protocol::protocol::FileChange> {
    changes
        .into_iter()
        .map(|change| {
            let path = PathBuf::from(change.path);
            let file_change = match change.kind {
                dcode_app_server_protocol::PatchChangeKind::Add => {
                    dcode_protocol::protocol::FileChange::Add {
                        content: change.diff,
                    }
                }
                dcode_app_server_protocol::PatchChangeKind::Delete => {
                    dcode_protocol::protocol::FileChange::Delete {
                        content: change.diff,
                    }
                }
                dcode_app_server_protocol::PatchChangeKind::Update { move_path } => {
                    dcode_protocol::protocol::FileChange::Update {
...
fn request_permissions_from_params(
    params: dcode_app_server_protocol::PermissionsRequestApprovalParams,
...
fn request_user_input_from_params(params: ToolRequestUserInputParams) -> RequestUserInputEvent {
    RequestUserInputEvent {
        ...
        questions: params
            ...
            .into_iter()
            .map(
                |question| dcode_protocol::request_user_input::RequestUserInputQuestion {
                    id: question.id,
...
fn web_search_action_to_core(
    action: dcode_app_server_protocol::WebSearchAction,
) -> dcode_protocol::models::WebSearchAction {
    match action {
        dcode_app_server_protocol::WebSearchAction::Search { query, queries } => {
            dcode_protocol::models::WebSearchAction::Search { query, queries }
        }
        dcode_app_server_protocol::WebSearchAction::OpenPage { url } => {
            dcode_protocol::models::WebSearchAction::OpenPage { url }
        }
        dcode_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            dcode_protocol::models::WebSearchAction::FindInPage { url, pattern }
        }
        dcode_app_server_protocol::WebSearchAction::Other => {
            dcode_protocol::models::WebSearchAction::Other
        }
    }
}
...
impl ChatWidget {
    ...
    #[cfg(test)]
    fn handle_steer_rejected_error(&mut self, dcode_error_info: &CoreDcodeErrorInfo) -> bool {
        matches!(
            dcode_error_info,
            CoreDcodeErrorInfo::ActiveTurnNotSteerable { .. }
        ) && self.enqueue_rejected_steer()
    }

    fn handle_app_server_steer_rejected_error(
        &mut self,
        dcode_error_info: &AppServerDcodeErrorInfo,
    ) -> bool {
        matches!(
            dcode_error_info,
    ...
    fn handle_non_retry_error(
        &mut self,
        message: String,
        dcode_error_info: Option<AppServerDcodeErrorInfo>,
    ) {
        if dcode_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if let Some(info) = dcode_error_info
            .as_ref()
            ...
            CollabAgentTool::SpawnAgent => {
                ...
                if !matches!(status, CollabAgentToolCallStatus::InProgress) {
                    ...
                    self.on_collab_event(multi_agents::spawn_end(
                        dcode_protocol::protocol::CollabAgentSpawnEndEvent {
                            call_id: id,
                            sender_thread_id,
            ...
            CollabAgentTool::SendInput => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::interaction_end(
                        dcode_protocol::protocol::CollabAgentInteractionEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                ...
                if let Some(receiver_thread_id) = first_receiver {
                    if matches!(status, CollabAgentToolCallStatus::InProgress) {
                        self.on_collab_event(multi_agents::resume_begin(
                            dcode_protocol::protocol::CollabResumeBeginEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: None,
                        ...
                        self.on_collab_event(multi_agents::resume_end(
                            dcode_protocol::protocol::CollabResumeEndEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: None,
            ...
            CollabAgentTool::Wait => {
                if matches!(status, CollabAgentToolCallStatus::InProgress) {
                    self.on_collab_event(multi_agents::waiting_begin(
                        dcode_protocol::protocol::CollabWaitingBeginEvent {
                            sender_thread_id,
                    ...
                    self.on_collab_event(multi_agents::waiting_end(
                        dcode_protocol::protocol::CollabWaitingEndEvent {
                            sender_thread_id,
                            call_id: id,
                            agent_statuses,
                            statuses,
            ...
            CollabAgentTool::CloseAgent => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::close_end(
                        dcode_protocol::protocol::CollabCloseEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                            receiver_agent_nickname: None,
    ...
    pub(crate) fn new_with_app_event(common: ChatWidgetInit) -> Self {
        Self::new_with_op_target(common, DcodeOpTarget::AppEvent)
    }
    ...
    pub(crate) fn new_with_op_sender(
        ...
        Self::new_with_op_target(common, DcodeOpTarget::Direct(dcode_op_tx))
    }

    fn new_with_op_target(common: ChatWidgetInit, dcode_op_target: DcodeOpTarget) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
            app_event_tx,
    ...
    fn handle_thread_item(
        ...
        match item {
            ThreadItem::UserMessage { id, content } => {
                let user_message = dcode_protocol::items::UserMessageItem {
                    id,
                    content: content
                        .into_iter()
                        .map(dcode_app_server_protocol::UserInput::into_core)
                        .collect(),
                };
                let dcode_protocol::protocol::EventMsg::UserMessage(event) =
            ...
            ThreadItem::AgentMessage {
                ...
                self.on_agent_message_item_completed(AgentMessageItem {
                    ...
                    memory_citation: memory_citation.map(|citation| {
                        dcode_protocol::memory_citation::MemoryCitation {
                            entries: citation
                                .entries
                                .into_iter()
                                .map(
                                    |entry| dcode_protocol::memory_citation::MemoryCitationEntry {
                                        path: entry.path,
            ...
            ThreadItem::CommandExecution {
                ...
                if matches!(
                    status,
                    dcode_app_server_protocol::CommandExecutionStatus::InProgress
                ) {
                    ...
                    self.on_exec_command_end(ExecCommandEndEvent {
                        ...
                        status: match status {
                            dcode_app_server_protocol::CommandExecutionStatus::Completed => {
                                dcode_protocol::protocol::ExecCommandStatus::Completed
                            }
                            dcode_app_server_protocol::CommandExecutionStatus::Failed => {
                                dcode_protocol::protocol::ExecCommandStatus::Failed
                            }
                            dcode_app_server_protocol::CommandExecutionStatus::Declined => {
                                dcode_protocol::protocol::ExecCommandStatus::Declined
                            }
                            dcode_app_server_protocol::CommandExecutionStatus::InProgress => {
                                dcode_protocol::protocol::ExecCommandStatus::Failed
                            }
            ...
            ThreadItem::FileChange {
                ...
                if !matches!(
                    status,
                    dcode_app_server_protocol::PatchApplyStatus::InProgress
                ) {
                    self.on_patch_apply_end(dcode_protocol::protocol::PatchApplyEndEvent {
                        call_id: id,
                        turn_id: turn_id.clone(),
                        stdout: String::new(),
                        stderr: String::new(),
                        success: !matches!(
                            status,
                            dcode_app_server_protocol::PatchApplyStatus::Failed
                        ),
                        changes: app_server_patch_changes_to_core(changes),
                        status: match status {
                            dcode_app_server_protocol::PatchApplyStatus::Completed => {
                                dcode_protocol::protocol::PatchApplyStatus::Completed
                            }
                            dcode_app_server_protocol::PatchApplyStatus::Failed => {
                                dcode_protocol::protocol::PatchApplyStatus::Failed
                            }
                            dcode_app_server_protocol::PatchApplyStatus::Declined => {
                                dcode_protocol::protocol::PatchApplyStatus::Declined
                            }
                            dcode_app_server_protocol::PatchApplyStatus::InProgress => {
                                dcode_protocol::protocol::PatchApplyStatus::Failed
                            }
            ...
            ThreadItem::McpToolCall {
                ...
                self.on_mcp_tool_call_end(dcode_protocol::protocol::McpToolCallEndEvent {
                    call_id: id,
                    invocation: dcode_protocol::protocol::McpInvocation {
                        server,
                        tool,
                    ...
                    result: match (result, error) {
                        ...
                        (Some(result), None) => Ok(dcode_protocol::mcp::CallToolResult {
            ...
            ThreadItem::WebSearch { id, query, action } => {
                ...
                self.on_web_search_end(WebSearchEndEvent {
                    ...
                    action: action
                        ...
                        .unwrap_or(dcode_protocol::models::WebSearchAction::Other),
                });
            }
        ...
        match notification {
            ...
            ServerNotification::ThreadNameUpdated(notification) => {
                match ThreadId::from_string(&notification.thread_id) {
                    Ok(thread_id) => self.on_thread_name_updated(
                        dcode_protocol::protocol::ThreadNameUpdatedEvent {
                            thread_id,
            ...
            ServerNotification::CommandExecutionOutputDelta(notification) => {
                self.on_exec_command_output_delta(ExecCommandOutputDeltaEvent {
                    call_id: notification.item_id,
                    stream: dcode_protocol::protocol::ExecOutputStream::Stdout,
            ...
            ServerNotification::HookStarted(notification) => {
                if let Some(run) = convert_via_json(notification.run) {
                    self.on_hook_started(dcode_protocol::protocol::HookStartedEvent {
            ...
            ServerNotification::HookCompleted(notification) => {
                if let Some(run) = convert_via_json(notification.run) {
                    self.on_hook_completed(dcode_protocol::protocol::HookCompletedEvent {
                        turn_id: notification.turn_id,
                        run,
                    });
                }
            ...
            ServerNotification::Error(notification) => {
                if notification.will_retry {
                    ...
                    self.handle_non_retry_error(
                        notification.error.message,
                        notification.error.dcode_error_info,
                    );
                }
            }
            ...
            ServerNotification::ThreadRealtimeStarted(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_started(
                        dcode_protocol::protocol::RealtimeConversationStartedEvent {
            ...
            ServerNotification::ThreadRealtimeItemAdded(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        dcode_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: dcode_protocol::protocol::RealtimeEvent::ConversationItemAdded(
                                notification.item,
                            ),
                        },
                    );
            ...
            ServerNotification::ThreadRealtimeOutputAudioDelta(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        dcode_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: dcode_protocol::protocol::RealtimeEvent::AudioOut(
            ...
            ServerNotification::ThreadRealtimeError(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        dcode_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: dcode_protocol::protocol::RealtimeEvent::Error(
            ...
            ServerNotification::ThreadRealtimeClosed(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_closed(
                        dcode_protocol::protocol::RealtimeConversationClosedEvent {
    ...
    fn on_mcp_server_elicitation_request(
        ...
        params: dcode_app_server_protocol::McpServerElicitationRequestParams,
    ) {
        let request = dcode_protocol::approvals::ElicitationRequestEvent {
            turn_id: params.turn_id,
            server_name: params.server_name,
            id: request_id,
            request: match params.request {
                dcode_app_server_protocol::McpServerElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema,
                ...
                dcode_app_server_protocol::McpServerElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
            ...
            TurnStatus::Failed => {
                if let Some(error) = notification.turn.error {
                        ...
                        == Some(&(notification.turn.id.clone(), error.message.clone()))
                    {
                        ...
                        self.handle_non_retry_error(error.message, error.dcode_error_info);
                    }
                } else {
                    self.last_non_retry_error = None;
                    self.finalize_turn();
    ...
    fn on_guardian_review_notification(
        ...
        id: String,
        turn_id: String,
        review: dcode_app_server_protocol::GuardianApprovalReview,
        ...
        self.on_guardian_assessment(GuardianAssessmentEvent {
            ...
            status: match review.status {
                dcode_app_server_protocol::GuardianApprovalReviewStatus::InProgress => {
                    GuardianAssessmentStatus::InProgress
                }
                dcode_app_server_protocol::GuardianApprovalReviewStatus::Approved => {
                    GuardianAssessmentStatus::Approved
                }
                dcode_app_server_protocol::GuardianApprovalReviewStatus::Denied => {
                    GuardianAssessmentStatus::Denied
                }
                dcode_app_server_protocol::GuardianApprovalReviewStatus::Aborted => {
                    GuardianAssessmentStatus::Aborted
                }
            },
            risk_score: review.risk_score,
            risk_level: review.risk_level.map(|risk_level| match risk_level {
                dcode_app_server_protocol::GuardianRiskLevel::Low => {
                    dcode_protocol::protocol::GuardianRiskLevel::Low
                }
                dcode_app_server_protocol::GuardianRiskLevel::Medium => {
                    dcode_protocol::protocol::GuardianRiskLevel::Medium
                }
                dcode_app_server_protocol::GuardianRiskLevel::High => {
                    dcode_protocol::protocol::GuardianRiskLevel::High
                }
            }),
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                {
                } else if let Some(kind) = dcode_error_info
                    .as_ref()
    ...
    fn status_line_project_root(&self) -> Option<PathBuf> {
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_dcode_folder } => {
                    dot_dcode_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    fn status_line_value_for_item(&self, item: &StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::DcodeVersion => Some(CODEX_CLI_VERSION.to_string()),
    ...
    /// Forward a command directly to dcode.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.dcode_op_target {
            DcodeOpTarget::Direct(dcode_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = dcode_op_tx.send(op.into_core()) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            DcodeOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::DcodeOp(op.into()));
            }
        }
        true
    }
