...
use dcode_feedback::feedback_diagnostics::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use dcode_feedback::feedback_diagnostics::FeedbackDiagnostics;
...
const CODEX_FEEDBACK_INTERNAL_URL: &str = "http://go/dcode-feedback-internal";
            ...
            Ok(()) => {
                ...
                let mut lines = vec![Line::from(match issue_url.as_ref() {
                    Some(_) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        format!("{prefix} Please report this in #dcode-feedback:")
                    }
                ...
                match issue_url {
                    Some(url) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        lines.extend([
                            ...
                            Line::from(vec![
                                "    ".into(),
                                format!("https://go/dcode-feedback/{thread_id}").bold(),
                            ]),
                        ]);
                    }
                    Some(url) => {
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "dcode-logs.log".into()]).into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Dcode session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use dcode_feedback::feedback_diagnostics::FeedbackDiagnostic;
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = dcode_feedback::DcodeFeedback::new().snapshot(None);
    ...
    fn feedback_view_with_connectivity_diagnostics() {
        ...
        let snapshot = dcode_feedback::DcodeFeedback::new()
            .snapshot(None)
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/dcode-feedback-internal".to_string();
