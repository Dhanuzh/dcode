...
fn theme_picker_subtitle(dcode_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = dcode_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    dcode_home: Option<&Path>,
    terminal_width: Option<u16>,
) -> SelectionViewParams {
    // Snapshot the current theme so we can restore on cancel.
    let original_theme = highlight::current_syntax_theme();

    let entries = highlight::list_available_themes(dcode_home);
    let dcode_home_owned = dcode_home.map(Path::to_path_buf);
    ...
    let preview_home = dcode_home_owned.clone();
...
mod tests {
    ...
    #[test]
    fn subtitle_uses_tilde_path_when_dcode_home_under_home_directory() {
        let home = dirs::home_dir().expect("home directory should be available");
        let dcode_home = home.join(".dcode");

        let subtitle = theme_picker_subtitle(Some(&dcode_home), Some(200));
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let dcode_home = home.join(long_segment).join(".dcode");

        let subtitle = theme_picker_subtitle(Some(&dcode_home), Some(140));
    ...
    fn subtitle_falls_back_for_94_column_terminal_side_by_side_layout() {
        ...
        let subtitle = theme_picker_subtitle(Some(&dcode_home), Some(94));
