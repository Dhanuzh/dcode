...
#[cfg(test)]
use dcode_protocol::items::ContextCompactionItem;
#[cfg(test)]
use dcode_protocol::items::ImageGenerationItem;
#[cfg(test)]
        ...
        ServerNotification::Error(notification) => Some((
            ...
            vec![Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    dcode_error_info: notification
                        .error
                        .dcode_error_info
                        .and_then(app_server_dcode_error_info_to_core),
                }),
            }],
        )),
...
fn token_usage_from_app_server(
    value: dcode_app_server_protocol::TokenUsageBreakdown,
) -> TokenUsage {
    TokenUsage {
...
fn command_execution_completed_event(turn_id: &str, item: &ThreadItem) -> Option<Vec<Event>> {
    ...
    let status = match status {
        dcode_app_server_protocol::CommandExecutionStatus::InProgress => return Some(Vec::new()),
        dcode_app_server_protocol::CommandExecutionStatus::Completed => {
            ExecCommandStatus::Completed
        }
        dcode_app_server_protocol::CommandExecutionStatus::Failed => ExecCommandStatus::Failed,
        dcode_app_server_protocol::CommandExecutionStatus::Declined => ExecCommandStatus::Declined,
    };
...
) -> Option<dcode_protocol::models::WebSearchAction> {
    match action {
        dcode_app_server_protocol::WebSearchAction::Search { query, queries } => {
            Some(dcode_protocol::models::WebSearchAction::Search { query, queries })
        }
        dcode_app_server_protocol::WebSearchAction::OpenPage { url } => {
            Some(dcode_protocol::models::WebSearchAction::OpenPage { url })
        }
        dcode_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            Some(dcode_protocol::models::WebSearchAction::FindInPage { url, pattern })
        }
        dcode_app_server_protocol::WebSearchAction::Other => {
            Some(dcode_protocol::models::WebSearchAction::Other)
        }
    }
}

#[cfg(test)]
fn app_server_dcode_error_info_to_core(
    value: dcode_app_server_protocol::DcodeErrorInfo,
) -> Option<dcode_protocol::protocol::DcodeErrorInfo> {
...
mod tests {
    ...
    use dcode_app_server_protocol::DcodeErrorInfo;
    use dcode_app_server_protocol::CommandAction;
    ...
    fn bridges_thread_snapshot_turns_for_resume_restore() {
        ...
        assert_eq!(
            error.dcode_error_info,
            Some(dcode_protocol::protocol::DcodeErrorInfo::Other)
        );
            ...
            &Turn {
                ...
                items: vec![
                    ...
                    ThreadItem::WebSearch {
                        ...
                        action: Some(dcode_app_server_protocol::WebSearchAction::Other),
                    },
                    ThreadItem::ImageGeneration {
