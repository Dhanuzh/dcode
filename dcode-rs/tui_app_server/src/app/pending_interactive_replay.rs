...
impl ElicitationRequestKey {
    fn new(server_name: String, request_id: dcode_protocol::mcp::RequestId) -> Self {
        Self {
            server_name,
            request_id,
        }
...
fn app_server_request_id_to_mcp_request_id(
    ...
    match request_id {
        AppServerRequestId::String(value) => dcode_protocol::mcp::RequestId::String(value.clone()),
        AppServerRequestId::Integer(value) => dcode_protocol::mcp::RequestId::Integer(*value),
    }
}
...
mod tests {
    ...
    fn thread_event_snapshot_drops_resolved_elicitation_after_outbound_resolution() {
        let mut store = ThreadEventStore::new(8);
        let request_id = dcode_protocol::mcp::RequestId::String("request-1".to_string());
        store.push_request(elicitation_request("server-1", "request-1", "turn-1"));

        store.note_outbound_op(&Op::ResolveElicitation {
            server_name: "server-1".to_string(),
            request_id,
            decision: dcode_protocol::approvals::ElicitationAction::Accept,
            content: None,
            meta: None,
        });
