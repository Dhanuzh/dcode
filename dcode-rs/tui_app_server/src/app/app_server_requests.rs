...
use dcode_app_server_protocol::GrantedPermissionProfile;
...
use dcode_protocol::mcp::RequestId as McpRequestId;
                    ...
                    Ok::<AppServerRequestResolution, String>(AppServerRequestResolution {
                        ...
                        result: serde_json::to_value(McpServerElicitationRequestResponse {
                            action: match decision {
                                dcode_protocol::approvals::ElicitationAction::Accept => {
                                    McpServerElicitationAction::Accept
                                }
                                dcode_protocol::approvals::ElicitationAction::Decline => {
                                    McpServerElicitationAction::Decline
                                }
                                dcode_protocol::approvals::ElicitationAction::Cancel => {
...
mod tests {
    ...
    use dcode_app_server_protocol::McpElicitationObjectType;
    use dcode_app_server_protocol::McpElicitationSchema;
    use dcode_app_server_protocol::McpServerElicitationRequest;
    use dcode_app_server_protocol::McpServerElicitationRequestParams;
    use dcode_app_server_protocol::PermissionGrantScope;
    use dcode_app_server_protocol::PermissionsRequestApprovalParams;
    use dcode_app_server_protocol::PermissionsRequestApprovalResponse;
    use dcode_app_server_protocol::RequestId as AppServerRequestId;
    use dcode_app_server_protocol::ServerRequest;
    use dcode_app_server_protocol::ToolRequestUserInputAnswer;
    use dcode_app_server_protocol::ToolRequestUserInputParams;
    use dcode_app_server_protocol::ToolRequestUserInputResponse;
    use dcode_protocol::approvals::ElicitationAction;
    ...
    fn resolves_permissions_and_user_input_through_app_server_request_id() {
        ...
        let permissions = pending
            .take_resolution(&Op::RequestPermissionsResponse {
                id: "perm-1".to_string(),
                response: dcode_protocol::request_permissions::RequestPermissionsResponse {
                    permissions: serde_json::from_value(json!({
                        "network": { "enabled": null }
                    }))
                    .expect("valid permissions"),
                    scope: dcode_protocol::request_permissions::PermissionGrantScope::Session,
                },
            })
        ...
        let user_input = pending
            .take_resolution(&Op::UserInputAnswer {
                id: "turn-2".to_string(),
                response: dcode_protocol::request_user_input::RequestUserInputResponse {
    ...
    fn rejects_dynamic_tool_calls_as_unsupported() {
        ...
        let unsupported = pending
            .note_server_request(&ServerRequest::DynamicToolCall {
                ...
                params: dcode_app_server_protocol::DynamicToolCallParams {
                    thread_id: "thread-1".to_string(),
                    turn_id: "turn-1".to_string(),
    ...
    fn does_not_mark_chatgpt_auth_refresh_as_unsupported() {
        ...
        assert_eq!(
            pending.note_server_request(&ServerRequest::ChatgptAuthTokensRefresh {
                ...
                params: dcode_app_server_protocol::ChatgptAuthTokensRefreshParams {
                    reason: dcode_app_server_protocol::ChatgptAuthTokensRefreshReason::Unauthorized,
