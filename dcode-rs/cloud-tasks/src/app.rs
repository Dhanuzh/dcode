...
pub struct EnvironmentRow {
    ...
    pub is_pinned: bool,
    pub repo_hints: Option<String>, // e.g., "openai/dcode"
}
...
use dcode_cloud_tasks_client::CloudBackend;
...
pub struct AttemptView {
    ...
    pub status: dcode_cloud_tasks_client::AttemptStatus,
...
pub enum AppEvent {
    ...
    DetailsMessagesLoaded {
        ...
        attempt_status: dcode_cloud_tasks_client::AttemptStatus,
    },
    DetailsFailed {
        id: TaskId,
        title: String,
    ...
    AttemptsLoaded {
        id: TaskId,
        attempts: Vec<dcode_cloud_tasks_client::TurnAttempt>,
    },
    /// Background completion of new task submission
    NewTaskSubmitted(Result<dcode_cloud_tasks_client::CreatedTask, String>),
    ...
    ApplyFinished {
        id: TaskId,
        result: std::result::Result<dcode_cloud_tasks_client::ApplyOutcome, String>,
    },
}
...
mod tests {
    use super::*;
    use chrono::Utc;
    use dcode_cloud_tasks_client::CloudTaskError;
    ...
    impl dcode_cloud_tasks_client::CloudBackend for FakeBackend {
        async fn list_tasks(
            &self,
            env: Option<&str>,
            limit: Option<i64>,
            cursor: Option<&str>,
        ) -> dcode_cloud_tasks_client::Result<dcode_cloud_tasks_client::TaskListPage> {
            ...
            for (i, t) in titles.into_iter().enumerate() {
                out.push(TaskSummary {
                    ...
                    title: t.to_string(),
                    status: dcode_cloud_tasks_client::TaskStatus::Ready,
                    updated_at: Utc::now(),
                    environment_id: env.map(str::to_string),
                    environment_label: None,
                    summary: dcode_cloud_tasks_client::DiffSummary::default(),
                    is_review: false,
            ...
            Ok(dcode_cloud_tasks_client::TaskListPage {
                tasks: limited,
        ...
        async fn get_task_summary(
            &self,
            id: TaskId,
        ) -> dcode_cloud_tasks_client::Result<TaskSummary> {
        ...
        async fn get_task_diff(
            &self,
            _id: TaskId,
        ) -> dcode_cloud_tasks_client::Result<Option<String>> {
            Err(dcode_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
        ...
        async fn get_task_messages(
            &self,
            _id: TaskId,
        ) -> dcode_cloud_tasks_client::Result<Vec<String>> {
            Ok(vec![])
        }
        async fn get_task_text(
            &self,
            _id: TaskId,
        ) -> dcode_cloud_tasks_client::Result<dcode_cloud_tasks_client::TaskText> {
            Ok(dcode_cloud_tasks_client::TaskText {
                prompt: Some("Example prompt".to_string()),
                messages: Vec::new(),
                turn_id: Some("fake-turn".to_string()),
                sibling_turn_ids: Vec::new(),
                attempt_placement: Some(0),
                attempt_status: dcode_cloud_tasks_client::AttemptStatus::Completed,
            })
        }
        ...
        async fn list_sibling_attempts(
            ...
            _task: TaskId,
            _turn_id: String,
        ) -> dcode_cloud_tasks_client::Result<Vec<dcode_cloud_tasks_client::TurnAttempt>> {
            Ok(Vec::new())
        }
        ...
        ) -> dcode_cloud_tasks_client::Result<dcode_cloud_tasks_client::ApplyOutcome> {
            Err(dcode_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
        ...
        async fn create_task(
            ...
            _qa_mode: bool,
            _best_of_n: usize,
        ) -> dcode_cloud_tasks_client::Result<dcode_cloud_tasks_client::CreatedTask> {
            Err(dcode_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
