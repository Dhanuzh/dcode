...
use dcode_core::token_data::TokenData;
...
use dcode_connectors::AllConnectorsCacheKey;
use dcode_connectors::DirectoryListResponse;

pub use dcode_core::connectors::AppInfo;
pub use dcode_core::connectors::connector_display_label;
use dcode_core::connectors::filter_disallowed_connectors;
pub use dcode_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use dcode_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use dcode_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use dcode_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
use dcode_core::connectors::merge_connectors;
use dcode_core::connectors::merge_plugin_apps;
pub use dcode_core::connectors::with_app_enabled_state;
use dcode_core::plugins::AppConnectorId;
...
pub async fn list_cached_all_connectors(config: &Config) -> Option<Vec<AppInfo>> {
    ...
    if init_chatgpt_token_from_auth(&config.dcode_home, config.cli_auth_credentials_store_mode)
        .await
        .is_err()
    {
        return None;
    ...
    dcode_connectors::cached_all_connectors(&cache_key).map(|connectors| {
...
pub async fn list_all_connectors_with_options(
    ...
    init_chatgpt_token_from_auth(&config.dcode_home, config.cli_auth_credentials_store_mode)
        .await?;

    let token_data =
        get_chatgpt_token_data().ok_or_else(|| anyhow::anyhow!("ChatGPT token not available"))?;
    let cache_key = all_connectors_cache_key(config, &token_data);
    let connectors = dcode_connectors::list_all_connectors_with_options(
        cache_key,
...
fn plugin_apps_for_config(config: &Config) -> Vec<dcode_core::plugins::AppConnectorId> {
    PluginsManager::new(config.dcode_home.clone())
...
mod tests {
    use super::*;
    use dcode_core::connectors::connector_install_url;
    use dcode_core::plugins::AppConnectorId;
