...
mod tests {
    use super::*;
    use dcode_protocol::approvals::EscalationPermissions;
    ...
    /// executable paths.
    ///
    /// The `/bin/bash` and `/tmp/dcode-execve-wrapper` values here are
    ...
    async fn start_session_exposes_wrapper_env_overlay() -> anyhow::Result<()> {
        ...
        let execve_wrapper = PathBuf::from("/tmp/dcode-execve-wrapper");
    ...
    async fn exec_closes_parent_socket_after_shell_spawn() -> anyhow::Result<()> {
        ...
        let server = EscalateServer::new(
            PathBuf::from("/bin/bash"),
            PathBuf::from("/tmp/dcode-execve-wrapper"),
            DeterministicEscalationPolicy {
                decision: EscalationDecision::run(),
            },
        );
