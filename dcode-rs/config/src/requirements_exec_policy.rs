...
use dcode_execpolicy::rule::PatternToken;
use dcode_execpolicy::rule::PrefixPattern;
use dcode_execpolicy::rule::PrefixRule;
use dcode_execpolicy::rule::RuleRef;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Dcode merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `dcode-execpolicy`.
