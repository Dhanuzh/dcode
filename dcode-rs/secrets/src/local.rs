...
impl LocalSecretsBackend {
    pub fn new(dcode_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            dcode_home,
            keyring_store,
        }
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.dcode_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(dcode_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let dcode_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(dcode_home.path());
        keyring.set_error(
            &account,
