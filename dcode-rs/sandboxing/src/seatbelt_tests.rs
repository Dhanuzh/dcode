...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/dcode-unreadable");
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/dcode-readable");
    let unreadable = absolute_path("/tmp/dcode-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_dcode_subpaths() {
    // Create a temporary workspace with two writable roots: one containing
    // top-level .git and .dcode directories and one without them.
    let tmp = TempDir::new().expect("tempdir");
    let PopulatedTmp {
        vulnerable_root,
    ...
    // write to .dcode/config.toml in the vulnerable root.
    ...
    // - write access to WRITABLE_ROOT_0 (but not its .git or .dcode), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
"#,
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_dcode_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .dcode/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_dcode_canonical.join("config.toml");
    let output = Command::new(MACOS_PATH_TO_SEATBELT_EXECUTABLE)
        .args(&args)
        .current_dir(&cwd)
        .output()
    ...
    // Verify that writing a file to the folder containing .git and .dcode is allowed.
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    // `.dcode` checks are done properly for cwd.
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: Default::default(),
        network_access: false,
...
struct PopulatedTmp {
    /// Path containing a .git and .dcode subfolder.
    ...
    /// ran `git commit` themselves, or modified .dcode/config.toml to
    ...
    dot_dcode_canonical: PathBuf,

    /// Path without .git or .dcode subfolders.
    empty_root: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".dcode")).expect("create .dcode");
    fs::write(
        vulnerable_root.join(".dcode").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .dcode/config.toml");
    ...
    let dot_dcode_canonical = vulnerable_root_canonical.join(".dcode");
