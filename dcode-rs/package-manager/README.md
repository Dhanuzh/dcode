# dcode-package-manager

`dcode-package-manager` is the shared installer used for versioned runtime bundles and other cached artifacts in `dcode-rs`.
...
```text
<dcode_home>/<default_cache_root_relative>
```
...
```rust,ignore
let config = PackageManagerConfig::new(dcode_home, MyPackage::new(...));
