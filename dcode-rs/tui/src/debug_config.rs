...
use dcode_core::config_loader::ConfigLayerEntry;
...
use dcode_core::config_loader::ResidencyRequirement;
use dcode_core::config_loader::SandboxModeRequirement;
use dcode_core::config_loader::WebSearchModeRequirement;
    ...
    if let Some(proxy) = session_network_proxy {
        ...
        let all_proxy = session_all_proxy_url(
            ...
            config
                ...
                .network
                .as_ref()
                .is_some_and(dcode_core::config::NetworkProxySpec::socks_enabled),
        );
...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project { dot_dcode_folder } => {
            format!(
                "project ({}/config.toml)",
                dot_dcode_folder.as_path().display()
            )
        }
...
mod tests {
    ...
    use dcode_core::config_loader::ConstrainedWithSource;
    use dcode_core::config_loader::McpServerIdentity;
    use dcode_core::config_loader::McpServerRequirement;
    use dcode_core::config_loader::NetworkConstraints;
    use dcode_core::config_loader::RequirementSource;
    use dcode_core::config_loader::ResidencyRequirement;
    use dcode_core::config_loader::SandboxModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\dcode\\config.toml")
        } else {
            absolute_path("/etc/dcode/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.dcode")
        } else {
            absolute_path("/repo/.dcode")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_dcode_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Dcode\\requirements.toml")
        } else {
            absolute_path("/etc/dcode/requirements.toml")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.dcode\\config.toml")
        } else {
            absolute_path("/home/alice/.dcode/config.toml")
        };
        let stack = ConfigLayerStack::new(
