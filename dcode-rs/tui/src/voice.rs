...
fn send_realtime_audio_chunk(
    ...
    tx.send(AppEvent::DcodeOp(Op::RealtimeConversationAudio(
...
async fn resolve_auth() -> Result<TranscriptionAuthContext, String> {
    let dcode_home = find_dcode_home().map_err(|e| format!("failed to find dcode home: {e}"))?;
    let auth = DcodeAuth::from_auth_storage(&dcode_home, AuthCredentialsStoreMode::Auto)
        .map_err(|e| format!("failed to read auth.json: {e}"))?
        .ok_or_else(|| "No Dcode auth is configured; please run `dcode login`".to_string())?;
    ...
    let (endpoint, request) =
        if matches!(auth.mode, AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens) {
            ...
            let mut req = client
                ...
                .multipart(form)
                .header("User-Agent", get_dcode_user_agent());
