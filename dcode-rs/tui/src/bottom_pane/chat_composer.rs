...
//!
//! For the detailed burst state machine, see `dcode-rs/tui/src/bottom_pane/paste_burst.rs`.
...
use dcode_protocol::custom_prompts::PROMPTS_CMD_PREFIX;
...
use dcode_utils_fuzzy_match::fuzzy_match;
...
use dcode_core::plugins::PluginCapabilitySummary;
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let slug = dcode_core::connectors::connector_mention_slug(connector);
...
mod tests {
    ...
    fn mention_items_hide_plugin_owned_skill_and_app_duplicates() {
        ...
        composer.set_skill_mentions(Some(vec![SkillMetadata {
            ...
            interface: Some(dcode_core::skills::model::SkillInterface {
            ...
            scope: dcode_protocol::protocol::SkillScope::Repo,
        }]));
        composer.set_plugin_mentions(Some(vec![PluginCapabilitySummary {
            ...
            app_connector_ids: vec![dcode_core::plugins::AppConnectorId(
...
程序员 与 Unicode 同行";
    ...
    fn file_completion_preserves_large_paste_placeholder_elements() {
        ...
        composer.on_file_search_result(
            ...
            vec![FileMatch {
                ...
                match_type: dcode_file_search::MatchType::File,
    ...
    fn pasting_filepath_attaches_image() {
        ...
        let tmp_path: PathBuf = tmp.path().join("dcode_tui_test_paste_image.png");
