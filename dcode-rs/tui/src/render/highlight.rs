...
/// fork reloads complete). The first call persists `name` and `dcode_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), dcode_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let dcode_home_set_ok = CODEX_HOME.set(dcode_home.clone()).is_ok();
    ...
    if !override_set_ok || !dcode_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, dcode_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = dcode_home
...
fn custom_theme_path(name: &str, dcode_home: &Path) -> PathBuf {
    dcode_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{dcode_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, dcode_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, dcode_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, dcode_home: Option<&Path>) -> Theme {
...
fn build_default_theme() -> Theme {
    ...
    let dcode_home = CODEX_HOME
        .get()
        .and_then(|dcode_home| dcode_home.as_deref());
    resolve_theme_with_override(name, dcode_home)
}
...
pub(crate) fn resolve_theme_by_name(name: &str, dcode_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// found in `{dcode_home}/themes/`.
pub(crate) fn list_available_themes(dcode_home: Option<&Path>) -> Vec<ThemeEntry> {
