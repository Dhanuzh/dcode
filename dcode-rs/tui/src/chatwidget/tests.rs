...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let thread_manager = Arc::new(
        dcode_core::test_support::thread_manager_with_models_provider(
            DcodeAuth::from_api_key("test"),
            cfg.model_provider.clone(),
        ),
    );
    let auth_manager =
        dcode_core::test_support::auth_manager_from_auth(DcodeAuth::from_api_key("test"));
...
async fn make_chatwidget_manual(
    ...
    let auth_manager =
        dcode_core::test_support::auth_manager_from_auth(DcodeAuth::from_api_key("test"));
    let dcode_home = cfg.dcode_home.clone();
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    chat.auth_manager = dcode_core::test_support::auth_manager_from_auth(
        DcodeAuth::create_dummy_chatgpt_auth_for_testing(),
    );
    chat.models_manager = Arc::new(ModelsManager::new(
        chat.config.dcode_home.clone(),
...
async fn rate_limit_switch_prompt_skips_non_dcode_limit() {
    let auth = DcodeAuth::create_dummy_chatgpt_auth_for_testing();
    let (mut chat, _, _) = make_chatwidget_manual(Some("gpt-5")).await;
    chat.auth_manager = dcode_core::test_support::auth_manager_from_auth(auth);
...
async fn terminal_title_can_render_app_name_item() {
    ...
    assert_eq!(chat.last_terminal_title, Some("dcode".to_string()));
}
            ...
            chat.config
                ...
                .find_map(|layer| match &layer.name {
                    ConfigLayerSource::Project { dot_dcode_folder } => {
                        dot_dcode_folder.as_path().parent().map(|path| {
                            path.file_name()
                                .map(|name| name.to_string_lossy().to_string())
