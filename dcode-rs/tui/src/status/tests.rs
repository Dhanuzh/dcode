...
async fn test_config(temp_home: &TempDir) -> Config {
    ConfigBuilder::default()
        .dcode_home(temp_home.path().to_path_buf())
        .build()
        .await
        .expect("load config")
}
...
fn token_info_for(model_slug: &str, config: &Config, usage: &TokenUsage) -> TokenUsageInfo {
    let context_window =
        dcode_core::test_support::construct_model_info_offline(model_slug, config).context_window;
    TokenUsageInfo {
...
async fn status_snapshot_includes_reasoning_details() {
    ...
    let model_slug = dcode_core::test_support::get_model_offline(config.model.as_deref());
...
async fn status_snapshot_includes_credits_and_limits() {
    let temp_home = TempDir::new().expect("temp home");
    let mut config = test_config(&temp_home).await;
    config.model = Some("gpt-5.1-dcode".to_string());
    config.cwd = PathBuf::from("/workspace/tests");
