...
mod tests {
    ...
    fn thread_event_snapshot_keeps_pending_request_user_input() {
        ...
        let request = Event {
            ...
            msg: EventMsg::RequestUserInput(
                dcode_protocol::request_user_input::RequestUserInputEvent {
    ...
    fn thread_event_snapshot_drops_resolved_exec_approval_after_outbound_approval_id() {
        ...
        store.note_outbound_op(&Op::ExecApproval {
            ...
            decision: dcode_protocol::protocol::ReviewDecision::Approved,
        });
    ...
    fn thread_event_snapshot_drops_resolved_patch_approval_after_outbound_approval() {
        ...
        store.push_event(Event {
            ...
            msg: EventMsg::ApplyPatchApprovalRequest(
                dcode_protocol::protocol::ApplyPatchApprovalRequestEvent {
                    call_id: "call-1".to_string(),
                    turn_id: "turn-1".to_string(),
                    changes: HashMap::new(),
                    reason: None,
    ...
    fn thread_event_snapshot_drops_resolved_elicitation_after_outbound_resolution() {
        ...
        store.push_event(Event {
            id: "ev-1".to_string(),
            msg: EventMsg::ElicitationRequest(dcode_protocol::approvals::ElicitationRequestEvent {
                turn_id: Some("turn-1".to_string()),
                server_name: "server-1".to_string(),
                id: request_id.clone(),
                request: dcode_protocol::approvals::ElicitationRequest::Form {
                    meta: None,
