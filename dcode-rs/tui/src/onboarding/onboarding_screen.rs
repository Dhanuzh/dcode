...
impl OnboardingScreen {
    pub(crate) fn new(tui: &mut Tui, args: OnboardingScreenArgs) -> Self {
        ...
        if show_login_screen {
            let highlighted_mode = if config.model_provider_id == "github-copilot" {
                // For the GitHub Copilot provider, default to the GithubCopilot
                // OAuth device-code flow rather than the OpenAI/ChatGPT options.
                SignInOption::GithubCopilot
            } else if !config.model_provider.requires_openai_auth
                && config.model_provider.env_key.is_some()
            {
                // Other third-party providers (Groq, Anthropic, etc.) authenticate
                // with a plain API key stored in an environment variable.
                SignInOption::ApiKey
            } else {
