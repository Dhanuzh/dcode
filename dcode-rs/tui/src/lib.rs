...
// The standalone `dcode-tui` binary prints a short help message before the
...
use dcode_core::INTERACTIVE_SESSION_SOURCES;
use dcode_core::RolloutRecorder;
use dcode_core::ThreadSortKey;
...
mod voice {
    ...
    use dcode_core::config::Config;
    use dcode_protocol::protocol::RealtimeAudioFrame;
    use std::sync::Arc;
    use std::sync::Mutex;
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = dcode_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let dcode_home = match find_dcode_home() {
        Ok(dcode_home) => dcode_home.to_path_buf(),
        Err(err) => {
    ...
    if let Err(err) =
        dcode_core::personality_migration::maybe_migrate_personality(&dcode_home, &config_toml)
            .await
    {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&dcode_home).await?;
    ...
    let log_dir = dcode_core::config::log_dir(&config)?;
    ...
    // Dcode CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("dcode-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for dcode crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("dcode_core=info,dcode_tui=info,dcode_rmcp_client=info")
        })
    };
    ...
    let feedback = dcode_feedback::DcodeFeedback::new();
    ...
    let log_db_layer = dcode_core::state_db::get_state_db(&config)
        .await
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: dcode_protocol::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let auth_manager = AuthManager::shared(
        initial_config.dcode_home.clone(),
        /*enable_dcode_api_key_env*/ false,
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            let path = if is_uuid {
                find_thread_path_by_id_str(&config.dcode_home, id_str).await?
            } else {
                find_thread_path_by_name_str(&config.dcode_home, id_str).await?
            };
            match path {
                Some(path) => {
                    let thread_id =
                        ...
                        .await
                        {
                            ...
                            None => {
                                ...
                                return Ok(AppExitInfo {
                                    ...
                                    update_action: None,
                                    exit_reason: ExitReason::Fatal(format!(
                                        "Found latest saved session at {rollout_path}, but failed to read its metadata. Run `dcode fork` to choose from existing sessions."
                                    )),
                                });
                            }
                        }
                ...
                match resolve_session_thread_id(path.as_path(), /*id_str_if_uuid*/ None).await {
                    ...
                    None => {
                        ...
                        return Ok(AppExitInfo {
                            ...
                            update_action: None,
                            exit_reason: ExitReason::Fatal(format!(
                                "Found latest saved session at {rollout_path}, but failed to read its metadata. Run `dcode resume` to choose from existing sessions."
                            )),
                        });
                    }
                }
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let dcode_home = config.dcode_home.clone();
        match DcodeAuth::from_auth_storage(&dcode_home, config.cli_auth_credentials_store_mode) {
    ...
    } else if config.model_provider.experimental_bearer_token.is_some() {
        // A bearer token is directly embedded in the provider config.
        LoginStatus::AuthMode(AuthMode::ApiKey)
    } else if let Some(ref env_key) = config.model_provider.env_key {
        // Provider authenticates via an environment variable (e.g. GITHUB_COPILOT_TOKEN).
            ...
            .is_some()
        {
            ...
            // Also check auth.json — the GitHub Copilot OAuth device-code flow
            // stores its token there via `login_with_api_key`.
            let dcode_home = config.dcode_home.clone();
            match DcodeAuth::from_auth_storage(
                &dcode_home,
        ...
        // No auth required (e.g. local models).
        LoginStatus::AuthMode(AuthMode::ApiKey)
    }
}
...
fn should_show_login_screen(login_status: LoginStatus, config: &Config) -> bool {
    // For providers with no auth requirements (e.g. local OSS models), skip login.
    let provider_requires_auth = config.model_provider.requires_openai_auth
...
mod tests {
    ...
    use dcode_core::config::ConfigBuilder;
    use dcode_core::config::ConfigOverrides;
    use dcode_core::config::ProjectConfig;
    ...
    fn build_turn_context(config: &Config, cwd: PathBuf) -> TurnContextItem {
        ...
        TurnContextItem {
            ...
            summary: config
                ...
                .unwrap_or(dcode_protocol::config_types::ReasoningSummary::Auto),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let dcode_home = temp_dir.path().to_path_buf();
...
"#
    ...
    async fn read_session_cwd_prefers_sqlite_when_thread_id_present() -> std::io::Result<()> {
        ...
        let runtime = dcode_state::StateRuntime::init(
        ...
        let mut builder = dcode_state::ThreadMetadataBuilder::new(
            thread_id,
            rollout_path.clone(),
            chrono::Utc::now(),
            SessionSource::Cli,
