...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    // run_dcode_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
    ...
    let source_catalog_path = dcode_utils_cargo_bin::find_resource!("../core/models.json")?;
    ...
    let custom_catalog_path = dcode_home.path().join("catalog.json");
    std::fs::write(
        &custom_catalog_path,
...
"#
    ...
    std::fs::write(dcode_home.path().join("config.toml"), config_contents)?;

    let fixture_path =
        dcode_utils_cargo_bin::find_resource!("../core/tests/cli_responses_fixture.sse")?;
    let dcode = if let Ok(path) = dcode_utils_cargo_bin::cargo_bin("dcode") {
        path
    } else {
        let fallback = repo_root.join("dcode-rs/target/debug/dcode");
        if fallback.is_file() {
            fallback
        } else {
            eprintln!("skipping integration test because dcode binary is unavailable");
            return Ok(());
        }
    };

    let exec_output = std::process::Command::new(&dcode)
        .arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-C")
        .arg(&repo_root)
        .arg("seed session for resume")
        .env("DCODE_HOME", dcode_home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("CODEX_RS_SSE_FIXTURE", fixture_path)
        .env("OPENAI_BASE_URL", "http://unused.local")
        .output()
        .context("failed to execute dcode exec")?;
    anyhow::ensure!(
    ...
    env.insert(
        "DCODE_HOME".to_string(),
        dcode_home.path().display().to_string(),
    );
    ...
    let spawned = dcode_utils_pty::spawn_pty_process(
        dcode.to_string_lossy().as_ref(),
        &args,
        &repo_root,
        &env,
        &None,
        dcode_utils_pty::TerminalSize::default(),
    )
    .await?;

    let mut output = Vec::new();
    let dcode_utils_pty::SpawnedProcess {
        session,
        stdout_rx,
        stderr_rx,
        exit_rx,
    } = spawned;
    let mut output_rx = dcode_utils_pty::combine_output_receivers(stdout_rx, stderr_rx);
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for dcode resume to exit");
        }
    };
    anyhow::ensure!(
        exit_code == 0 || exit_code == 130,
        "unexpected exit code from dcode resume: {exit_code}; output: {}",
        String::from_utf8_lossy(&output)
    );

    let config_contents = std::fs::read_to_string(dcode_home.path().join("config.toml"))?;
