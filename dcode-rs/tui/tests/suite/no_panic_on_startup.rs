...
"#,
    ...
    std::fs::write(dcode_home.join("config.toml"), config_contents)?;

    let DcodeCliOutput { exit_code, output } = run_dcode_cli(dcode_home, cwd).await?;
    assert_ne!(0, exit_code, "Dcode CLI should exit nonzero.");
    assert!(
        output.contains("ERROR: Failed to initialize dcode:"),
...
struct DcodeCliOutput {
    exit_code: i32,
    output: String,
}

async fn run_dcode_cli(
    dcode_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<DcodeCliOutput> {
    let dcode_cli = dcode_utils_cargo_bin::cargo_bin("dcode")?;
    let mut env = HashMap::new();
    env.insert(
        "DCODE_HOME".to_string(),
        dcode_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = dcode_utils_pty::spawn_pty_process(
        dcode_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for dcode CLI to exit");
        }
    };
