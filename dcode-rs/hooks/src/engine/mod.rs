...
use dcode_config::ConfigLayerStack;
...
pub(crate) struct ConfiguredHandler {
    pub event_name: dcode_protocol::protocol::HookEventName,
...
impl ConfiguredHandler {
    ...
    fn event_name_label(&self) -> &'static str {
        match self.event_name {
            dcode_protocol::protocol::HookEventName::PreToolUse => "pre-tool-use",
            dcode_protocol::protocol::HookEventName::SessionStart => "session-start",
            dcode_protocol::protocol::HookEventName::UserPromptSubmit => "user-prompt-submit",
    ...
    pub(crate) fn new(
        ...
        if cfg!(windows) {
            return Self {
                handlers: Vec::new(),
                warnings: vec![
                    "Disabled `dcode_hooks` for this session because `hooks.json` lifecycle hooks are not supported on Windows yet."
                        .to_string(),
                ],
                shell,
            };
