...
/// Special argv[1] flag used when the Dcode executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `dcode-apply-patch` (to avoid forcing
/// `dcode-arg0` to depend on `dcode-core`), it is part of the "dcode core"
/// process-invocation contract between the apply-patch runtime and the arg0
/// dispatcher.
pub const CODEX_CORE_APPLY_PATCH_ARG1: &str = "--dcode-run-as-apply-patch";
