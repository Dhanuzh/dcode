...
use crate::dcode_message_processor::DcodeMessageProcessor;
use crate::dcode_message_processor::DcodeMessageProcessorArgs;
...
use dcode_app_server_protocol::ChatgptAuthTokensRefreshParams;
use dcode_app_server_protocol::ChatgptAuthTokensRefreshReason;
...
use dcode_app_server_protocol::ExperimentalApi;
use dcode_app_server_protocol::ExternalAgentConfigDetectParams;
use dcode_app_server_protocol::ExternalAgentConfigImportParams;
...
use dcode_app_server_protocol::experimental_required_message;
use dcode_arg0::Arg0DispatchPaths;
use dcode_core::AnalyticsEventsClient;
...
use dcode_core::default_client::DEFAULT_ORIGINATOR;
use dcode_core::default_client::SetOriginatorError;
use dcode_core::default_client::USER_AGENT_SUFFIX;
use dcode_core::default_client::get_dcode_user_agent;
...
use dcode_login::auth::ExternalAuthRefreshContext;
use dcode_login::auth::ExternalAuthRefreshReason;
use dcode_login::auth::ExternalAuthRefresher;
use dcode_login::auth::ExternalAuthTokens;
...
const TUI_APP_SERVER_CLIENT_NAME: &str = "dcode-tui";
...
pub(crate) struct MessageProcessor {
    outgoing: Arc<OutgoingMessageSender>,
    dcode_message_processor: DcodeMessageProcessor,
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) enable_dcode_api_key_env: bool,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let dcode_message_processor = DcodeMessageProcessor::new(DcodeMessageProcessorArgs {
        ...
        let external_agent_config_api = ExternalAgentConfigApi::new(config.dcode_home.clone());
        let fs_api = FsApi::default();

        Self {
            outgoing,
            dcode_message_processor,
            config_api,
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                ...
                let dcode_request = match serde_json::from_value::<ClientRequest>(request_json) {
                    Ok(dcode_request) => dcode_request,
                    Err(err) => {
                        let error = JSONRPCErrorError {
                            code: INVALID_REQUEST_ERROR_CODE,
                            message: format!("Invalid request: {err}"),
    ...
    pub(crate) fn thread_created_receiver(&self) -> broadcast::Receiver<ThreadId> {
        self.dcode_message_processor.thread_created_receiver()
    }
    ...
    pub(crate) async fn drain_background_tasks(&self) {
        self.dcode_message_processor.drain_background_tasks().await;
    }
    ...
    pub(crate) async fn shutdown_threads(&self) {
        self.dcode_message_processor.shutdown_threads().await;
    }
    ...
    async fn handle_client_request(
        ...
        match dcode_request {
            // Handle Initialize internally so DcodeMessageProcessor does not have to concern
            ...
            ClientRequest::Initialize { request_id, params } => {
                ...
                let originator = if name == TUI_APP_SERVER_CLIENT_NAME {
                    ...
                    // need to retain the legacy TUI `dcode_cli_rs` originator behavior.
                ...
                let user_agent = get_dcode_user_agent();
        ...
        if let Some(reason) = dcode_request.experimental_reason()
        ...
        match dcode_request {
            ...
            other => {
                ...
                // inline the full `DcodeMessageProcessor::process_request` future, which
    ...
    async fn handle_config_value_write(
        ...
        match self.config_api.write_value(params).await {
            Ok(response) => {
                self.dcode_message_processor.clear_plugin_related_caches();
                self.dcode_message_processor
