...
fn init_test_tracing() -> &'static TestTracing {
    ...
    TEST_TRACING.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("dcode-app-server-message-processor-tests");
...
impl TracingHarness {
    async fn new() -> Result<Self> {
        ...
        let config = Arc::new(build_test_config(dcode_home.path(), &server.uri()).await?);
                ...
                ClientRequest::Initialize {
                    ...
                    params: InitializeParams {
                        client_info: ClientInfo {
                            name: "dcode-app-server-tests".to_string(),
                            title: None,
...
async fn build_test_config(dcode_home: &Path, server_uri: &str) -> Result<Config> {
    ...
    loop {
        ...
        match envelope {
            crate::outgoing_message::OutgoingEnvelope::ToConnection {
                ...
                if matches!(
                    notification,
                    dcode_app_server_protocol::ServerNotification::ThreadStarted(_)
                ) {
                    return;
                }
            }
...
async fn turn_start_jsonrpc_span_parents_core_turn_spans() -> Result<()> {
    ...
    let spans = wait_for_exported_spans(harness.tracing, |spans| {
        spans.iter().any(|span| {
            ...
            span_attr(span, "dcode.op") == Some("user_input")
                && span.span_context.trace_id() == remote_trace_id
        })
    })
    .await;
    ...
    let core_turn_span =
        find_span_with_trace(&spans, remote_trace_id, "dcode.op=user_input", |span| {
            span_attr(span, "dcode.op") == Some("user_input")
        });
