...
use dcode_app_server_protocol::ConfigReadResponse;
use dcode_app_server_protocol::ConfigRequirements;
use dcode_app_server_protocol::ConfigRequirementsReadResponse;
use dcode_app_server_protocol::ConfigValueWriteParams;
use dcode_app_server_protocol::ConfigWriteErrorCode;
use dcode_app_server_protocol::ConfigWriteResponse;
use dcode_app_server_protocol::JSONRPCErrorError;
use dcode_app_server_protocol::NetworkRequirements;
use dcode_app_server_protocol::SandboxMode;
use dcode_core::AnalyticsEventsClient;
use dcode_core::ThreadManager;
use dcode_core::config::ConfigService;
use dcode_core::config::ConfigServiceError;
...
use dcode_core::config_loader::ResidencyRequirement as CoreResidencyRequirement;
use dcode_core::config_loader::SandboxModeRequirement as CoreSandboxModeRequirement;
use dcode_core::plugins::PluginId;
use dcode_core::plugins::collect_plugin_enabled_candidates;
use dcode_core::plugins::installed_plugin_telemetry_metadata;
...
impl ConfigApi {
    ...
    fn emit_plugin_toggle_events(&self, pending_changes: std::collections::BTreeMap<String, bool>) {
        for (plugin_id, enabled) in pending_changes {
            ...
            let metadata =
                installed_plugin_telemetry_metadata(self.dcode_home.as_path(), &plugin_id);
            if enabled {
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(dcode_app_server_protocol::AskForApproval::from)
                .collect()
        }),
...
) -> dcode_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => dcode_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: dcode_core::config_loader::NetworkRequirementsToml,
) -> NetworkRequirements {
    NetworkRequirements {
...
mod tests {
    use super::*;
    use dcode_core::AnalyticsEventsClient;
    use dcode_core::config_loader::NetworkRequirementsToml as CoreNetworkRequirementsToml;
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        let requirements = ConfigRequirementsToml {
            ...
            allowed_web_search_modes: Some(vec![
                dcode_core::config_loader::WebSearchModeRequirement::Cached,
            ]),
            guardian_developer_instructions: None,
            feature_requirements: Some(dcode_core::config_loader::FeatureRequirementsToml {
        ...
        assert_eq!(
            ...
            Some(vec![
                dcode_app_server_protocol::AskForApproval::Never,
                dcode_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(dcode_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        let dcode_home = TempDir::new().expect("create temp dir");
        let user_config_path = dcode_home.path().join("config.toml");
        ...
        let config_api = ConfigApi::new(
            ...
            AnalyticsEventsClient::new(
                analytics_config,
                dcode_core::test_support::auth_manager_from_auth(
                    dcode_core::DcodeAuth::from_api_key("test"),
                ),
            ),
        );
        ...
        let response = config_api
            .batch_write(ConfigBatchWriteParams {
                edits: vec![dcode_app_server_protocol::ConfigEdit {
                    key_path: "model".to_string(),
                    value: json!("gpt-5"),
                    merge_strategy: dcode_app_server_protocol::MergeStrategy::Replace,
                }],
        ...
        assert_eq!(
            response,
            ConfigWriteResponse {
                status: dcode_app_server_protocol::WriteStatus::Ok,
                version: response.version.clone(),
                file_path: dcode_utils_absolute_path::AbsolutePathBuf::try_from(
