...
//!
//! # Relationship to `dcode-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `dcode-app-server-client`,
...
use dcode_app_server_protocol::Result;
...
/// from the legacy `dcode_protocol::Event` model to the typed app-server
...
/// through `dcode-app-server-client`, which adds worker-task buffering,
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let mut processor_handle = tokio::spawn(async move {
            let mut processor = MessageProcessor::new(MessageProcessorArgs {
                ...
                enable_dcode_api_key_env: args.enable_dcode_api_key_env,
            });
...
mod tests {
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "dcode-in-process-test".to_string(),
                    title: None,
    ...
    fn guaranteed_delivery_helpers_cover_terminal_notifications() {
        ...
        assert!(legacy_notification_requires_delivery(
            &JSONRPCNotification {
                method: "dcode/event/task_complete".to_string(),
                params: None,
            }
        ));
        assert!(legacy_notification_requires_delivery(
        ...
        assert!(legacy_notification_requires_delivery(
            &JSONRPCNotification {
                method: "dcode/event/shutdown_complete".to_string(),
                params: None,
            }
        ));
        assert!(!legacy_notification_requires_delivery(
            &JSONRPCNotification {
                method: "dcode/event/item_started".to_string(),
                params: None,
            }
        ));
    }
