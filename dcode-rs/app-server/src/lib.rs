...
use dcode_app_server_protocol::TextPosition as AppTextPosition;
use dcode_app_server_protocol::TextRange as AppTextRange;
use dcode_core::ExecPolicyError;
use dcode_core::check_execpolicy_for_warnings;
use dcode_core::config_loader::ConfigLoadError;
use dcode_core::config_loader::TextRange as CoreTextRange;
...
enum OutboundControlEvent {
    ...
    Opened {
        ...
        // Allow dcode/event/* notifications to be emitted.
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        if let ConfigLayerSource::Project { dot_dcode_folder } = &layer.name {
...
pub async fn run_main_with_transport(
        ...
        .await
    {
        Ok(config) => {
            ...
            match effective_toml.try_into() {
                Ok(config_toml) => {
                    if let Err(err) = dcode_core::personality_migration::maybe_migrate_personality(
                        &config.dcode_home,
                        &config_toml,
                    )
                    .await
    ...
    if let Some(warning) = dcode_core::config::system_bwrap_warning() {
        config_warnings.push(ConfigWarningNotification {
            summary: warning,
            details: None,
            path: None,
    ...
    let feedback = DcodeFeedback::new();

    let otel = dcode_core::otel_init::build_provider(
        &config,
        env!("CARGO_PKG_VERSION"),
        Some("dcode-app-server"),
        default_analytics_enabled,
    )
    .map_err(|e| {
        std::io::Error::new(
    ...
    let log_db = dcode_state::StateRuntime::init(
