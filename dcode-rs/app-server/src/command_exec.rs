...
use dcode_app_server_protocol::CommandExecOutputDeltaNotification;
use dcode_app_server_protocol::CommandExecOutputStream;
use dcode_app_server_protocol::CommandExecResizeParams;
use dcode_app_server_protocol::CommandExecResizeResponse;
use dcode_app_server_protocol::CommandExecResponse;
use dcode_app_server_protocol::CommandExecTerminalSize;
use dcode_app_server_protocol::CommandExecTerminateParams;
use dcode_app_server_protocol::CommandExecTerminateResponse;
use dcode_app_server_protocol::CommandExecWriteParams;
use dcode_app_server_protocol::CommandExecWriteResponse;
use dcode_app_server_protocol::JSONRPCErrorError;
use dcode_app_server_protocol::ServerNotification;
use dcode_core::bytes_to_string_smart;
use dcode_core::config::StartedNetworkProxy;
use dcode_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS;
use dcode_core::exec::ExecExpiration;
use dcode_core::exec::IO_DRAIN_TIMEOUT_MS;
use dcode_core::exec::SandboxType;
use dcode_core::sandboxing::ExecRequest;
use dcode_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use dcode_utils_pty::ProcessHandle;
use dcode_utils_pty::SpawnedProcess;
use dcode_utils_pty::TerminalSize;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match dcode_core::sandboxing::execute_env(exec_request, /*stdout_stream*/ None)
                    .await
                {
                    Ok(output) => {
                        outgoing
        ...
        let spawned = if tty {
            dcode_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            dcode_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0).await
        } else {
            dcode_utils_pty::spawn_pipe_process_no_stdin(program, args, cwd.as_path(), &env, &arg0)
                .await
        };
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest {
            ...
            capture_policy: dcode_core::exec::ExecCapturePolicy::ShellTool,
            sandbox: SandboxType::WindowsRestrictedToken,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
            windows_sandbox_private_desktop: false,
            sandbox_permissions: dcode_core::sandboxing::SandboxPermissions::UseDefault,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: dcode_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: dcode_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: dcode_app_server_protocol::RequestId::Integer(100),
        };
        let sandbox_policy = SandboxPolicy::ReadOnly {
            access: ReadOnlyAccess::FullAccess,
            network_access: false,
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: dcode_app_server_protocol::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: dcode_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: dcode_app_server_protocol::RequestId::Integer(3),
        };
