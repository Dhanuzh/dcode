...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", dcode_home.path().display()),
            stderr: String::new(),
        }
    );
...
-> Result<()> {
    ...
    create_config_toml(dcode_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "dcode-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(dcode_home.path()).await?;
