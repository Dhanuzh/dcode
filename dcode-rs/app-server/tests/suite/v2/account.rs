...
use dcode_login::login_with_api_key;
...
fn create_config_toml(dcode_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(dcode_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(dcode_home.path().join("auth.json").exists());

    let mut mcp = McpProcess::new_with_env(dcode_home.path(), &[("OPENAI_API_KEY", None)]).await?;
    ...
    assert!(
        !dcode_home.path().join("auth.json").exists(),
...
async fn set_auth_token_updates_account_and_notifies() -> Result<()> {
    ...
    write_models_cache(dcode_home.path())?;
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(dcode_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<dcode_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(dcode_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![dcode_app_server_protocol::UserInput::Text {
