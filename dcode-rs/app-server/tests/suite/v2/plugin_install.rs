    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: AbsolutePathBuf::try_from(
                dcode_home.path().join("missing-marketplace.json"),
            )?,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        McpProcess::new_with_args(dcode_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_force_remote_sync_enables_remote_plugin_before_local_install() -> Result<()>
{
    ...
    let dcode_home = TempDir::new()?;
    write_plugin_remote_sync_config(dcode_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        dcode_home.path(),
    ...
    assert!(
        dcode_home
            .path()
            .join("plugins/cache/debug/sample-plugin/local/.dcode-plugin/plugin.json")
            .is_file()
    );
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let dcode_home = TempDir::new()?;
    write_analytics_config(dcode_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        dcode_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    let payload = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/dcode/analytics-events/events"
            }) {
...
async fn plugin_install_returns_apps_needing_auth() -> Result<()> {
    ...
    let dcode_home = TempDir::new()?;
    write_connectors_config(dcode_home.path(), &server_url)?;
    write_chatgpt_auth(
        dcode_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_connectors_config(dcode_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_analytics_config(dcode_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        dcode_home.join("config.toml"),
