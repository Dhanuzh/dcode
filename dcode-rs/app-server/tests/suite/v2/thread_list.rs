...
use dcode_core::ARCHIVED_SESSIONS_SUBDIR;
...
async fn init_mcp(dcode_home: &Path) -> Result<McpProcess> {
...
async fn list_threads_with_sort(
    ...
    let request_id = mcp
        .send_thread_list_request(dcode_app_server_protocol::ThreadListParams {
            cursor,
            limit,
            sort_key,
...
async fn thread_list_basic_empty() -> Result<()> {
    ...
    create_minimal_config(dcode_home.path())?;

    let mut mcp = init_mcp(dcode_home.path()).await?;
...
async fn thread_list_reports_system_error_idle_flag_after_failed_turn() -> Result<()> {
    ...
    let dcode_home = TempDir::new()?;
    create_runtime_config(dcode_home.path(), &server.uri())?;
    let mut mcp = init_mcp(dcode_home.path()).await?;
...
fn create_minimal_config(dcode_home: &std::path::Path) -> std::io::Result<()> {
...
fn create_runtime_config(dcode_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = dcode_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
async fn thread_list_respects_cwd_filter() -> Result<()> {
    ...
    let target_cwd = dcode_home.path().join("target-cwd");
    fs::create_dir_all(&target_cwd)?;
    set_rollout_cwd(
        rollout_path(dcode_home.path(), "2025-01-02T10-00-00", &filtered_id).as_path(),
        &target_cwd,
    )?;
...
"#,
    ...
    let state_db =
        dcode_state::StateRuntime::init(dcode_home.path().to_path_buf(), "mock_provider".into())
            .await?;
...
async fn thread_list_sort_updated_at_orders_by_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(dcode_home.path(), "2025-01-01T10-00-00", &id_old).as_path(),
        "2025-01-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(dcode_home.path(), "2025-01-01T11-00-00", &id_mid).as_path(),
        "2025-01-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(dcode_home.path(), "2025-01-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_paginates_with_cursor() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(dcode_home.path(), "2025-02-01T10-00-00", &id_a).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(dcode_home.path(), "2025-02-01T11-00-00", &id_b).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(dcode_home.path(), "2025-02-01T12-00-00", &id_c).as_path(),
...
async fn thread_list_updated_at_uses_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(dcode_home.path(), "2025-02-01T10-00-00", &thread_id).as_path(),
...
async fn thread_list_archived_filter() -> Result<()> {
    ...
    let archived_dir = dcode_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    fs::create_dir_all(&archived_dir)?;
    let archived_source = rollout_path(dcode_home.path(), "2025-03-01T09-00-00", &archived_id);
