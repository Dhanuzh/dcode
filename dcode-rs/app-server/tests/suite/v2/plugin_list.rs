...
use dcode_core::config::set_project_trust_level;
...
fn write_plugins_enabled_config(dcode_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
        dcode_home.join("config.toml"),
        r#"[features]
plugins = true
"#,
    )
...
async fn plugin_list_skips_invalid_marketplace_file() -> Result<()> {
    ...
    write_plugins_enabled_config(dcode_home.path())?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        "{not json",
    )?;

    let home = dcode_home.path().to_string_lossy().into_owned();
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let dcode_home = TempDir::new()?;
    std::fs::create_dir_all(dcode_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(dcode_home.path())?;
    std::fs::write(
        dcode_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "dcode-curated",
  "plugins": [
    {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&dcode_home, "dcode-curated", "enabled-plugin")?;
    write_installed_plugin(&dcode_home, "dcode-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "dcode-curated",
...
[plugins."enabled-plugin@dcode-curated"]
enabled = true

[plugins."disabled-plugin@dcode-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "dcode-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@dcode-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@dcode-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@dcode-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let dcode_home = TempDir::new()?;
    std::fs::create_dir_all(dcode_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&dcode_home, "dcode-curated", "shared-plugin")?;
    std::fs::write(
        dcode_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "dcode-curated",
...
[plugins."shared-plugin@dcode-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".dcode"))?;
    std::fs::write(
        workspace_enabled.path().join(".dcode/config.toml"),
        r#"[plugins."shared-plugin@dcode-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@dcode-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(plugin_root.join(".dcode-plugin"))?;
    write_plugins_enabled_config(dcode_home.path())?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@dcode-curated");
...
async fn plugin_list_force_remote_sync_returns_remote_sync_error_on_fail_open() -> Result<()> {
    let dcode_home = TempDir::new()?;
    write_plugin_sync_config(dcode_home.path(), "https://chatgpt.com/backend-api/")?;
    write_openai_curated_marketplace(dcode_home.path(), &["linear"])?;
    write_installed_plugin(&dcode_home, "openai-curated", "linear")?;
...
async fn plugin_list_force_remote_sync_reconciles_curated_plugin_state() -> Result<()> {
    ...
    write_plugin_sync_config(dcode_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        dcode_home.path(),
    ...
    write_openai_curated_marketplace(dcode_home.path(), &["linear", "gmail", "calendar"])?;
    write_installed_plugin(&dcode_home, "openai-curated", "linear")?;
    write_installed_plugin(&dcode_home, "openai-curated", "gmail")?;
    write_installed_plugin(&dcode_home, "openai-curated", "calendar")?;
  ...
  {"id":"2","name":"gmail","marketplace_name":"openai-curated","version":"1.0.0","enabled":false}
    ...
    Mock::given(method("GET"))
        ...
        .and(query_param("platform", "dcode"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_plugin_sync_config(dcode_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_curated_marketplace(dcode_home.path(), &["linear", "gmail"])?;
...
fn write_installed_plugin(
    ...
    let plugin_root = dcode_home
        .path()
        .join("plugins/cache")
        .join(marketplace_name)
        .join(plugin_name)
        .join("local/.dcode-plugin");
...
fn write_plugin_sync_config(dcode_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        dcode_home.join("config.toml"),
        format!(
            r#"
...
fn write_openai_curated_marketplace(
    ...
    let curated_root = dcode_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.dcode-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(dcode_home.join(".tmp"))?;
    std::fs::write(
        dcode_home.join(".tmp/plugins.sha"),
