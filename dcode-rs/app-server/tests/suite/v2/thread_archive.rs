...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(dcode_home.path(), &thread.id)
            .await?
            .is_none(),
    ...
    let discovered_path = find_thread_path_by_id_str(dcode_home.path(), &thread.id)
        .await?
    ...
    // Verify file moved.
    let archived_directory = dcode_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
