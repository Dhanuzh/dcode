...
async fn get_account_rate_limits_requires_auth() -> Result<()> {
    ...
    assert_eq!(
        error.error.message,
        "dcode account authentication required to read rate limits"
    );
...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(dcode_home.path(), &server_url)?;
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "dcode_other",
                "metered_feature": "dcode_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
...
fn write_chatgpt_base_url(dcode_home: &Path, base_url: &str) -> std::io::Result<()> {
