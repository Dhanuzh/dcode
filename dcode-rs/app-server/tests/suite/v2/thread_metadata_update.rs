...
use dcode_app_server_protocol::GitInfo;
...
use dcode_protocol::protocol::GitInfo as RolloutGitInfo;
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_stored_thread() -> Result<()> {
    ...
    let dcode_home = TempDir::new()?;
    create_config_toml(dcode_home.path(), &server.uri())?;
    let _state_db = init_state_db(dcode_home.path()).await?;
...
async fn thread_metadata_update_repairs_loaded_thread_without_resetting_summary() -> Result<()> {
    ...
    let dcode_home = TempDir::new()?;
    create_config_toml(dcode_home.path(), &server.uri())?;
    let state_db = init_state_db(dcode_home.path()).await?;
    ...
    let rollout_path = rollout_path(dcode_home.path(), "2025-01-06T08-30-00", &thread_id);
    reconcile_rollout(
        Some(&state_db),
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_archived_thread() -> Result<()> {
    ...
    let archived_source = rollout_path(dcode_home.path(), "2025-01-06T08-30-00", &thread_id);
...
async fn thread_metadata_update_can_clear_stored_git_fields() -> Result<()> {
    ...
    let thread_id = create_fake_rollout(
        ...
        Some(RolloutGitInfo {
            ...
            branch: Some("feature/sidebar-pr".to_string()),
            repository_url: Some("git@example.com:openai/dcode.git".to_string()),
        }),
    )?;
    let _state_db = init_state_db(dcode_home.path()).await?;
...
async fn init_state_db(dcode_home: &Path) -> Result<Arc<StateRuntime>> {
    let state_db = StateRuntime::init(dcode_home.to_path_buf(), "mock_provider".into()).await?;
