...
async fn thread_name_updated_broadcasts_for_loaded_threads() -> Result<()> {
    ...
    let conversation_id = create_rollout(dcode_home.path(), "2025-01-05T12-00-00")?;
...
async fn thread_name_updated_broadcasts_for_not_loaded_threads() -> Result<()> {
    ...
    let dcode_home = TempDir::new()?;
    create_config_toml(dcode_home.path(), &server.uri(), "never")?;
    let conversation_id = create_rollout(dcode_home.path(), "2025-01-05T12-05-00")?;
...
fn create_rollout(dcode_home: &std::path::Path, filename_ts: &str) -> Result<String> {
