...
use dcode_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use dcode_app_server::INVALID_PARAMS_ERROR_CODE;
use dcode_app_server_protocol::ByteRange;
use dcode_app_server_protocol::ClientInfo;
use dcode_app_server_protocol::CollabAgentStatus;
...
use dcode_app_server_protocol::TextElement;
...
use dcode_core::config::ConfigToml;
use dcode_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
...
const TEST_ORIGINATOR: &str = "dcode_vscode";
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Dcode VS Code Extension".to_string()),
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Dcode turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
    ...
    assert_eq!(
        started.turn.status,
        dcode_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-dcode-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    // Two Dcode turns hit the mock model (session start + turn/start).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    ...
    let image_path = dcode_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let dcode_home = tmp.path().to_path_buf();
    ...
    let mut mcp = McpProcess::new(dcode_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(dcode_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(dcode_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(&dcode_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(dcode_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![dcode_app_server_protocol::FileUpdateChange {
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    const REQUESTED_MODEL: &str = "gpt-5.1";
    const REQUESTED_REASONING_EFFORT: ReasoningEffort = ReasoningEffort::Low;
    const ROLE_MODEL: &str = "gpt-5.1-dcode-max";
    ...
    std::fs::write(
        dcode_home.path().join("custom-role.toml"),
        format!("model = \"{ROLE_MODEL}\"\nmodel_reasoning_effort = \"{ROLE_REASONING_EFFORT}\"\n",),
    )?;
    let config_path = dcode_home.path().join("config.toml");
