...
[plugins."demo-plugin@dcode-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&dcode_home, "dcode-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "dcode-curated");
    assert_eq!(response.plugin.marketplace_path, marketplace_path);
    assert_eq!(response.plugin.summary.id, "demo-plugin@dcode-curated");
...
async fn plugin_read_accepts_legacy_string_default_prompt() -> Result<()> {
  ...
  "interface": {
    ...
    write_plugins_enabled_config(&dcode_home)?;
...
async fn plugin_read_returns_invalid_request_when_plugin_manifest_is_missing() -> Result<()> {
  ...
  "plugins": [
    ...
    assert!(
        err.error
            .message
            .contains("missing or invalid .dcode-plugin/plugin.json")
    );
    Ok(())
}
...
fn write_plugins_enabled_config(dcode_home: &TempDir) -> Result<()> {
    std::fs::write(
        dcode_home.path().join("config.toml"),
        r#"[features]
plugins = true
