...
use dcode_utils_cargo_bin::cargo_bin;
...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    create_config_toml(dcode_home.path(), &server.uri(), "never")?;
    let mut mcp = McpProcess::new(dcode_home.path()).await?;

    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "dcode_vscode".to_string(),
    ...
    assert!(user_agent.starts_with("dcode_vscode/"));
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        ...
        &[(
            ...
            Some("dcode_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("dcode_originator_via_env_var/"));
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
}
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let dcode_home = TempDir::new()?;
    let notify_file = dcode_home.path().join("notify.json");
    let notify_capture = cargo_bin("dcode-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(dcode_home, server_uri, approval_policy, "")
}
