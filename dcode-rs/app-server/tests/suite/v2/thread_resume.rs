...
use dcode_app_server_protocol::SessionSource;
...
use dcode_app_server_protocol::UserInput;
use dcode_core::auth::AuthCredentialsStoreMode;
use dcode_core::auth::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use dcode_protocol::protocol::SessionSource as RolloutSessionSource;
use dcode_protocol::protocol::TurnStartedEvent;
use dcode_protocol::user_input::ByteRange;
...
const CODEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Dcode, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_rejects_unmaterialized_thread() -> Result<()> {
    ...
    create_config_toml(dcode_home.path(), &server.uri())?;
    ...
    let start_id = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("gpt-5.1-dcode-max".to_string()),
            ..Default::default()
        })
        .await?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let dcode_home = TempDir::new()?;
    let config_toml = dcode_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
...
"#,
    ...
    let repo_path = dcode_home.path().join("repo");
    ...
    let rollout_path = rollout_path(dcode_home.path(), "2025-01-05T12-00-00", &thread_id);
    ...
    let state_db =
        StateRuntime::init(dcode_home.path().to_path_buf(), "mock_provider".into()).await?;
...
async fn thread_resume_and_read_interrupt_incomplete_rollout_turn_when_thread_is_idle() -> Result<()>
{
    ...
    let rollout_file_path = rollout_path(dcode_home.path(), filename_ts, &conversation_id);
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let dcode_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(dcode_home.path(), &server.uri())?;
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = McpProcess::new(dcode_home.path()).await?;
    ...
    let mut secondary = McpProcess::new(dcode_home.path()).await?;
...
async fn thread_resume_rejoins_running_thread_even_with_override_mismatch() -> Result<()> {
    ...
    let ThreadResumeResponse { thread, model, .. } =
        to_response::<ThreadResumeResponse>(resume_resp)?;
    assert_eq!(model, "gpt-5.1-dcode-max");
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    let tmp = TempDir::new()?;
    let dcode_home = tmp.path().join("dcode_home");
    std::fs::create_dir(&dcode_home)?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&dcode_home, &server.uri())?;

    let mut primary = McpProcess::new(&dcode_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        thread_id,
        rollout_file_path,
    } = start_materialized_thread_and_restart(dcode_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let dcode_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(dcode_home.path(), &server.uri())?;
    create_config_toml_with_required_broken_mcp(dcode_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(dcode_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = McpProcess::new(dcode_home).await?;
    ...
    let mut second_mcp = McpProcess::new(dcode_home).await?;
...
fn create_config_toml(dcode_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = dcode_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
...
command = "dcode-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
fn setup_rollout_fixture(dcode_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(dcode_home, server_uri)?;
    ...
    let rollout_file_path = rollout_path(dcode_home, filename_ts, &conversation_id);
    set_rollout_mtime(rollout_file_path.as_path(), expected_updated_at_rfc3339)?;
    let before_modified = std::fs::metadata(&rollout_file_path)?.modified()?;
    let expected_updated_at = chrono::DateTime::parse_from_rfc3339(expected_updated_at_rfc3339)?
        .with_timezone(&Utc)
