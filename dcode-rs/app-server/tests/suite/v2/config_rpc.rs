...
use dcode_app_server_protocol::AppConfig;
use dcode_app_server_protocol::AppToolApproval;
use dcode_app_server_protocol::AppsConfig;
use dcode_app_server_protocol::AskForApproval;
use dcode_app_server_protocol::ConfigBatchWriteParams;
use dcode_app_server_protocol::ConfigEdit;
...
use dcode_app_server_protocol::MergeStrategy;
use dcode_app_server_protocol::RequestId;
use dcode_app_server_protocol::SandboxMode;
use dcode_app_server_protocol::ToolsV2;
use dcode_app_server_protocol::WriteStatus;
use dcode_core::config::set_project_trust_level;
use dcode_protocol::config_types::TrustLevel;
use dcode_protocol::config_types::WebSearchContextSize;
use dcode_protocol::config_types::WebSearchLocation;
...
fn write_config(dcode_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let dcode_home_path = dcode_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(dcode_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let dcode_home = TempDir::new()?;
    write_config(&dcode_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(dcode_home.path(), workspace.path(), TrustLevel::Trusted)?;
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_dcode_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let dcode_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", dcode_home)?;
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(dcode_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let dcode_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(dcode_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[dcode_app_server_protocol::ConfigLayer],
