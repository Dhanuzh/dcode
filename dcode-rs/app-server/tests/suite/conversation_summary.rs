...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_conversation_summary_by_relative_rollout_path_resolves_from_dcode_home() -> Result<()>
{
    let dcode_home = TempDir::new()?;
    let conversation_id = create_fake_rollout(
        dcode_home.path(),
    ...
    let thread_id = ThreadId::from_string(&conversation_id)?;
    let rollout_path = rollout_path(dcode_home.path(), FILENAME_TS, &conversation_id);
    let relative_path = rollout_path.strip_prefix(dcode_home.path())?.to_path_buf();
