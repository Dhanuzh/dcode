...
fn create_config_toml(dcode_home: &Path) -> std::io::Result<()> {
    let config_toml = dcode_home.join("config.toml");
    std::fs::write(
        config_toml,
        r#"
...
fn create_config_toml_forced_login(dcode_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = dcode_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn get_auth_status_no_auth() -> Result<()> {
    ...
    create_config_toml(dcode_home.path())?;
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    let dcode_home = TempDir::new()?;
    create_config_toml_custom_provider(dcode_home.path(), false)?;
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let dcode_home = TempDir::new()?;
    create_config_toml_forced_login(dcode_home.path(), "chatgpt")?;
