...
pub fn rollout_path(dcode_home: &Path, filename_ts: &str, thread_id: &str) -> PathBuf {
...
pub fn create_fake_rollout_with_source(
    ...
    let file_path = rollout_path(dcode_home, filename_ts, &uuid_str);
    let dir = file_path
        .parent()
...
pub fn create_fake_rollout_with_text_elements(
    ...
    let month = &filename_ts[5..7];
    let day = &filename_ts[8..10];
    let dir = dcode_home.join("sessions").join(year).join(month).join(day);
    fs::create_dir_all(&dir)?;
