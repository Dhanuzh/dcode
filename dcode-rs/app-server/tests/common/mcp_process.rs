...
use dcode_app_server_protocol::ClientNotification;
...
use dcode_app_server_protocol::ConfigBatchWriteParams;
use dcode_app_server_protocol::ConfigReadParams;
use dcode_app_server_protocol::ConfigValueWriteParams;
use dcode_app_server_protocol::ExperimentalFeatureListParams;
use dcode_app_server_protocol::FeedbackUploadParams;
use dcode_app_server_protocol::FsCopyParams;
use dcode_app_server_protocol::FsCreateDirectoryParams;
use dcode_app_server_protocol::FsGetMetadataParams;
use dcode_app_server_protocol::FsReadDirectoryParams;
use dcode_app_server_protocol::FsReadFileParams;
use dcode_app_server_protocol::FsRemoveParams;
use dcode_app_server_protocol::FsWriteFileParams;
...
use dcode_app_server_protocol::JSONRPCError;
...
pub const DEFAULT_CLIENT_NAME: &str = "dcode-app-server-tests";

impl McpProcess {
    pub async fn new(dcode_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(dcode_home, &[], &[]).await
    }

    pub async fn new_with_args(dcode_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(dcode_home, &[], args).await
    }
    ...
    pub async fn new_with_env(
        ...
        Self::new_with_env_and_args(dcode_home, env_overrides, &[]).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = dcode_utils_cargo_bin::cargo_bin("dcode-app-server")
            .context("should find binary for dcode-app-server")?;
        let mut cmd = Command::new(program);

        cmd.stdin(Stdio::piped());
        cmd.stdout(Stdio::piped());
        cmd.stderr(Stdio::piped());
        cmd.current_dir(dcode_home);
        cmd.env("DCODE_HOME", dcode_home);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("dcode-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `dcode-app-server` child process.
        //
