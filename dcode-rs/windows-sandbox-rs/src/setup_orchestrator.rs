...
pub const OFFLINE_USERNAME: &str = "DcodeSandboxOffline";
pub const ONLINE_USERNAME: &str = "DcodeSandboxOnline";
...
pub fn sandbox_dir(dcode_home: &Path) -> PathBuf {
    dcode_home.join(".sandbox")
}

pub fn sandbox_bin_dir(dcode_home: &Path) -> PathBuf {
    dcode_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(dcode_home: &Path) -> PathBuf {
    dcode_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(dcode_home: &Path) -> PathBuf {
    sandbox_dir(dcode_home).join("setup_marker.json")
}

pub fn sandbox_users_path(dcode_home: &Path) -> PathBuf {
    sandbox_secrets_dir(dcode_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, dcode_home);
...
fn run_setup_refresh_inner(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| dcode_home.to_path_buf());
    log_note(
        &format!(
...
fn gather_helper_read_roots(dcode_home: &Path) -> Vec<PathBuf> {
    ...
    let helper_dir = helper_bin_dir(dcode_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(dcode_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    if policy.has_full_disk_read_access() {
        gather_legacy_full_read_roots(command_cwd, policy, dcode_home)
    } else {
        gather_restricted_read_roots(command_cwd, policy, dcode_home)
    }
}
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("dcode-windows-sandbox-setup.exe");
            if candidate.exists() {
                return candidate;
            }
        }
    }
    PathBuf::from("dcode-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(dcode_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(dcode_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(dcode_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(dcode_home);
    ...
    run_setup_exe(&payload, needs_elevation, dcode_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, dcode_home);
    let mut read_roots = if let Some(roots) = read_roots_override {
        canonical_existing(&roots)
    } else {
        gather_read_roots(command_cwd, policy, dcode_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, dcode_home: &Path) -> Vec<PathBuf> {
    ...
    let dcode_home_key = canonical_path_key(dcode_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(dcode_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(dcode_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(dcode_home));
...
mod tests {
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let dcode_home = tmp.path().join("dcode-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &dcode_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&dcode_home)).expect("canonical helper dir");
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &dcode_home);
