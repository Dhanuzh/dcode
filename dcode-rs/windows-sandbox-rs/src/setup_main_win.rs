...
use base64::Engine;
use dcode_windows_sandbox::canonicalize_path;
use dcode_windows_sandbox::convert_string_sid_to_sid;
use dcode_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use dcode_windows_sandbox::ensure_allow_write_aces;
use dcode_windows_sandbox::extract_setup_failure;
use dcode_windows_sandbox::hide_newly_created_users;
use dcode_windows_sandbox::is_command_cwd_root;
use dcode_windows_sandbox::load_or_create_cap_sids;
use dcode_windows_sandbox::log_note;
use dcode_windows_sandbox::path_mask_allows;
use dcode_windows_sandbox::protect_workspace_agents_dir;
use dcode_windows_sandbox::protect_workspace_dcode_dir;
use dcode_windows_sandbox::sandbox_bin_dir;
use dcode_windows_sandbox::sandbox_dir;
use dcode_windows_sandbox::sandbox_secrets_dir;
use dcode_windows_sandbox::string_from_sid_bytes;
use dcode_windows_sandbox::to_wide;
use dcode_windows_sandbox::workspace_cap_sid_for_cwd;
use dcode_windows_sandbox::write_setup_error_report;
use dcode_windows_sandbox::SetupErrorCode;
use dcode_windows_sandbox::SetupErrorReport;
use dcode_windows_sandbox::SetupFailure;
use dcode_windows_sandbox::LOG_FILE_NAME;
use dcode_windows_sandbox::SETUP_VERSION;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(dcode_home) = std::env::var("DCODE_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&dcode_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.dcode_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.dcode_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.dcode_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    let workspace_sid_str = workspace_cap_sid_for_cwd(&payload.dcode_home, &payload.command_cwd)?;
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.dcode_home),
        &payload.real_user,
        &sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.dcode_home).display()
            ),
        ))
    })?;
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.dcode_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.dcode_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.dcode_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.dcode_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.dcode_home).join("sandbox_users.json");
        if legacy_users.exists() {
            let _ = std::fs::remove_file(&legacy_users);
        }
    }

    // Protect the current workspace's `.dcode` and `.agents` directories from tampering
    ...
    match unsafe { protect_workspace_dcode_dir(&payload.command_cwd, workspace_psid) } {
        Ok(true) => {
            let cwd_dcode = payload.command_cwd.join(".dcode");
            log_line(
                log,
                &format!(
                    "applied deny ACE to protect workspace .dcode {}",
                    cwd_dcode.display()
                ),
            )?;
        }
        ...
        Err(err) => {
            let cwd_dcode = payload.command_cwd.join(".dcode");
            refresh_errors.push(format!("deny ACE failed on {}: {err}", cwd_dcode.display()));
            log_line(
                log,
                &format!("deny ACE failed on {}: {err}", cwd_dcode.display()),
            )?;
        }
    }
