...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "dcode-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(dcode_home: &Path) -> PathBuf {
    sandbox_bin_dir(dcode_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, dcode_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(
    ...
    let destination = helper_bin_dir(dcode_home).join(file_name);
    match copy_from_source_if_needed(&source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(dcode_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), dcode_home.display());
    ...
    let destination = helper_bin_dir(dcode_home).join(kind.file_name());
    log_note(
        &format!(
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let dcode_home = Path::new(r"C:\Users\example\.dcode");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.dcode\.sandbox-bin"),
            helper_bin_dir(dcode_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("dcode-command-runner.exe");
        let runner_destination = helper_bin_dir(&dcode_home).join("dcode-command-runner.exe");
