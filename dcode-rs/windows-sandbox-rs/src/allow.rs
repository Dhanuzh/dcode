        ...
        let add_writable_root =
            |root: PathBuf,
             ...
             add_deny: &mut dyn FnMut(PathBuf)| {
                ...
                for protected_subdir in [".git", ".dcode", ".agents"] {
...
mod tests {
    ...
    #[test]
    fn denies_dcode_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let dcode_dir = command_cwd.join(".dcode");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&dcode_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&dcode_dir).unwrap(),
