mod windows_impl {
    ...
    /// Creates the sandbox user's Dcode home directory if it does not already exist.
    fn ensure_dcode_home_exists(p: &Path) -> Result<()> {
    ...
    fn find_runner_exe(dcode_home: &Path, log_dir: Option<&Path>) -> PathBuf {
        resolve_helper_for_launch(HelperExecutable::CommandRunner, dcode_home, log_dir)
    }

    /// Generates a unique named-pipe path used to communicate with the runner process.
    fn pipe_name(suffix: &str) -> String {
        let mut rng = SmallRng::from_entropy();
        format!(r"\\.\pipe\dcode-runner-{:x}-{}", rng.gen::<u128>(), suffix)
    }
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = dcode_home.join(".sandbox");
        ensure_dcode_home_exists(&sandbox_base)?;

        let logs_base_dir: Option<&Path> = Some(sandbox_base.as_path());
        log_start(&command, logs_base_dir);
        let sandbox_creds =
            require_logon_sandbox_creds(&policy, sandbox_policy_cwd, cwd, &env_map, dcode_home)?;
        ...
        let caps = load_or_create_cap_sids(dcode_home)?;
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => (
                ...
                vec![
                    ...
                    crate::cap::workspace_cap_sid_for_cwd(dcode_home, cwd)?,
                ],
            ),
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        let runner_exe = find_runner_exe(dcode_home, logs_base_dir);
        let runner_cmdline = runner_exe
            .to_str()
            .map(|s| s.to_string())
            .unwrap_or_else(|| "dcode-command-runner.exe".to_string());
            ...
            let spawn_request = FramedMessage {
                ...
                message: Message::SpawnRequest {
                    payload: Box::new(SpawnRequest {
                        ...
                        dcode_home: sandbox_base.clone(),
                        real_dcode_home: dcode_home.to_path_buf(),
                        cap_sids,
                        timeout_ms,
                        tty: false,
                        stdin_open: false,
