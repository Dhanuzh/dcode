...
use anyhow::Context;
use anyhow::Result;
use dcode_windows_sandbox::allow_null_device;
use dcode_windows_sandbox::convert_string_sid_to_sid;
use dcode_windows_sandbox::create_readonly_token_with_caps_from;
use dcode_windows_sandbox::create_workspace_write_token_with_caps_from;
use dcode_windows_sandbox::get_current_token_for_restriction;
use dcode_windows_sandbox::hide_current_user_profile_dir;
use dcode_windows_sandbox::ipc_framed::decode_bytes;
use dcode_windows_sandbox::ipc_framed::encode_bytes;
use dcode_windows_sandbox::ipc_framed::read_frame;
use dcode_windows_sandbox::ipc_framed::write_frame;
use dcode_windows_sandbox::ipc_framed::ErrorPayload;
use dcode_windows_sandbox::ipc_framed::ExitPayload;
use dcode_windows_sandbox::ipc_framed::FramedMessage;
use dcode_windows_sandbox::ipc_framed::Message;
use dcode_windows_sandbox::ipc_framed::OutputPayload;
use dcode_windows_sandbox::ipc_framed::OutputStream;
use dcode_windows_sandbox::log_note;
use dcode_windows_sandbox::parse_policy;
use dcode_windows_sandbox::read_handle_loop;
use dcode_windows_sandbox::spawn_process_with_pipes;
use dcode_windows_sandbox::to_wide;
use dcode_windows_sandbox::PipeSpawnHandles;
use dcode_windows_sandbox::SandboxPolicy;
use dcode_windows_sandbox::StderrMode;
use dcode_windows_sandbox::StdinMode;
use std::ffi::c_void;
use std::fs::File;
...
fn read_spawn_request(
    reader: &mut File,
) -> Result<dcode_windows_sandbox::ipc_framed::SpawnRequest> {
...
fn spawn_ipc_process(
    req: &dcode_windows_sandbox::ipc_framed::SpawnRequest,
) -> Result<IpcSpawnedProcess> {
    let log_dir = req.dcode_home.clone();
    hide_current_user_profile_dir(req.dcode_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_dcode_home={}",
            req.cwd.display(),
            req.command,
            req.real_dcode_home.display()
        ),
        Some(&req.dcode_home),
    );
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, conpty) = dcode_windows_sandbox::spawn_conpty_process_as_user(
            h_token,
            &req.command,
            &effective_cwd,
            &req.env,
...
pub fn main() -> Result<()> {
    ...
    let msg = FramedMessage {
        ...
        message: Message::SpawnReady {
            payload: dcode_windows_sandbox::ipc_framed::SpawnReady {
