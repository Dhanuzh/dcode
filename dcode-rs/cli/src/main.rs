...
use dcode_chatgpt::apply_command::ApplyCommand;
use dcode_chatgpt::apply_command::run_apply_command;
use dcode_cli::LandlockCommand;
use dcode_cli::SeatbeltCommand;
use dcode_cli::WindowsCommand;
use dcode_cli::login::read_api_key_from_stdin;
use dcode_cli::login::run_login_status;
use dcode_cli::login::run_login_with_api_key;
use dcode_cli::login::run_login_with_chatgpt;
use dcode_cli::login::run_login_with_device_code;
use dcode_cli::login::run_logout;
use dcode_cloud_tasks::Cli as CloudTasksCli;
use dcode_exec::Cli as ExecCli;
use dcode_exec::Command as ExecCommand;
use dcode_exec::ReviewArgs;
use dcode_execpolicy::ExecPolicyCheckCommand;
use dcode_responses_api_proxy::Args as ResponsesApiProxyArgs;
use dcode_state::StateRuntime;
use dcode_state::state_db_path;
use dcode_tui::AppExitInfo;
use dcode_tui::Cli as TuiCli;
use dcode_tui::ExitReason;
use dcode_tui::update_action::UpdateAction;
...
use dcode_features::is_known_feature_key;
...
enum Subcommand {
    ...
    /// Launch the Dcode desktop app (downloads the macOS installer if missing).
    #[cfg(target_os = "macos")]
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | dcode login --with-api-key`)"
    )]
    with_api_key: bool,
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = dcode_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: dcode_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://developers.openai.com/dcode/config-advanced/#metrics for more details.
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    if let Some(resume_cmd) =
        dcode_core::util::resume_command(thread_name.as_deref(), conversation_id)
    {
...
async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let dcode_bin = std::env::current_exe()?;
            dcode_app_server_test_client::send_message_v2(&dcode_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            dcode_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        ...
        Some(Subcommand::McpServer) => {
            ...
            dcode_mcp_server::run_main(arg0_paths.clone(), root_config_overrides).await?;
        }
        ...
        Some(Subcommand::AppServer(app_server_cli)) => match app_server_cli.subcommand {
            None => {
                ...
                dcode_app_server::run_main_with_transport(
                    arg0_paths.clone(),
                    root_config_overrides,
                    dcode_core::config_loader::LoaderOverrides::default(),
                    app_server_cli.analytics_default_enabled,
                    transport,
                    dcode_protocol::protocol::SessionSource::VSCode,
                )
                .await?;
            }
            Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                ...
                let options = dcode_app_server_protocol::GenerateTsOptions {
                    experimental_api: gen_cli.experimental,
                    ..Default::default()
                };
                dcode_app_server_protocol::generate_ts_with_options(
                    &gen_cli.out_dir,
            ...
            Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                ...
                dcode_app_server_protocol::generate_json_with_experimental(
                    &gen_cli.out_dir,
                    gen_cli.experimental,
                )?;
            }
            Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                dcode_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
            }
        },
        #[cfg(target_os = "macos")]
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | dcode login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            dcode_cloud_tasks::run_main(cloud_cli, arg0_paths.dcode_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(sandbox_args)) => match sandbox_args.cmd {
            SandboxCommand::Macos(mut seatbelt_cli) => {
                ...
                dcode_cli::debug_sandbox::run_command_under_seatbelt(
                    seatbelt_cli,
                    arg0_paths.dcode_linux_sandbox_exe.clone(),
                )
                .await?;
            }
            SandboxCommand::Linux(mut landlock_cli) => {
                ...
                dcode_cli::debug_sandbox::run_command_under_landlock(
                    landlock_cli,
                    arg0_paths.dcode_linux_sandbox_exe.clone(),
                )
                .await?;
            ...
            SandboxCommand::Windows(mut windows_cli) => {
                ...
                dcode_cli::debug_sandbox::run_command_under_windows(
                    windows_cli,
                    arg0_paths.dcode_linux_sandbox_exe.clone(),
                )
                .await?;
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || dcode_responses_api_proxy::run_main(args))
                .await??;
        }
        Some(Subcommand::StdioToUds(cmd)) => {
            reject_remote_mode_for_subcommand(root_remote.as_deref(), "stdio-to-uds")?;
            let socket_path = cmd.socket_path;
            tokio::task::spawn_blocking(move || dcode_stdio_to_uds::run(socket_path.as_path()))
                .await??;
        }
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: &str) -> anyhow::Result<()> {
    ...
    maybe_print_under_development_feature_warning(&dcode_home, interactive, feature);
    Ok(())
}
...
fn maybe_print_under_development_feature_warning(
    ...
    let config_path = dcode_home.join(dcode_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_clear_memories_command(
    ...
    let memory_root = config.dcode_home.join("memories");
...
async fn run_interactive_tui(
    ...
    let terminal_info = dcode_terminal_detection::terminal_info();
    ...
    let use_app_server_tui = dcode_tui::should_use_app_server_tui(&interactive).await?;
    let normalized_remote = remote
        .as_deref()
        .map(dcode_tui_app_server::normalize_remote_addr)
        .transpose()
...
fn into_app_server_tui_cli(cli: TuiCli) -> dcode_tui_app_server::Cli {
    dcode_tui_app_server::Cli {
        prompt: cli.prompt,
        images: cli.images,
...
fn into_legacy_update_action(
    action: dcode_tui_app_server::update_action::UpdateAction,
) -> UpdateAction {
    match action {
        dcode_tui_app_server::update_action::UpdateAction::NpmGlobalLatest => {
            UpdateAction::NpmGlobalLatest
        }
        dcode_tui_app_server::update_action::UpdateAction::BunGlobalLatest => {
            UpdateAction::BunGlobalLatest
        }
        dcode_tui_app_server::update_action::UpdateAction::BrewUpgrade => UpdateAction::BrewUpgrade,
    }
}

fn into_legacy_exit_reason(reason: dcode_tui_app_server::ExitReason) -> ExitReason {
    match reason {
        dcode_tui_app_server::ExitReason::UserRequested => ExitReason::UserRequested,
        dcode_tui_app_server::ExitReason::Fatal(message) => ExitReason::Fatal(message),
    }
}

fn into_legacy_app_exit_info(exit_info: dcode_tui_app_server::AppExitInfo) -> AppExitInfo {
    AppExitInfo {
...
fn finalize_resume_interactive(
    ...
    // configuration surface area as `dcode` without additional flags.
...
/// Merge flags provided to `dcode resume`/`dcode fork` so they take precedence over any
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(dcode_exec::Command::Resume(args)) = exec.command else {
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run dcode resume my-thread".to_string(),
            ]
        );
    }
    ...
    fn resume_merges_option_flags_and_full_auto() {
        ...
        assert_matches!(
            ...
            Some(dcode_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(dcode_utils_cli::ApprovalModeCliArg::OnRequest)
        );
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        ...
        assert_eq!(
            app_server.listen,
            dcode_app_server::AppServerTransport::Stdio
        );
    }
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["dcode", "--remote", "ws://127.0.0.1:4500"])
            .expect("parse");
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["dcode", "resume", "--remote", "ws://127.0.0.1:4500"])
                .expect("parse");
    ...
    fn app_server_listen_websocket_url_parses() {
        ...
        assert_eq!(
            app_server.listen,
            dcode_app_server::AppServerTransport::WebSocket {
