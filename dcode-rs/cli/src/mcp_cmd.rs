...
use dcode_core::config::load_global_mcp_servers;
...
use dcode_core::mcp::auth::McpOAuthLoginSupport;
use dcode_core::mcp::auth::ResolvedMcpOAuthScopes;
use dcode_core::mcp::auth::compute_auth_statuses;
use dcode_core::mcp::auth::discover_supported_scopes;
use dcode_core::mcp::auth::oauth_login_support;
use dcode_core::mcp::auth::resolve_oauth_scopes;
use dcode_core::mcp::auth::should_retry_without_scopes;
use dcode_core::plugins::PluginsManager;
use dcode_protocol::protocol::McpAuthStatus;
use dcode_rmcp_client::delete_oauth_tokens;
use dcode_rmcp_client::perform_oauth_login;
...
/// - `add`    — add a server launcher entry to `~/.dcode/config.toml`
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: dcode_rmcp_client::OAuthCredentialsStoreMode,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let dcode_home = find_dcode_home().context("failed to resolve CODEX_HOME")?;
    let mut servers = load_global_mcp_servers(&dcode_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", dcode_home.display()))?;
    ...
    ConfigEditsBuilder::new(&dcode_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", dcode_home.display()))?;
