...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\dcode.zip").as_deref(),
            Some("/mnt/c/Temp/dcode.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/dcode.tgz").as_deref(),
            Some("/mnt/d/Work/dcode.tgz")
        );
        assert!(win_path_to_wsl("/home/user/dcode").is_none());
    }
