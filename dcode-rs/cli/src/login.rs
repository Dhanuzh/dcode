...
//! into a one-shot CLI command while still producing a durable `dcode-login.log` artifact that
...
use dcode_core::auth::logout;
use dcode_core::config::Config;
use dcode_login::ServerOptions;
use dcode_login::run_device_code_login;
...
/// command produce a durable `dcode-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match dcode_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("dcode-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("dcode_cli=info,dcode_core=info,dcode_login=info"));
...
pub fn read_api_key_from_stdin() -> String {
    ...
    if stdin.is_terminal() {
        eprintln!(
            "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | dcode login --with-api-key`."
        );
        std::process::exit(1);
    }
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match DcodeAuth::from_auth_storage(&config.dcode_home, config.cli_auth_credentials_store_mode) {
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout(&config.dcode_home, config.cli_auth_credentials_store_mode) {
        Ok(true) => {
