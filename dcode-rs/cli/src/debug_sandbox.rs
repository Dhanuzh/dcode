...
#[cfg(target_os = "macos")]
use dcode_core::spawn::CODEX_SANDBOX_ENV_VAR;
...
#[cfg(target_os = "macos")]
use dcode_sandboxing::seatbelt::create_seatbelt_command_args_for_policies_with_extensions;
...
pub async fn run_command_under_seatbelt(
    ...
    _dcode_linux_sandbox_exe: Option<PathBuf>,
) -> anyhow::Result<()> {
...
async fn run_command_under_sandbox(
    ...
    if let SandboxType::Windows = sandbox_type {
        #[cfg(target_os = "windows")]
        {
            ...
            let base_dir = config.dcode_home.clone();
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Landlock => {
            ...
            let dcode_linux_sandbox_exe = config
                .dcode_linux_sandbox_exe
                .expect("dcode-linux-sandbox executable not found");
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_dcode_home(
        cli_overrides,
        dcode_linux_sandbox_exe,
        full_auto,
        /*dcode_home*/ None,
...
async fn load_debug_sandbox_config_with_dcode_home(
    cli_overrides: Vec<(String, TomlValue)>,
    dcode_linux_sandbox_exe: Option<PathBuf>,
    full_auto: bool,
    ...
    let config = build_debug_sandbox_config(
        cli_overrides.clone(),
        ConfigOverrides {
            dcode_linux_sandbox_exe: dcode_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        dcode_home.clone(),
    )
    ...
    if config_uses_permission_profiles(&config) {
        if full_auto {
            anyhow::bail!(
                "`dcode sandbox --full-auto` is only supported for legacy `sandbox_mode` configs; choose a writable `[permissions]` profile instead"
            );
        }
        return Ok(config);
    }
...
async fn build_debug_sandbox_config(
    ...
    if let Some(dcode_home) = dcode_home {
        builder = builder
            .dcode_home(dcode_home.clone())
            .fallback_cwd(Some(dcode_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    fn write_permissions_profile_config(
        ...
        std::fs::write(dcode_home.path().join("config.toml"), config)?;
        Ok(())
    }
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_dcode_home(
            Vec::new(),
            None,
            false,
            Some(dcode_home_path),
        )
        .await?;
    ...
    async fn debug_sandbox_rejects_full_auto_for_permission_profiles() -> anyhow::Result<()> {
        ...
        let err = load_debug_sandbox_config_with_dcode_home(
            Vec::new(),
            None,
            true,
            Some(dcode_home.path().to_path_buf()),
        )
        .await
