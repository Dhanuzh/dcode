...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_dcode_app_path() {
        eprintln!(
            "Opening Dcode Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_dcode_app(&app_path, &workspace).await?;
        return Ok(());
    }
    eprintln!("Dcode Desktop not found; downloading installer...");
    let installed_app = download_and_install_dcode_to_user_applications(&download_url)
        .await
        .context("failed to download/install Dcode Desktop")?;
    eprintln!(
        "Launching Dcode Desktop from {installed_app}...",
        installed_app = installed_app.display()
    );
    open_dcode_app(&installed_app, &workspace).await?;
    Ok(())
}

fn find_existing_dcode_app_path() -> Option<PathBuf> {
    candidate_dcode_app_paths()
        .into_iter()
        .find(|candidate| candidate.is_dir())
}

fn candidate_dcode_app_paths() -> Vec<PathBuf> {
    let mut paths = vec![PathBuf::from("/Applications/Dcode.app")];
    if let Some(home) = std::env::var_os("HOME") {
        paths.push(PathBuf::from(home).join("Applications").join("Dcode.app"));
    }
    paths
}

async fn open_dcode_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
...
async fn download_and_install_dcode_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("dcode-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Dcode.dmg");
    download_dmg(dmg_url, &dmg_path).await?;

    eprintln!("Mounting Dcode Desktop installer...");
    ...
    let result = async {
        let app_in_volume = find_dcode_app_in_mount(&mount_point)
            .context("failed to locate Dcode.app in mounted dmg")?;
        install_dcode_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_dcode_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        eprintln!(
            "Installing Dcode Desktop into {applications_dir}...",
        ...
        let dest_app = applications_dir.join("Dcode.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Dcode.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Dcode.app to any applications directory");
}
...
fn find_dcode_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Dcode.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tDcode\t/Volumes/Dcode\n";
        assert_eq!(
    ...
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tDcode Installer\t/Volumes/Dcode Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Dcode Installer")
        );
    }
}
