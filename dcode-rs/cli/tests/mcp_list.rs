...
fn dcode_command(dcode_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::new(dcode_utils_cargo_bin::cargo_bin("dcode")?);
...
fn list_shows_empty_state() -> Result<()> {
    ...
    let mut cmd = dcode_command(dcode_home.path())?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = dcode_command(dcode_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(dcode_home.path()).await?;
    ...
    let mut list_cmd = dcode_command(dcode_home.path())?;
    ...
    let mut list_json_cmd = dcode_command(dcode_home.path())?;
    ...
    let mut get_cmd = dcode_command(dcode_home.path())?;
    ...
    let mut get_json_cmd = dcode_command(dcode_home.path())?;
    get_json_cmd
