...
use dcode_login::AuthManager;
use dcode_login::DcodeAuth;
...
pub enum Feature {
    ...
    ImageDetailOriginal,
    /// Compress request bodies (zstd) when sending streaming requests to dcode-backend.
...
impl Features {
    ...
    pub fn apps_enabled_for_auth(&self, auth: Option<&DcodeAuth>) -> bool {
        self.enabled(Feature::Apps) && auth.is_some_and(DcodeAuth::is_chatgpt_auth)
    }
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://developers.openai.com/dcode/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            announcement: "NEW: JavaScript REPL is now available in /experimental. Enable it, then start a new chat or restart Dcode to use it.",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        id: Feature::DcodeGitCommit,
        key: "dcode_git_commit",
        stage: Stage::UnderDevelopment,
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            name: "Apps",
            menu_description: "Use a connected ChatGPT App using \"$\". Install Apps via /apps command. Restart Dcode after enabling.",
            announcement: "NEW: Use ChatGPT Apps (Connectors) in Dcode via $ mentions. Enable in /experimental and restart Dcode!",
        },
        default_enabled: false,
    },
    FeatureSpec {
