//! Cloud-hosted config requirements for Dcode.
//!
...
//! requirements cannot be loaded for those accounts, Dcode fails configuration loading rather than
...
use dcode_core::auth::DcodeAuth;
use dcode_core::auth::RefreshTokenError;
...
const CLOUD_REQUIREMENTS_FETCH_ATTEMPT_METRIC: &str = "dcode.cloud_requirements.fetch_attempt";
const CLOUD_REQUIREMENTS_FETCH_FINAL_METRIC: &str = "dcode.cloud_requirements.fetch_final";
const CLOUD_REQUIREMENTS_LOAD_METRIC: &str = "dcode.cloud_requirements.load";
...
const CLOUD_REQUIREMENTS_CACHE_WRITE_HMAC_KEY: &[u8] =
    b"dcode-cloud-requirements-cache-v3-064f8542-75b4-494c-a294-97d3ce597271";
...
fn auth_identity(auth: &DcodeAuth) -> (Option<String>, Option<String>) {
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: dcode_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
    ...
    async fn fetch_with_timeout(
        ...
        let _timer =
            dcode_otel::start_global_timer("dcode.cloud_requirements.fetch.duration_ms", &[]);
...
pub fn cloud_requirements_loader_for_storage(
    ...
    cloud_requirements_loader(auth_manager, chatgpt_base_url, dcode_home)
}
...
mod tests {
    ...
    fn write_auth_json(dcode_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
        std::fs::write(dcode_home.join("auth.json"), serde_json::to_string(&value)?)?;
        Ok(())
    }
...
"#,
        ...
        assert_eq!(
            result,
            Some(ConfigRequirementsToml {
                apps: Some(dcode_core::config_loader::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        "connector_5f3c8c41a1e54ad7a76272c89e2554fa".to_string(),
                        dcode_core::config_loader::AppRequirementToml {
                            enabled: Some(false),
                        },
                    )]),
                }),
    ...
    async fn fetch_cloud_requirements_recovers_after_unauthorized_reload_updates_cache_identity() {
        ...
        let path = dcode_home.path().join(CLOUD_REQUIREMENTS_CACHE_FILENAME);
