                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Dcode failed.",
                            io::ErrorKind::Other,
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("dcode_cli_simplified_flow".to_string(), "true".to_string()),
...
pub(crate) async fn persist_tokens_async(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        save_auth(&dcode_home, &auth, auth_credentials_store_mode)
    })
    .await
...
/// Returns true when the OAuth callback represents a missing Dcode entitlement.
fn is_missing_dcode_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_dcode_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_dcode_entitlement_error(error_code, error_description) {
        return "Dcode is not enabled for your workspace. Contact your workspace administrator to request access to Dcode.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_dcode_entitlement_error(code, error_description) {
            (
                "You do not have access to Dcode".to_string(),
                "This account is not currently authorized to use Dcode in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Dcode.".to_string(),
                "Contact your workspace administrator to get access to Dcode, then return to Dcode and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Dcode to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
    template
