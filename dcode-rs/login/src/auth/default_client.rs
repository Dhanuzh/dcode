//! Default Dcode HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`DcodeHttpClient` construction.
//!
//! Use [`crate::default_client`] or [`dcode_login::default_client`] from other crates in this
//! workspace.

use dcode_client::BuildCustomCaTransportError;
use dcode_client::DcodeHttpClient;
pub use dcode_client::DcodeRequestBuilder;
use dcode_client::build_reqwest_client_with_custom_ca;
...
/// Parenthesis will be added by Dcode. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "dcode_cli_rs";
pub const CODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "CODEX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-dcode-residency";
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "dcode_vscode"
        || originator_value.starts_with("Dcode ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "dcode_atlas" || originator_value == "dcode_chatgpt_desktop"
}

pub fn get_dcode_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Dcode user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Dcode user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Dcode originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> DcodeHttpClient {
    let inner = build_reqwest_client();
    DcodeHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary Dcode HTTP traffic.
///
/// This starts from the standard Dcode user agent, default headers, and sandbox-specific proxy
...
/// Tries to build the default reqwest client used for ordinary Dcode HTTP traffic.
///
