...
fn missing_auth_json_returns_none() {
    let dir = tempdir().unwrap();
    let auth = DcodeAuth::from_auth_storage(dir.path(), AuthCredentialsStoreMode::File)
        .expect("call should succeed");
    assert_eq!(auth, None);
}

#[tokio::test]
#[serial(dcode_api_key)]
async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
    ...
    let auth = super::load_auth(dcode_home.path(), false, AuthCredentialsStoreMode::File)
        .unwrap()
        .unwrap();
...
fn write_auth_file(params: AuthFileParams, dcode_home: &Path) -> std::io::Result<String> {
...
async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
    let dcode_home = tempdir().unwrap();
    login_with_api_key(dcode_home.path(), "sk-test", AuthCredentialsStoreMode::File)
        .expect("seed api key");

    let config = build_config(dcode_home.path(), Some(ForcedLoginMethod::Chatgpt), None).await;
...
async fn enforce_login_restrictions_logs_out_for_workspace_mismatch() {
    ...
    let config = build_config(dcode_home.path(), None, Some("org_mine".to_string())).await;
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(DCODE_API_KEY_ENV_VAR, "sk-env");
    let dcode_home = tempdir().unwrap();
