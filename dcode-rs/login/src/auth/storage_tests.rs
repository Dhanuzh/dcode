...
async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
    ...
    let storage = FileAuthStorage::new(dcode_home.path().to_path_buf());
...
async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
    ...
    let file = get_auth_file(dcode_home.path());
    storage
        .save(&auth_dot_json)
        .context("failed to save auth file")?;
...
fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
    ...
    seed_keyring_with_auth(
        &mock_keyring,
        || compute_store_key(dcode_home.path()),
        &expected,
    )?;
...
fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
    let dcode_home = PathBuf::from("~/.dcode");

    let key = compute_store_key(dcode_home.as_path())?;
...
fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
    ...
    let auth_file = get_auth_file(dcode_home.path());
    std::fs::write(&auth_file, "stale")?;
    ...
    let key = compute_store_key(dcode_home.path())?;
    assert_keyring_saved_auth_and_removed_fallback(&mock_keyring, &key, dcode_home.path(), &auth);
    Ok(())
}
...
fn keyring_auth_storage_delete_removes_keyring_and_file() -> anyhow::Result<()> {
    ...
    let (key, auth_file) =
        seed_keyring_and_fallback_auth_file_for_delete(&mock_keyring, dcode_home.path(), || {
            compute_store_key(dcode_home.path())
        })?;
...
fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
    let dcode_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = AutoAuthStorage::new(dcode_home.path().to_path_buf(), Arc::new(mock_keyring));
