...
use dcode_app_server_protocol::AuthMode as ApiAuthMode;
...
use dcode_client::DcodeHttpClient;
...
struct ChatgptAuthState {
    ...
    client: DcodeHttpClient,
}

impl PartialEq for DcodeAuth {
...
impl DcodeAuth {
    fn from_auth_dot_json(
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(dcode_home.to_path_buf(), storage_mode);
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &DcodeHttpClient {
        &self.state.client
    }
}

pub const OPENAI_API_KEY_ENV_VAR: &str = "OPENAI_API_KEY";
pub const DCODE_API_KEY_ENV_VAR: &str = "DCODE_API_KEY";
...
pub fn read_dcode_api_key_from_env() -> Option<String> {
    env::var(DCODE_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `dcode_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    dcode_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(dcode_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(dcode_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(dcode_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(dcode_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(dcode_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(dcode_home, auth_credentials_store_mode)?;
    Ok(removed_ephemeral || removed_managed)
}

fn load_auth(
    dcode_home: &Path,
    enable_dcode_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<DcodeAuth>> {
    let build_auth = |auth_dot_json: AuthDotJson, storage_mode| {
        DcodeAuth::from_auth_dot_json(dcode_home, auth_dot_json, storage_mode)
    };

    // API key via env var takes precedence over any other auth method.
    if enable_dcode_api_key_env && let Some(api_key) = read_dcode_api_key_from_env() {
        return Ok(Some(DcodeAuth::from_api_key(api_key.as_str())));
    }
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(DcodeAuth::api_auth_mode),
            )
            .field(
                "external_refresher",
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(DcodeAuth::get_account_id);
        let mode = if cached_auth
            .as_ref()
            .is_some_and(DcodeAuth::is_external_chatgpt_tokens)
        {
...
/// hands out cloned `DcodeAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific DcodeAuth, for testing only.
    pub fn from_auth_for_testing(auth: DcodeAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            dcode_home: PathBuf::from("non-existent"),
    ...
    /// Create an AuthManager with a specific DcodeAuth and dcode home, for testing only.
    pub fn from_auth_for_testing_with_home(auth: DcodeAuth, dcode_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth {
            auth: Some(auth),
            external_refresher: None,
        };
    ...
    pub fn auth_cached(&self) -> Option<DcodeAuth> {
    ...
    pub async fn auth(&self) -> Option<DcodeAuth> {
    ...
    fn reload_if_account_id_matches(&self, expected_account_id: Option<&str>) -> ReloadOutcome {
        ...
        let new_auth = self.load_auth_from_storage();
        let new_account_id = new_auth.as_ref().and_then(DcodeAuth::get_account_id);
    ...
    fn auths_equal_for_refresh(a: Option<&DcodeAuth>, b: Option<&DcodeAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
    ...
    fn auths_equal(a: Option<&DcodeAuth>, b: Option<&DcodeAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    fn load_auth_from_storage(&self) -> Option<DcodeAuth> {
        load_auth(
            &self.dcode_home,
            self.enable_dcode_api_key_env,
    ...
    fn set_cached_auth(&self, new_auth: Option<DcodeAuth>) -> bool {
    ...
    pub fn dcode_api_key_env_enabled(&self) -> bool {
        self.enable_dcode_api_key_env
    }
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        let expected_account_id = auth_before_reload
            .as_ref()
            .and_then(DcodeAuth::get_account_id);
    ...
    pub async fn refresh_token_from_authority(&self) -> Result<(), RefreshTokenError> {
        ...
        match auth {
            DcodeAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            DcodeAuth::Chatgpt(chatgpt_auth) => {
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.dcode_home, self.auth_credentials_store_mode)?;
    ...
    pub fn get_api_auth_mode(&self) -> Option<ApiAuthMode> {
        self.auth_cached().as_ref().map(DcodeAuth::api_auth_mode)
    }

    pub fn auth_mode(&self) -> Option<crate::AuthMode> {
        self.auth_cached().as_ref().map(DcodeAuth::auth_mode)
    }

    fn is_stale_for_proactive_refresh(auth: &DcodeAuth) -> bool {
        let chatgpt_auth = match auth {
            DcodeAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return false,
        };
