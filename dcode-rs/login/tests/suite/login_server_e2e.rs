...
use dcode_login::auth::AuthCredentialsStoreMode;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = dcode_home.clone();

    let opts = ServerOptions {
        dcode_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: dcode_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = dcode_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_dcode_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let dcode_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_dcode_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Dcode"),
        "error body should clearly explain the Dcode access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_dcode_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Dcode to retry"),
    ...
    assert!(
        !body.contains("get access to Dcode"),
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_dcode_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        dcode_home: first_dcode_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: dcode_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_dcode_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        dcode_home: second_dcode_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: dcode_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
