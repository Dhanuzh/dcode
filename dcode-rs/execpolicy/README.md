...
- The legacy rule matcher lives in `dcode-execpolicy-legacy`.
...
- From the Dcode CLI, run `dcode execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
dcode execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
dcode execpolicy check \
...
```bash
cargo run -p dcode-execpolicy -- check --rules path/to/policy.rules git status
```
