...
use dcode_apply_patch::CODEX_CORE_APPLY_PATCH_ARG1;
use dcode_utils_home_dir::find_dcode_home;
#[cfg(unix)]
use std::os::unix::fs::symlink;
use tempfile::TempDir;

const LINUX_SANDBOX_ARG0: &str = "dcode-linux-sandbox";
const APPLY_PATCH_ARG0: &str = "apply_patch";
const MISSPELLED_APPLY_PATCH_ARG0: &str = "applypatch";
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "dcode-execve-wrapper";
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            dcode_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if argv1 == CODEX_CORE_APPLY_PATCH_ARG1 {
        ...
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                ...
                match dcode_apply_patch::apply_patch(&patch_arg, &mut stdout, &mut stderr) {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
            }
    ...
    match prepend_path_entry_for_dcode_aliases() {
        Ok(path_entry) => Some(path_entry),
        Err(err) => {
            // It is possible that Dcode will proceed successfully even if
...
/// While we want to deploy the Dcode CLI as a single executable for simplicity,
...
/// `dcode-linux-sandbox` we *directly* execute
/// [`dcode_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.dcode/.env` before creating any threads.
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    runtime.block_on(async move {
        ...
        let paths = Arg0DispatchPaths {
            dcode_linux_sandbox_exe: if cfg!(target_os = "linux") {
                current_exe.or_else(|| {
                    path_entry
                        .as_ref()
...
/// Load env vars from ~/.dcode/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `CODEX_`.
fn load_dotenv() {
    if let Ok(dcode_home) = find_dcode_home()
        && let Ok(iter) = dotenvy::from_path_iter(dcode_home.join(".env"))
    {
        set_filtered(iter);
    }
}
...
///   with the "secret" --dcode-run-as-apply-patch flag.
///
...
pub fn prepend_path_entry_for_dcode_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    let dcode_home = find_dcode_home()?;
    #[cfg(not(debug_assertions))]
    {
        // Guard against placing helpers in system temp directories outside debug builds.
        let temp_root = std::env::temp_dir();
        if dcode_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (dcode_home: {dcode_home:?})"
                ),
            ));
        }
    }
    ...
    let temp_root = dcode_home.join("tmp").join("arg0");
