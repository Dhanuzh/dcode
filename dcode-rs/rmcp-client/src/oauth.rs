...
const KEYRING_SERVICE: &str = "Dcode MCP Credentials";
...
/// Determine where Dcode should store and read MCP credentials.
...
mod tests {
    ...
    use dcode_keyring_store::tests::MockKeyringStore;

    struct TempDcodeHome {
    ...
    impl TempDcodeHome {
        fn new() -> Self {
            ...
            unsafe {
                std::env::set_var("DCODE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempDcodeHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("DCODE_HOME");
            }
        }
    }
