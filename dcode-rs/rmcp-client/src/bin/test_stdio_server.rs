...
const MEMO_URI: &str = "memo://dcode/example-note";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Dcode TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Dcode TUI):
    /// - Build this binary: `cargo build -p dcode-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `dcode mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Dcode TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://dcode/{slug}".to_string(),
            name: "dcode-memo".to_string(),
            title: Some("Dcode Memo".to_string()),
            description: Some(
                "Template for memo://dcode/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Dcode TUI handling of MCP image outputs.
///
