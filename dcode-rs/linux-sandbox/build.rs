...
fn try_build_vendored_bwrap() -> Result<(), String> {
    ...
    std::fs::write(
        &config_h,
        r#"#pragma once
#define PACKAGE_STRING "bubblewrap built at dcode build-time"
"#,
    )
...
/// 2. The vendored bubblewrap tree under `dcode-rs/vendor/bubblewrap`.
...
Set CODEX_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under dcode-rs/vendor.",
        vendor_dir.display()
    ))
}
