...
- a `dcode-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Dcode CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `dcode-exec` CLI can check if its arg0 is `dcode-linux-sandbox` and, if so, execute as if it were `dcode-linux-sandbox`
  - this should also be true of the `dcode` multitool CLI
...
Dcode also surfaces a startup warning when `/usr/bin/bwrap` is missing or too
...
- If `/usr/bin/bwrap` is missing or too old to support the required flags,
  Dcode also surfaces a startup warning instead of printing directly from the
  sandbox helper.
...
- When the default bubblewrap pipeline is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.dcode`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface still uses legacy names like `dcode debug landlock`.
