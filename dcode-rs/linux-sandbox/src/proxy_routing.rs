...
const PROXY_SOCKET_DIR_PREFIX: &str = "dcode-linux-sandbox-proxy-";
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(dcode_home) = std::env::var_os("DCODE_HOME") {
        let candidate = PathBuf::from(dcode_home).join("tmp");
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("dcode-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("dcode-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("dcode-linux-sandbox-proxy-x"),
            None
        );
