...
//! - sensitive subpaths such as `.git` and `.dcode` remain read-only even when
...
/// inside a writable root (e.g., `.dcode -> ./decoy`). In that case we mount
...
mod tests {
    use super::*;
    use dcode_protocol::protocol::FileSystemAccessMode;
    use dcode_protocol::protocol::FileSystemPath;
    use dcode_protocol::protocol::FileSystemSandboxEntry;
    use dcode_protocol::protocol::FileSystemSandboxPolicy;
    use dcode_protocol::protocol::FileSystemSpecialPath;
