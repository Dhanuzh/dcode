...
use dcode_core::config::types::ShellEnvironmentPolicy;
use dcode_core::error::DcodeErr;
use dcode_core::error::Result;
use dcode_core::error::SandboxErr;
use dcode_core::exec::ExecCapturePolicy;
use dcode_core::exec::ExecParams;
use dcode_core::exec::process_exec_tool_call;
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> dcode_core::exec::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<dcode_core::exec::ExecToolCallOutput> {
...
async fn run_cmd_result_with_policies(
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_dcode-linux-sandbox");
    let dcode_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
fn is_bwrap_unavailable_output(output: &dcode_core::exec::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(DcodeErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(DcodeErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<dcode_core::exec::ExecToolCallOutput>,
    context: &str,
) -> dcode_core::exec::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(DcodeErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_dcode-linux-sandbox");
    let dcode_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
...
#[tokio::test]
async fn sandbox_blocks_git_and_dcode_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_dcode = tmpdir.path().join(".dcode");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_dcode).expect("create .dcode");

    let git_target = dot_git.join("config");
    let dcode_target = dot_dcode.join("config.toml");
    ...
    let dcode_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", dcode_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            false,
        ...
        .await,
        ".dcode write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(dcode_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_dcode_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-dcode");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_dcode = tmpdir.path().join(".dcode");
    symlink(&decoy, &dot_dcode).expect("create .dcode symlink");
    ...
    let dcode_output = expect_denied(
        ...
        .await,
        ".dcode symlink replacement should be denied",
    );
    assert_ne!(dcode_output.exit_code, 0);
}
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = PathBuf::from(env!("CARGO_BIN_EXE_dcode-linux-sandbox"))
        .parent()
