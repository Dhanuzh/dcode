...
use dcode_package_manager::ArchiveFormat;
...
#[test]
fn default_release_locator_uses_openai_dcode_github_releases() {
...
fn load_cached_runtime_reads_installed_runtime() {
    let dcode_home = TempDir::new().unwrap_or_else(|error| panic!("{error}"));
    ...
    let runtime = load_cached_runtime(
        &dcode_home.path().join(DEFAULT_CACHE_ROOT_RELATIVE),
        runtime_version,
    )
    .unwrap_or_else(|error| panic!("{error}"));
...
fn load_cached_runtime_uses_custom_cache_root() {
    let dcode_home = TempDir::new().unwrap_or_else(|error| panic!("{error}"));
    let runtime_version = "2.5.6";
    let custom_cache_root = dcode_home.path().join("runtime-cache");
    let platform =
        ArtifactRuntimePlatform::detect_current().unwrap_or_else(|error| panic!("{error}"));
    ...
    let output = client
        .execute_build(ArtifactBuildRequest {
            source: concat!(
                ...
                "console.log(typeof dcodeArtifacts);\n",
