...
use dcode_package_manager::ManagedPackage;
use dcode_package_manager::PackageManager;
use dcode_package_manager::PackageManagerConfig;
use dcode_package_manager::PackageManagerError;
use dcode_package_manager::PackageReleaseArchive;
use reqwest::Client;
use std::path::Path;
...
/// Relative cache root for installed artifact runtimes under `dcode_home`.
...
/// Configuration for resolving artifact runtimes under a Dcode home directory.
#[derive(Clone, Debug, PartialEq, Eq)]
...
impl ArtifactRuntimeManagerConfig {
    /// Creates a runtime-manager config from a Dcode home and explicit release locator.
    pub fn new(dcode_home: PathBuf, release: ArtifactRuntimeReleaseLocator) -> Self {
        Self {
    ...
    pub fn with_default_release(dcode_home: PathBuf, runtime_version: impl Into<String>) -> Self {
        Self::new(
            dcode_home,
