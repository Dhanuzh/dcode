...
pub fn is_js_runtime_available(dcode_home: &Path, runtime_version: &str) -> bool {
    load_cached_runtime(&default_cached_runtime_root(dcode_home), runtime_version)
        .ok()
...
pub(crate) fn resolve_js_runtime_from_candidates(
    ...
    dcode_app_candidates: Vec<PathBuf>,
) -> Option<JsRuntime> {
    node_runtime.or(electron_runtime).or_else(|| {
        dcode_app_candidates
            .into_iter()
...
pub(crate) fn dcode_app_runtime_candidates() -> Vec<PathBuf> {
