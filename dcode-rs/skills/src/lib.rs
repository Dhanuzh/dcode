...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".dcode-system-skills.marker";
...
pub fn system_cache_root_dir(dcode_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(dcode_home)
        .and_then(|dcode_home| system_cache_root_dir_abs(&dcode_home))
...
fn system_cache_root_dir_abs(dcode_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    dcode_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}
...
/// install is skipped.
pub fn install_system_skills(dcode_home: &Path) -> Result<(), SystemSkillsError> {
    let dcode_home = AbsolutePathBuf::try_from(dcode_home)
        .map_err(|source| SystemSkillsError::io("normalize dcode home dir", source))?;
    let skills_root_dir = dcode_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&dcode_home)
