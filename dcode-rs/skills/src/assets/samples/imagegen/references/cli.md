...
Set a stable path to the skill CLI (default `DCODE_HOME` is `~/.dcode`):

```
export DCODE_HOME="${DCODE_HOME:-$HOME/.dcode}"
export IMAGE_GEN="$DCODE_HOME/skills/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/dcode-network.md`
