...
- This repo’s scaffold writes `.dcode-plugin/plugin.json`; treat that as the manifest location this skill generates.
...
- `plugins` (`array`): Ordered plugin entries. This order determines how Dcode renders plugins.
...
- `source` (`object`): Plugin source descriptor.
  ...
  - `path` (`string`): Relative plugin path based on the marketplace root.
    ...
    - Local plugin in `~/.agents/plugins/marketplace.json`: `./.dcode/plugins/<plugin-name>`
