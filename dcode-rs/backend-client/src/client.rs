...
impl Client {
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &DcodeAuth) -> Result<Self> {
        let token = auth.get_token().map_err(anyhow::Error::from)?;
        let mut client = Self::new(base_url)?
            .with_user_agent(get_dcode_user_agent())
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("dcode-cli"));
        }
    ...
    pub async fn get_rate_limits_many(&self) -> Result<Vec<RateLimitSnapshot>> {
        let url = match self.path_style {
            PathStyle::DcodeApi => format!("{}/api/dcode/usage", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::DcodeApi => format!("{}/api/dcode/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::DcodeApi => format!("{}/api/dcode/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::DcodeApi => format!(
                "{}/api/dcode/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the managed requirements file from dcode-backend.
    ///
    /// `GET /api/dcode/config/requirements` (Dcode API style) or
    ...
    pub async fn get_config_requirements_file(
        ...
        let url = match self.path_style {
            PathStyle::DcodeApi => format!("{}/api/dcode/config/requirements", self.base_url),
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::DcodeApi => format!("{}/api/dcode/tasks", self.base_url),
...
mod tests {
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![crate::types::AdditionalRateLimitDetails {
                limit_name: "dcode_other".to_string(),
                metered_feature: "dcode_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("dcode"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("dcode_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("dcode_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("dcode"));
    }
}
