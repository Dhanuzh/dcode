...
/// The easiest way to iterate is to add unit tests and have Dcode fix the implementation.
...
/// Tests are at the top to encourage using TDD + Dcode to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd dcode-rs && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner = r#"cd /Users/pakrym/code/dcode && rg -n "dcode_api" dcode-rs -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n dcode_api dcode-rs -S".to_string(),
                query: Some("dcode_api".to_string()),
                path: Some("dcode-rs".to_string()),
            }],
        );
    }
