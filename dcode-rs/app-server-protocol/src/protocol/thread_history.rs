...
use dcode_protocol::protocol::CompactedItem;
use dcode_protocol::protocol::ContextCompactedEvent;
use dcode_protocol::protocol::DynamicToolCallResponseEvent;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `dcode-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &dcode_protocol::models::ResponseItem) {
    ...
    fn handle_item_started(&mut self, payload: &ItemStartedEvent) {
        match &payload.item {
            dcode_protocol::items::TurnItem::Plan(plan) => {
            ...
            dcode_protocol::items::TurnItem::UserMessage(_)
            | dcode_protocol::items::TurnItem::HookPrompt(_)
            | dcode_protocol::items::TurnItem::AgentMessage(_)
            | dcode_protocol::items::TurnItem::Reasoning(_)
            | dcode_protocol::items::TurnItem::WebSearch(_)
            | dcode_protocol::items::TurnItem::ImageGeneration(_)
            | dcode_protocol::items::TurnItem::ContextCompaction(_) => {}
        }
    }
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &dcode_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &dcode_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &dcode_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &dcode_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &dcode_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &dcode_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &dcode_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &dcode_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &dcode_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &dcode_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &dcode_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(&mut self, payload: &dcode_protocol::protocol::ReviewRequest) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &dcode_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        turn.error = Some(V2TurnError {
            ...
            dcode_error_info: payload.dcode_error_info.clone().map(Into::into),
...
pub fn convert_patch_changes(
    changes: &HashMap<std::path::PathBuf, dcode_protocol::protocol::FileChange>,
...
fn convert_dynamic_tool_content_items(
    items: &[dcode_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            dcode_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            dcode_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
...
fn map_patch_change_kind(change: &dcode_protocol::protocol::FileChange) -> PatchChangeKind {
    match change {
        dcode_protocol::protocol::FileChange::Add { .. } => PatchChangeKind::Add,
        dcode_protocol::protocol::FileChange::Delete { .. } => PatchChangeKind::Delete,
        dcode_protocol::protocol::FileChange::Update { move_path, .. } => PatchChangeKind::Update {
            move_path: move_path.clone(),
        },
    }
}

fn format_file_change_diff(change: &dcode_protocol::protocol::FileChange) -> String {
    match change {
        dcode_protocol::protocol::FileChange::Add { content } => content.clone(),
        dcode_protocol::protocol::FileChange::Delete { content } => content.clone(),
...
mod tests {
    ...
    use dcode_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use dcode_protocol::items::TurnItem as CoreTurnItem;
    use dcode_protocol::items::UserMessageItem as CoreUserMessageItem;
    use dcode_protocol::items::build_hook_prompt_message;
    use dcode_protocol::models::MessagePhase as CoreMessagePhase;
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                dcode_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(dcode_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(dcode_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: dcode_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(dcode_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                dcode_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                dcode_error_info: Some(DcodeErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                ...
                dcode_error_info: Some(
                    crate::protocol::v2::DcodeErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(dcode_protocol::models::ResponseItem::Message {
                id: Some("msg-1".into()),
                role: "user".into(),
