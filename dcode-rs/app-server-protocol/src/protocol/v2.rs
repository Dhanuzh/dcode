...
use dcode_experimental_api_macros::ExperimentalApi;
use dcode_protocol::account::PlanType;
use dcode_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use dcode_protocol::approvals::ExecApprovalRequestSkillMetadata as CoreExecApprovalRequestSkillMetadata;
use dcode_protocol::approvals::ExecPolicyAmendment as CoreExecPolicyAmendment;
use dcode_protocol::approvals::NetworkApprovalContext as CoreNetworkApprovalContext;
use dcode_protocol::approvals::NetworkApprovalProtocol as CoreNetworkApprovalProtocol;
use dcode_protocol::approvals::NetworkPolicyAmendment as CoreNetworkPolicyAmendment;
use dcode_protocol::approvals::NetworkPolicyRuleAction as CoreNetworkPolicyRuleAction;
use dcode_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use dcode_protocol::config_types::CollaborationMode;
use dcode_protocol::config_types::CollaborationModeMask as CoreCollaborationModeMask;
...
use dcode_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use dcode_protocol::config_types::WebSearchToolConfig;
use dcode_protocol::items::AgentMessageContent as CoreAgentMessageContent;
use dcode_protocol::items::TurnItem as CoreTurnItem;
use dcode_protocol::mcp::Resource as McpResource;
use dcode_protocol::mcp::ResourceTemplate as McpResourceTemplate;
use dcode_protocol::mcp::Tool as McpTool;
use dcode_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use dcode_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
use dcode_protocol::models::FileSystemPermissions as CoreFileSystemPermissions;
use dcode_protocol::models::MacOsAutomationPermission as CoreMacOsAutomationPermission;
use dcode_protocol::models::MacOsContactsPermission as CoreMacOsContactsPermission;
use dcode_protocol::models::MacOsPreferencesPermission as CoreMacOsPreferencesPermission;
use dcode_protocol::models::MacOsSeatbeltProfileExtensions as CoreMacOsSeatbeltProfileExtensions;
use dcode_protocol::models::MessagePhase;
use dcode_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use dcode_protocol::models::PermissionProfile as CorePermissionProfile;
use dcode_protocol::models::ResponseItem;
use dcode_protocol::openai_models::InputModality;
use dcode_protocol::openai_models::ModelAvailabilityNux as CoreModelAvailabilityNux;
use dcode_protocol::openai_models::ReasoningEffort;
use dcode_protocol::openai_models::default_input_modalities;
use dcode_protocol::parse_command::ParsedCommand as CoreParsedCommand;
use dcode_protocol::plan_tool::PlanItemArg as CorePlanItemArg;
use dcode_protocol::plan_tool::StepStatus as CorePlanStepStatus;
use dcode_protocol::protocol::AgentStatus as CoreAgentStatus;
use dcode_protocol::protocol::AskForApproval as CoreAskForApproval;
use dcode_protocol::protocol::DcodeErrorInfo as CoreDcodeErrorInfo;
use dcode_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use dcode_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use dcode_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
use dcode_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use dcode_protocol::protocol::GuardianRiskLevel as CoreGuardianRiskLevel;
use dcode_protocol::protocol::HookEventName as CoreHookEventName;
use dcode_protocol::protocol::HookExecutionMode as CoreHookExecutionMode;
use dcode_protocol::protocol::HookHandlerType as CoreHookHandlerType;
use dcode_protocol::protocol::HookOutputEntry as CoreHookOutputEntry;
use dcode_protocol::protocol::HookOutputEntryKind as CoreHookOutputEntryKind;
use dcode_protocol::protocol::HookRunStatus as CoreHookRunStatus;
use dcode_protocol::protocol::HookRunSummary as CoreHookRunSummary;
use dcode_protocol::protocol::HookScope as CoreHookScope;
use dcode_protocol::protocol::ModelRerouteReason as CoreModelRerouteReason;
use dcode_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use dcode_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
use dcode_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
use dcode_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use dcode_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use dcode_protocol::protocol::ReadOnlyAccess as CoreReadOnlyAccess;
use dcode_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
use dcode_protocol::protocol::RealtimeConversationVersion;
use dcode_protocol::protocol::ReviewDecision as CoreReviewDecision;
use dcode_protocol::protocol::SessionSource as CoreSessionSource;
use dcode_protocol::protocol::SkillDependencies as CoreSkillDependencies;
use dcode_protocol::protocol::SkillErrorInfo as CoreSkillErrorInfo;
use dcode_protocol::protocol::SkillInterface as CoreSkillInterface;
use dcode_protocol::protocol::SkillMetadata as CoreSkillMetadata;
use dcode_protocol::protocol::SkillScope as CoreSkillScope;
use dcode_protocol::protocol::SkillToolDependency as CoreSkillToolDependency;
use dcode_protocol::protocol::SubAgentSource as CoreSubAgentSource;
use dcode_protocol::protocol::TokenUsage as CoreTokenUsage;
use dcode_protocol::protocol::TokenUsageInfo as CoreTokenUsageInfo;
...
use dcode_protocol::user_input::ByteRange as CoreByteRange;
use dcode_protocol::user_input::TextElement as CoreTextElement;
use dcode_protocol::user_input::UserInput as CoreUserInput;
...
/// This translation layer make sure that we expose dcode error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `dcodeErrorInfo` variant.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub enum DcodeErrorInfo {
...
impl From<CoreDcodeErrorInfo> for DcodeErrorInfo {
    fn from(value: CoreDcodeErrorInfo) -> Self {
        match value {
            CoreDcodeErrorInfo::ContextWindowExceeded => DcodeErrorInfo::ContextWindowExceeded,
            CoreDcodeErrorInfo::UsageLimitExceeded => DcodeErrorInfo::UsageLimitExceeded,
            CoreDcodeErrorInfo::ServerOverloaded => DcodeErrorInfo::ServerOverloaded,
            CoreDcodeErrorInfo::HttpConnectionFailed { http_status_code } => {
                DcodeErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreDcodeErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                DcodeErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreDcodeErrorInfo::InternalServerError => DcodeErrorInfo::InternalServerError,
            CoreDcodeErrorInfo::Unauthorized => DcodeErrorInfo::Unauthorized,
            CoreDcodeErrorInfo::BadRequest => DcodeErrorInfo::BadRequest,
            CoreDcodeErrorInfo::ThreadRollbackFailed => DcodeErrorInfo::ThreadRollbackFailed,
            CoreDcodeErrorInfo::SandboxError => DcodeErrorInfo::SandboxError,
            CoreDcodeErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                DcodeErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreDcodeErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                DcodeErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreDcodeErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                DcodeErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreDcodeErrorInfo::Other => DcodeErrorInfo::Other,
        }
    }
}
...
v2_enum_from_core!(
    pub enum ReviewDelivery from dcode_protocol::protocol::ReviewDelivery {
        Inline, Detached
    }
);

v2_enum_from_core!(
    pub enum McpAuthStatus from dcode_protocol::protocol::McpAuthStatus {
        Unsupported,
        NotLoggedIn,
        BearerToken,
        OAuth
...
pub enum ConfigLayerSource {
    ...
    /// Path to a .dcode/ folder within a project. There could be multiple of
    /// these between `cwd` and the project/repo root.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    Project {
        dot_dcode_folder: AbsolutePathBuf,
    },
...
pub struct ExternalAgentConfigDetectParams {
    /// If true, include detection under the user's home (~/.claude, ~/.dcode, etc.).
    #[serde(default, skip_serializing_if = "std::ops::Not::not")]
...
impl SandboxPolicy {
    pub fn to_core(&self) -> dcode_protocol::protocol::SandboxPolicy {
        match self {
...
impl From<dcode_protocol::protocol::SandboxPolicy> for SandboxPolicy {
    fn from(value: dcode_protocol::protocol::SandboxPolicy) -> Self {
        match value {
            dcode_protocol::protocol::SandboxPolicy::DangerFullAccess => {
                SandboxPolicy::DangerFullAccess
            }
            ...
            dcode_protocol::protocol::SandboxPolicy::ExternalSandbox { network_access } => {
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that Dcode-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, Dcode attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// Dcode attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that Dcode was previously using.
    ///
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `dcode`).
...
pub struct ThreadStartParams {
    ...
    /// This is for internal use only (e.g. Dcode Cloud).
...
pub struct Thread {
    ...
    /// Origin of the thread (CLI, VSCode, dcode exec, dcode app-server, etc.).
...
pub struct TurnError {
    pub message: String,
    pub dcode_error_info: Option<DcodeErrorInfo>,
    #[serde(default)]
...
impl From<dcode_protocol::models::WebSearchAction> for WebSearchAction {
    fn from(value: dcode_protocol::models::WebSearchAction) -> Self {
        match value {
            dcode_protocol::models::WebSearchAction::Search { query, queries } => {
                WebSearchAction::Search { query, queries }
            }
            dcode_protocol::models::WebSearchAction::OpenPage { url } => {
                WebSearchAction::OpenPage { url }
            }
            dcode_protocol::models::WebSearchAction::FindInPage { url, pattern } => {
                WebSearchAction::FindInPage { url, pattern }
            }
            dcode_protocol::models::WebSearchAction::Other => WebSearchAction::Other,
        }
    }
}
...
impl From<dcode_protocol::items::HookPromptFragment> for HookPromptFragment {
    fn from(value: dcode_protocol::items::HookPromptFragment) -> Self {
        Self {
            text: value.text,
...
impl McpServerElicitationAction {
    pub fn to_core(self) -> dcode_protocol::approvals::ElicitationAction {
        match self {
            Self::Accept => dcode_protocol::approvals::ElicitationAction::Accept,
            Self::Decline => dcode_protocol::approvals::ElicitationAction::Decline,
            Self::Cancel => dcode_protocol::approvals::ElicitationAction::Cancel,
        }
    }
}
...
pub struct McpServerElicitationRequestParams {
    pub thread_id: String,
    /// Active Dcode turn when this elicitation was observed, if app-server could correlate one.
    ///
...
impl From<DynamicToolCallOutputContentItem>
    for dcode_protocol::dynamic_tools::DynamicToolCallOutputContentItem
{
...
mod tests {
    ...
    use dcode_protocol::items::ReasoningItem;
    use dcode_protocol::items::TurnItem;
    use dcode_protocol::items::UserMessageItem;
    use dcode_protocol::items::WebSearchItem;
    use dcode_protocol::models::WebSearchAction as CoreWebSearchAction;
    use dcode_protocol::protocol::NetworkAccess as CoreNetworkAccess;
    use dcode_protocol::protocol::ReadOnlyAccess as CoreReadOnlyAccess;
    use dcode_protocol::user_input::UserInput as CoreUserInput;
        ...
        let user_item = TurnItem::UserMessage(UserMessageItem {
            ...
            content: vec![
                ...
                CoreUserInput::Skill {
                    ...
                    path: PathBuf::from("/repo/.dcode/skills/skill-creator/SKILL.md"),
                },
    ...
    #[test]
    fn dcode_error_info_serializes_http_status_code_in_camel_case() {
        let value = DcodeErrorInfo::ResponseTooManyFailedAttempts {
    ...
    #[test]
    fn dcode_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
        let value = DcodeErrorInfo::ActiveTurnNotSteerable {
