...
fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let mut child = Command::new(dcode_utils_cargo_bin::cargo_bin("dcode-stdio-to-uds")?)
        .arg(&socket_path)
        .stdin(Stdio::from(stdin))
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .context("failed to spawn dcode-stdio-to-uds")?;
    ...
    let status = loop {
        ...
        if Instant::now() >= deadline {
            ...
            anyhow::bail!(
                "dcode-stdio-to-uds did not exit in time; server events: {:?}; stderr: {}",
                server_events,
    ...
    assert!(
        status.success(),
        "dcode-stdio-to-uds exited with {status}; server events: {:?}; stderr: {}",
        server_events,
        String::from_utf8_lossy(&stderr).trim_end()
    );
