...
fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let dcode_home = temp_dir.path().join(".dcode");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = dcode_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("dcode-arg0-session");
    let sibling_arg0_dir = arg0_root.join("dcode-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("dcode-execve-wrapper");
