...
use dcode_app_server_protocol::Tools;
use dcode_app_server_protocol::UserSavedConfig;
use dcode_features::Feature;
use dcode_features::FeatureConfigSource;
use dcode_features::FeatureOverrides;
use dcode_features::Features;
use dcode_features::FeaturesToml;
use dcode_protocol::config_types::AltScreenMode;
...
use dcode_protocol::config_types::WebSearchConfig;
...
use dcode_protocol::models::MacOsSeatbeltProfileExtensions;
...
use dcode_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_dcode_runtime;
use crate::config::permissions::network_proxy_config_from_profile_network;
use crate::config::profile::ConfigProfile;
use dcode_network_proxy::NetworkProxyConfig;
...
pub mod service;
pub mod types;
pub use dcode_config::Constrained;
pub use dcode_config::ConstraintError;
pub use dcode_config::ConstraintResult;
pub use dcode_network_proxy::NetworkProxyAuditMetadata;
...
pub use dcode_git::GhostSnapshotConfig;
...
fn system_bwrap_warning_for_path(system_bwrap_path: &Path) -> Option<String> {
    if !system_bwrap_path.is_file() {
        return Some(format!(
            "Dcode could not find system bubblewrap at {}. Please install bubblewrap with your package manager. Dcode will use the vendored bubblewrap in the meantime.",
    ...
    Some(format!(
        "Dcode found system bubblewrap at {}, but it is too old to support `--argv0`. Please upgrade bubblewrap with your package manager. Dcode will use the vendored bubblewrap in the meantime.",
        system_bwrap_path.display()
    ))
}
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(dcode_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) fn test_config() -> Config {
    let dcode_home = tempfile::tempdir().expect("create temp dir");
...
pub struct Config {
    ...
    ///
    /// - `None`: use default attribution (`Dcode <noreply@openai.com>`)
    /// - `Some("")` or whitespace-only: disable commit attribution
    /// - `Some("...")`: use the provided attribution text verbatim
    pub commit_attribution: Option<String>,

    /// Optional external notifier command. When set, Dcode will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Dcode
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.dcode/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Dcode"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Dcode '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Dcode home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Dcode can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Dcode unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/dcode-rs/rmcp-client/src/oauth.rs#L2
    /// file: CODEX_HOME/.credentials.json
    ///       This file will be readable to Dcode and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Dcode will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Dcode state (defaults to `~/.dcode` but can be
    /// overridden by the `CODEX_HOME` environment variable).
    pub dcode_home: PathBuf,

    /// Directory where Dcode stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Dcode writes log files (defaults to `$CODEX_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.dcode/history.jsonl`.
    pub history: History,
    ...
    /// Path to the `dcode-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `dcode-linux-sandbox`.
    pub dcode_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `dcode-execve-wrapper` executable used for shell
    ...
    /// When `true`, checks for Dcode updates on startup and surfaces update prompts.
    /// Set to `false` only if your Dcode updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Dcode product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Dcode product surfaces.
    /// Defaults to `true`.
...
impl ConfigBuilder {
    pub fn dcode_home(mut self, dcode_home: PathBuf) -> Self {
        self.dcode_home = Some(dcode_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let dcode_home = dcode_home.map_or_else(find_dcode_home, std::io::Result::Ok)?;
...
impl Config {
    ...
    pub fn load_default_with_cli_overrides(
        ...
        let dcode_home = find_dcode_home()?;
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &dcode_home)?;
    ...
    /// ignores user settings. For example, the `dcode exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `dcode_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, dcode_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .dcode/ folders.
...
pub fn set_project_trust_level(
    ...
    ConfigEditsBuilder::new(dcode_home)
        .set_project_trust_level(project_path, trust_level)
        .apply_blocking()
}

/// Save the default OSS provider preference to config.toml
pub fn set_default_oss_provider(dcode_home: &Path, provider: &str) -> std::io::Result<()> {
...
/// Base config deserialized from ~/.dcode/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Dcode will likely degrade model performance.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          https://github.com/openai/codex/blob/main/dcode-rs/rmcp-client/src/oauth.rs#L2
    /// file: Use a file in the Dcode home directory.
    ...
    /// Directory where Dcode writes log files, for example `dcode-tui.log`.
    ...
    /// directories for `.dcode` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use dcode_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
                    ...
                    if p.is_trusted() || p.is_untrusted() {
                        if cfg!(target_os = "windows")
                            && windows_sandbox_level
                                == dcode_protocol::config_types::WindowsSandboxLevel::Disabled
                        {
...
impl Config {
    ...
    fn load_from_base_config_with_overrides(
        ...
        Self::load_config_with_layer_stack(cfg, overrides, dcode_home, config_layer_stack)
    }
    ...
    pub(crate) fn load_config_with_layer_stack(
        ...
        let user_instructions = Self::load_instructions(Some(&dcode_home));
        ...
        let memories_root = memory_root(&dcode_home);
        ...
        let sqlite_home = cfg
            ...
            .unwrap_or_else(|| dcode_home.to_path_buf());
        ...
        let helper_readable_roots = get_readable_roots_required_for_dcode_runtime(
            &dcode_home,
            zsh_path.as_ref(),
    ...
    fn load_instructions(dcode_dir: Option<&Path>) -> Option<String> {
        let base = dcode_dir?;
...
/// Returns the path to the Dcode configuration directory, which can be
...
///   directory exists.
pub fn find_dcode_home() -> std::io::Result<PathBuf> {
    dcode_utils_home_dir::find_dcode_home()
}

/// Returns the path to the folder where Dcode logs are stored. Does not verify
