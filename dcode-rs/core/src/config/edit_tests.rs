...
fn blocking_set_model_top_level() {
    ...
    let dcode_home = tmp.path();

    apply_blocking(
        dcode_home,
        None,
        &[ConfigEdit::SetModel {
            model: Some("gpt-5.1-dcode".to_string()),
            effort: Some(ReasoningEffort::High),
        }],
    )
    .expect("persist");

    let contents = std::fs::read_to_string(dcode_home.join(CONFIG_TOML_FILE)).expect("read config");
    let expected = r#"model = "gpt-5.1-dcode"
...
"#,
    ...
    let raw = std::fs::read_to_string(dcode_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = dcode_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let dcode_home = tmp.path();
    let link_a = dcode_home.join("a.toml");
    let link_b = dcode_home.join("b.toml");
    let config_path = dcode_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(dcode_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(dcode_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_dcode_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let dcode_home = tmp.path();
    std::fs::write(
        dcode_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        ...
        None,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-dcode-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-dcode-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
"#,
    ...
    apply_blocking(dcode_home, None, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !dcode_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(dcode_home.join(CONFIG_TOML_FILE)).expect("read config");
    assert_eq!(contents, initial_expected);

    let updated_expected = r#"model = "gpt-5.1-dcode"
model_reasoning_effort = "high"
"#;
    ConfigEditsBuilder::new(dcode_home)
        .set_model(Some("gpt-5.1-dcode"), Some(ReasoningEffort::High))
    ...
    contents = std::fs::read_to_string(dcode_home.join(CONFIG_TOML_FILE)).expect("read config");
