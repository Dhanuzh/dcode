...
use dcode_app_server_protocol::Config as ApiConfig;
use dcode_app_server_protocol::ConfigBatchWriteParams;
use dcode_app_server_protocol::ConfigLayerMetadata;
...
use dcode_app_server_protocol::OverriddenMetadata;
...
impl ConfigService {
    ...
    pub fn new_with_defaults(dcode_home: PathBuf) -> Self {
        Self {
            dcode_home,
        ...
        let layers = match params.cwd.as_deref() {
            Some(cwd) => {
                ...
                crate::config::ConfigBuilder::default()
                    .dcode_home(self.dcode_home.clone())
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<dcode_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.dcode_home)
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), &self.dcode_home).map_err(
                |err| {
                    ConfigServiceError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .dcode/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_dcode_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_dcode_folder.display(),
        ),
