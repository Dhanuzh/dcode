...
async fn apply_explorer_role_sets_model_and_adds_session_flags_layer() {
    ...
    assert_eq!(config.model.as_deref(), Some("gpt-5.1-dcode-mini"));
...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.dcode_linux_sandbox_exe = Some(PathBuf::from("/tmp/dcode-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/dcode-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(
        config.dcode_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/dcode-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/dcode-execve-wrapper"))
    );
}
