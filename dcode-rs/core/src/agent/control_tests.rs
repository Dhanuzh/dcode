...
impl AgentControlHarness {
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<DcodeThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<DcodeThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<DcodeThread>, message: &str) {
    thread
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, DcodeErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn spawn_agent_can_fork_parent_thread_history() {
    ...
    parent_thread
        ...
        .await;
    let turn_context = parent_thread.dcode.session.new_default_turn().await;
    ...
    let history = child_thread.dcode.session.clone_history().await;
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let DcodeErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected DcodeErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let DcodeErr::AgentLimitReached { max_threads } = err else {
        panic!("expected DcodeErr::AgentLimitReached");
    };
...
async fn multi_agent_v2_completion_sends_inter_agent_message_to_direct_parent() {
    ...
    let tester_turn = tester_thread.dcode.session.new_default_turn().await;
    tester_thread
        .dcode
        .session
        .send_event(
