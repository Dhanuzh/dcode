...
use dcode_state::DirectionalThreadSpawnEdgeStatus;
...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> DcodeThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    async fn resume_single_agent_from_rollout(
        ...
        let inherited_exec_policy = self
            ...
            .await;
        let rollout_path =
            match find_thread_path_by_id_str(config.dcode_home.as_path(), &thread_id.to_string())
                .await?
            {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> DcodeResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        result: DcodeResult<String>,
    ) -> DcodeResult<String> {
        if matches!(result, Err(DcodeErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> DcodeResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread.dcode.session.ensure_rollout_materialized().await;
            thread.dcode.session.flush_rollout().await;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> DcodeResult<String> {
        let state = self.upgrade()?;
    ...
    async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> DcodeResult<String> {
        let descendant_ids = self.live_thread_spawn_descendants(agent_id).await?;
        let result = self.shutdown_live_agent(agent_id).await;
        for descendant_id in descendant_ids {
            match self.shutdown_live_agent(descendant_id).await {
                Ok(_) | Err(DcodeErr::ThreadNotFound(_)) | Err(DcodeErr::InternalAgentDied) => {}
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        let agent_path = current_agent_path
            ...
            .map_err(DcodeErr::UnsupportedOperation)?;
        if agent_path.is_root() {
            return Err(DcodeErr::UnsupportedOperation(
        ...
        Err(DcodeErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> DcodeResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> DcodeResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> DcodeResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| DcodeErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_shell_snapshot_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        parent_thread.dcode.session.user_shell().shell_snapshot()
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        let parent_config = parent_thread.dcode.session.get_config().await;
        if !crate::exec_policy::child_uses_parent_exec_policy(&parent_config, child_config) {
            return None;
        }

        Some(Arc::clone(
            &parent_thread.dcode.session.services.exec_policy,
        ))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> DcodeResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> DcodeResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
