...
use dcode_protocol::permissions::FileSystemSandboxKind;
...
use dcode_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: dcode_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: dcode_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let exec_req = manager
        .transform(crate::sandboxing::SandboxTransformRequest {
            ...
            dcode_linux_sandbox_exe: dcode_linux_sandbox_exe.as_ref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
        .map_err(DcodeErr::from)?;
    Ok(exec_req)
}
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        dcode_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    ...
    if let Some(metrics) = dcode_otel::metrics::global() {
        let _ = metrics.counter(
            "dcode.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_dcode_home;
    use dcode_protocol::config_types::WindowsSandboxLevel;
    use dcode_windows_sandbox::run_windows_sandbox_capture;
    use dcode_windows_sandbox::run_windows_sandbox_capture_elevated;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        DcodeErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let dcode_home = find_dcode_home().map_err(|err| {
        DcodeErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve dcode_home: {err}"
        )))
    })?;
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(DcodeErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, DcodeErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(DcodeErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(DcodeErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(DcodeErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
pub(crate) mod errors {
    ...
    impl From<SandboxTransformError> for DcodeErr {
        fn from(err: SandboxTransformError) -> Self {
            match err {
                ...
                SandboxTransformError::SeatbeltUnavailable => DcodeErr::UnsupportedOperation(
...
async fn exec(
    ...
    if sandbox == SandboxType::WindowsRestrictedToken {
        ...
        if let Some(reason) = support.unsupported_reason {
            return Err(DcodeErr::Io(io::Error::other(reason)));
        }
