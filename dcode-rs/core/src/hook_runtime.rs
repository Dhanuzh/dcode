...
use dcode_hooks::PreToolUseOutcome;
use dcode_hooks::PreToolUseRequest;
use dcode_hooks::SessionStartOutcome;
use dcode_hooks::UserPromptSubmitOutcome;
use dcode_hooks::UserPromptSubmitRequest;
...
pub(crate) async fn run_pending_session_start_hooks(
    ...
    let request = dcode_hooks::SessionStartRequest {
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    dcode_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
