...
use dcode_otel::sanitize_metric_tag_value;
...
pub fn sandbox_setup_is_complete(dcode_home: &Path) -> bool {
    dcode_windows_sandbox::sandbox_setup_is_complete(dcode_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_dcode_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = dcode_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = dcode_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if dcode_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            dcode_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
        "dcode.windows_sandbox.elevated_setup_canceled"
    } else {
        "dcode.windows_sandbox.elevated_setup_failure"
    }
}
...
pub fn run_elevated_setup(
    ...
    dcode_windows_sandbox::run_elevated_setup(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
pub fn run_legacy_setup_preflight(
    ...
    dcode_windows_sandbox::run_windows_sandbox_legacy_preflight(
        policy,
        policy_cwd,
        dcode_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    dcode_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let dcode_home = request.dcode_home;
    let active_profile = request.active_profile;
    let setup_dcode_home = dcode_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_dcode_home.as_path()) {
                    run_elevated_setup(
                        &policy,
                        policy_cwd.as_path(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_dcode_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(dcode_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let Some(metrics) = dcode_otel::metrics::global() else {
        return;
    };
    let mode_tag = windows_sandbox_setup_mode_tag(mode);
    let _ = metrics.record_duration(
        "dcode.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "dcode.windows_sandbox.setup_success",
        /*inc*/ 1,
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "dcode.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("originator", originator_tag), ("mode", mode_tag)],
    );
