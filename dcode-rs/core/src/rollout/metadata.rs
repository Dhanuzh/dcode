...
use dcode_state::BackfillState;
use dcode_state::BackfillStats;
use dcode_state::BackfillStatus;
use dcode_state::DB_ERROR_METRIC;
use dcode_state::DB_METRIC_BACKFILL;
use dcode_state::DB_METRIC_BACKFILL_DURATION_MS;
use dcode_state::ExtractionOutcome;
use dcode_state::ThreadMetadataBuilder;
use dcode_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(runtime: &dcode_state::StateRuntime, config: &Config) {
    let metric_client = dcode_otel::metrics::global();
    ...
    let sessions_root = config.dcode_home.join(rollout::SESSIONS_SUBDIR);
    let archived_root = config.dcode_home.join(rollout::ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.dcode_home.as_path(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(dcode_home: &Path, path: &Path) -> String {
    path.strip_prefix(dcode_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
