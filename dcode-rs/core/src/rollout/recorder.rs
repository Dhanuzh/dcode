//! Persist Dcode session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use dcode_protocol::protocol::ResumedHistory;
...
///
/// ```ignore
/// $ jq -C . ~/.dcode/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.dcode/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Dcode home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let dcode_home = config.dcode_home.as_path();
        ...
        let fs_page = if archived {
            let root = dcode_home.join(ARCHIVED_SESSIONS_SUBDIR);
            get_threads_in_root(
                root,
                fs_page_size,
                cursor,
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.dcode/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.dcode_home.clone();
...
async fn sync_thread_state_after_write(
    ...
    if new_thread_memory_mode.is_some()
        || items
            .iter()
            .any(dcode_state::rollout_item_affects_thread_metadata)
    {
...
impl From<dcode_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: dcode_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
            .map(|item| ThreadItem {
