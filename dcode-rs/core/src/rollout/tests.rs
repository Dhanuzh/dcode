...
async fn insert_state_db_thread(
    ...
    let runtime = dcode_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
...
async fn find_thread_path_repairs_missing_db_row_after_filesystem_fallback() {
    ...
    let _runtime = dcode_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
        .expect("state db should initialize");
    _runtime
        .mark_backfill_complete(None)
