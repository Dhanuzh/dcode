...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let dcode_home = dir.path().to_path_buf();
    ...
    let runtime = dcode_state::StateRuntime::init(dcode_home.clone(), "test-provider".to_string())
        .await
        .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(dcode_home.as_path(), first_path.as_path());
    ...
    config.dcode_home = dcode_home.clone();
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let dcode_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = dcode_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = dcode_home.join("sessions");
