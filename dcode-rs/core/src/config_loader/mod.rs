...
use dcode_config::ConfigRequirementsWithSources;
...
pub use dcode_config::AppRequirementToml;
pub use dcode_config::AppsRequirementsToml;
pub use dcode_config::CloudRequirementsLoadError;
pub use dcode_config::CloudRequirementsLoadErrorCode;
pub use dcode_config::CloudRequirementsLoader;
pub use dcode_config::ConfigError;
pub use dcode_config::ConfigLayerEntry;
pub use dcode_config::ConfigLayerStack;
pub use dcode_config::ConfigLayerStackOrdering;
pub use dcode_config::ConfigLoadError;
pub use dcode_config::ConfigRequirements;
pub use dcode_config::ConfigRequirementsToml;
pub use dcode_config::ConstrainedWithSource;
pub use dcode_config::FeatureRequirementsToml;
pub use dcode_config::LoaderOverrides;
pub use dcode_config::McpServerIdentity;
pub use dcode_config::McpServerRequirement;
pub use dcode_config::NetworkConstraints;
pub use dcode_config::NetworkRequirementsToml;
pub use dcode_config::RequirementSource;
pub use dcode_config::ResidencyRequirement;
pub use dcode_config::SandboxModeRequirement;
pub use dcode_config::Sourced;
pub use dcode_config::TextPosition;
pub use dcode_config::TextRange;
pub use dcode_config::WebSearchModeRequirement;
pub(crate) use dcode_config::build_cli_overrides_layer;
pub(crate) use dcode_config::config_error_from_toml;
pub use dcode_config::format_config_error;
pub use dcode_config::format_config_error_with_source;
pub(crate) use dcode_config::io_error_from_config_error;
pub use dcode_config::merge_toml_values;
#[cfg(test)]
pub(crate) use dcode_config::version_for_toml;

/// On Unix systems, load default settings from this file path, if present.
/// Note that /etc/dcode/ is treated as a "config folder," so subfolders such
/// as skills/ and rules/ will also be honored.
pub const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/dcode/config.toml";
...
pub(crate) async fn first_layer_config_error(layers: &ConfigLayerStack) -> Option<ConfigError> {
    dcode_config::first_layer_config_error::<ConfigToml>(layers, CONFIG_TOML_FILE).await
}

pub(crate) async fn first_layer_config_error_from_entries(
    layers: &[ConfigLayerEntry],
) -> Option<ConfigError> {
    dcode_config::first_layer_config_error_from_entries::<ConfigToml>(layers, CONFIG_TOML_FILE)
        .await
}
...
/// - system    `/etc/dcode/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Dcode\requirements.toml` (Windows)
///
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/dcode/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Dcode\config.toml` (Windows)
/// - user      `${CODEX_HOME}/config.toml`
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.dcode/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.dcode/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/dcode/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers = layer_io::load_config_layers_internal(dcode_home, overrides).await?;
    ...
    // user.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, dcode_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/dcode/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_dcode_system_dir() -> PathBuf {
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_dcode_folder: dot_dcode_folder.clone(),
    };
...
async fn load_project_layers(
    ...
    let dcode_home_abs = AbsolutePathBuf::from_absolute_path(dcode_home)?;
    let dcode_home_normalized =
        normalize_path(dcode_home_abs.as_path()).unwrap_or_else(|_| dcode_home_abs.to_path_buf());
    let mut dirs = cwd
        .as_path()
        .ancestors()
        .scan(false, |done, a| {
    ...
    for dir in dirs {
        let dot_dcode = dir.join(".dcode");
        if !tokio::fs::metadata(&dot_dcode)
            .await
        ...
        let dot_dcode_abs = AbsolutePathBuf::from_absolute_path(&dot_dcode)?;
        let dot_dcode_normalized =
            normalize_path(dot_dcode_abs.as_path()).unwrap_or_else(|_| dot_dcode_abs.to_path_buf());
        if dot_dcode_abs == dcode_home_abs || dot_dcode_normalized == dcode_home_normalized {
            continue;
        }
        let config_file = dot_dcode_abs.join(CONFIG_TOML_FILE)?;
        match tokio::fs::read_to_string(&config_file).await {
            Ok(contents) => {
                ...
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_dcode_abs.as_path())?;
...
/// from a file like `/etc/dcode/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Dcode to function correctly.
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Dcode").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Dcode").join("config.toml"))
        );
    }
}
