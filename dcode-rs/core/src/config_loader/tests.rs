...
async fn make_config_for_test(
    ...
    tokio::fs::write(
        dcode_home.join(CONFIG_TOML_FILE),
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .dcode_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = dcode_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        dcode_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
fn schema_error_points_to_feature_value() {
    ...
    let _guard = dcode_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = dcode_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let dcode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&dcode_home).await?;
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                dcode_config::CloudRequirementsLoadErrorCode::RequestFailed,
                None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".dcode")).await?;
    tokio::fs::create_dir_all(project_root.join(".dcode")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".dcode").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".dcode").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let dcode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&dcode_home).await?;
    make_config_for_test(&dcode_home, &project_root, TrustLevel::Trusted, None).await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            super::ConfigLayerSource::Project { dot_dcode_folder } => Some(dot_dcode_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".dcode").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".dcode").as_path()
    );
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_dcode_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".dcode").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".dcode").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".dcode").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".dcode").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let dcode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&dcode_home).await?;
    tokio::fs::write(dcode_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_dcode_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_dcode_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".dcode"))?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            raw_toml: None,
            version: version_for_toml(&TomlValue::Table(toml::map::Map::new())),
...
#[tokio::test]
async fn dcode_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let dcode_home = home_dir.join(".dcode");
    tokio::fs::create_dir_all(&dcode_home).await?;
    tokio::fs::write(dcode_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn dcode_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_dcode = project_root.join(".dcode");
    let nested_dot_dcode = nested.join(".dcode");

    tokio::fs::create_dir_all(&nested_dot_dcode).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(nested_dot_dcode.join(CONFIG_TOML_FILE), "foo = \"child\"\n").await?;

    tokio::fs::create_dir_all(&project_dot_dcode).await?;
    make_config_for_test(&project_dot_dcode, &project_root, TrustLevel::Trusted, None).await?;
    let user_config_path = project_dot_dcode.join(CONFIG_TOML_FILE);
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_dcode_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_dcode)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let dcode_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&dcode_home_untrusted).await?;
    make_config_for_test(
        &dcode_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        None,
    )
    .await?;
    let untrusted_config_path = dcode_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let dcode_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&dcode_home_unknown).await?;
    tokio::fs::write(
        dcode_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;
...
-> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let dot_dcode = project_root.join(".dcode");
    let dcode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nested).await?;
    tokio::fs::create_dir_all(&dot_dcode).await?;
    tokio::fs::create_dir_all(&dcode_home).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(
        dot_dcode.join(CONFIG_TOML_FILE),
        r#"
[mcp_servers.sentry]
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".dcode")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".dcode").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let dcode_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&dcode_home).await?;
        let config_path = dcode_home.join(CONFIG_TOML_FILE);

        if let Some(trust_level) = trust_level {
            make_config_for_test(&dcode_home, &project_root, trust_level, None).await?;
...
mod requirements_exec_policy_tests {
    ...
    use dcode_config::RequirementsExecPolicyDecisionToml;
    use dcode_config::RequirementsExecPolicyParseError;
    use dcode_config::RequirementsExecPolicyPatternTokenToml;
    use dcode_config::RequirementsExecPolicyPrefixRuleToml;
    use dcode_config::RequirementsExecPolicyToml;
    use dcode_execpolicy::Decision;
    use dcode_execpolicy::Evaluation;
    use dcode_execpolicy::RuleMatch;
    use dcode_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_dcode_folder_with_requirements(
        dot_dcode_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_dcode_folder = AbsolutePathBuf::from_absolute_path(dot_dcode_folder)
            .expect("absolute dot_dcode_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_dcode_folder_with_requirements(temp_dir.path(), requirements);
