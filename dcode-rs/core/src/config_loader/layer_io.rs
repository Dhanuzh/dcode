...
use dcode_config::config_error_from_toml;
use dcode_config::io_error_from_config_error;
...
#[cfg(unix)]
const CODEX_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/dcode/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/dcode/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(dcode_home)),
    )?;
...
pub(super) fn managed_config_default_path(dcode_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = dcode_home;
    ...
    #[cfg(not(unix))]
    {
        dcode_home.join("managed_config.toml")
    }
}
