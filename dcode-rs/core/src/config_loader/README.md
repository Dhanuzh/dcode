# `dcode-core` config loader

This module is the canonical place to **load and describe Dcode configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `dcode_core::config_loader`:

- `load_config_layers_state(dcode_home, cwd_opt, cli_overrides, overrides, cloud_requirements) -> ConfigLayerStack`
- `ConfigLayerStack`
...
```rust
use dcode_core::config_loader::{load_config_layers_state, LoaderOverrides};
