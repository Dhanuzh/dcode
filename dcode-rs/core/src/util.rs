...
/// `dcode_feedback::DcodeFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// dcode_core::feedback_tags!(model = "gpt-5", cached = true);
/// dcode_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
...
pub(crate) fn emit_feedback_request_tags_with_auth_env(
    ...
    feedback_tags!(
        ...
        auth_env_dcode_api_key_present = auth_env.dcode_api_key_env_present,
        auth_env_dcode_api_key_enabled = auth_env.dcode_api_key_env_enabled,
