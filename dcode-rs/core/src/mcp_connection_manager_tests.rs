use super::*;
use dcode_protocol::protocol::GranularApprovalConfig;
...
fn create_dcode_apps_tools_cache_context(
    dcode_home: PathBuf,
    account_id: Option<&str>,
    chatgpt_user_id: Option<&str>,
) -> DcodeAppsToolsCacheContext {
    DcodeAppsToolsCacheContext {
        dcode_home,
...
#[test]
fn dcode_apps_tools_cache_is_overwritten_by_last_write() {
    let dcode_home = tempdir().expect("tempdir");
    let cache_context = create_dcode_apps_tools_cache_context(
    ...
    write_cached_dcode_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 =
        read_cached_dcode_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].tool_name, "one");

    write_cached_dcode_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 =
        read_cached_dcode_apps_tools(&cache_context).expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].tool_name, "two");
}

#[test]
fn dcode_apps_tools_cache_is_scoped_per_user() {
    let dcode_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_dcode_apps_tools_cache_context(
        dcode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_dcode_apps_tools_cache_context(
        dcode_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    ...
    write_cached_dcode_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_dcode_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_dcode_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_dcode_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn dcode_apps_tools_cache_filters_disallowed_connectors() {
    let dcode_home = tempdir().expect("tempdir");
    let cache_context = create_dcode_apps_tools_cache_context(
        dcode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached = read_cached_dcode_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn dcode_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let dcode_home = tempdir().expect("tempdir");
    let cache_context = create_dcode_apps_tools_cache_context(
        dcode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    assert!(read_cached_dcode_apps_tools(&cache_context).is_none());
}

#[test]
fn dcode_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let dcode_home = tempdir().expect("tempdir");
    let cache_context = create_dcode_apps_tools_cache_context(
        dcode_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_dcode_apps_tools_loads_from_disk_cache() {
    let dcode_home = tempdir().expect("tempdir");
    let cache_context = create_dcode_apps_tools_cache_context(
        dcode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_dcode_apps_tools(&cache_context, &cached_tools);
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        .get("mcp__dcode_apps__calendar_create_event")
...
#[test]
fn elicitation_capability_enabled_only_for_dcode_apps() {
    let dcode_apps_capability = elicitation_capability_for_server(CODEX_APPS_MCP_SERVER_NAME);
    assert!(matches!(
        dcode_apps_capability,
