...
use dcode_otel::metrics::names::TURN_TTFM_DURATION_METRIC;
use dcode_otel::metrics::names::TURN_TTFT_DURATION_METRIC;
    ...
    match item {
        ...
        ResponseItem::Reasoning {
            ...
            summary.iter().any(|entry| match entry {
                dcode_protocol::models::ReasoningItemReasoningSummary::SummaryText { text } => {
                    !text.is_empty()
                }
            }) || content.as_ref().is_some_and(|entries| {
                entries.iter().any(|entry| match entry {
                    dcode_protocol::models::ReasoningItemContent::ReasoningText { text }
                    | dcode_protocol::models::ReasoningItemContent::Text { text } => {
                        !text.is_empty()
                    }
                })
            })
