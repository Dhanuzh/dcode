...
use crate::dcode::DcodeSpawnArgs;
use crate::dcode::DcodeSpawnOk;
use crate::dcode::INITIAL_SUBMIT_ID;
use crate::dcode_thread::DcodeThread;
...
use dcode_app_server_protocol::ThreadHistoryBuilder;
...
struct TempDcodeHomeGuard {
    path: PathBuf,
}

impl Drop for TempDcodeHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

fn build_file_watcher(dcode_home: PathBuf, skills_manager: Arc<SkillsManager>) -> Arc<FileWatcher> {
    ...
    let file_watcher = match FileWatcher::new(dcode_home) {
...
/// Represents a newly created Dcode thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<DcodeThread>,
...
pub struct ThreadManager {
    ...
    _test_dcode_home_guard: Option<TempDcodeHomeGuard>,
}
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<DcodeThread>>>>,
...
impl ThreadManager {
    pub fn new(
        ...
        // Use the user's configured model provider for model fetching so that
        // custom providers (e.g. GitHub Copilot) show their own model list.
        // Fall back to the OpenAI provider for standard API-key auth.
        let openai_models_provider = config.model_provider.clone();
        ...
        let file_watcher = build_file_watcher(dcode_home.clone(), Arc::clone(&skills_manager));
        Self {
            ...
            _test_dcode_home_guard: None,
        }
    }

    /// Construct with a dummy AuthManager containing the provided DcodeAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let dcode_home = std::env::temp_dir().join(format!(
            "dcode-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dcode_home)
            .unwrap_or_else(|err| panic!("temp dcode home dir create failed: {err}"));
        let mut manager =
            Self::with_models_provider_and_home_for_tests(auth, provider, dcode_home.clone());
        manager._test_dcode_home_guard = Some(TempDcodeHomeGuard { path: dcode_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided DcodeAuth and dcode home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> DcodeResult<Arc<DcodeThread>> {
        self.state.get_thread(thread_id).await
    }

    pub async fn start_thread(&self, config: Config) -> DcodeResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<dcode_protocol::dynamic_tools::DynamicToolSpec>,
        persist_extended_history: bool,
    ) -> DcodeResult<NewThread> {
    ...
    /// as `Arc<DcodeThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<DcodeThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> DcodeResult<Arc<DcodeThread>> {
        let threads = self.threads.read().await;
        threads
            .get(&thread_id)
            .cloned()
            .ok_or_else(|| DcodeErr::ThreadNotFound(thread_id))
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> DcodeResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<DcodeThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        .await?;
        self.finalize_thread_spawn(dcode, thread_id, watch_registration)
            .await
    }
    ...
    async fn finalize_thread_spawn(
        ...
        let event = dcode.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(DcodeErr::SessionConfiguredNotFirstEvent);
            }
        };

        let thread = Arc::new(DcodeThread::new(
            dcode,
