...
//!
//! The [`McpConnectionManager`] owns one [`dcode_rmcp_client::RmcpClient`] per
...
use dcode_async_utils::CancelErr;
use dcode_async_utils::OrCancelExt;
use dcode_config::Constrained;
use dcode_protocol::approvals::ElicitationRequest;
...
use dcode_protocol::protocol::McpStartupFailure;
...
use dcode_rmcp_client::RmcpClient;
use dcode_rmcp_client::SendElicitation;
...
use crate::dcode::INITIAL_SUBMIT_ID;
...
const CODEX_APPS_TOOLS_CACHE_DIR: &str = "cache/dcode_apps_tools";
const MCP_TOOLS_LIST_DURATION_METRIC: &str = "dcode.mcp.tools.list.duration_ms";
const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str = "dcode.mcp.tools.fetch_uncached.duration_ms";
const MCP_TOOLS_CACHE_WRITE_DURATION_METRIC: &str = "dcode.mcp.tools.cache_write.duration_ms";
...
pub(crate) fn dcode_apps_tools_cache_key(
    auth: Option<&crate::DcodeAuth>,
) -> DcodeAppsToolsCacheKey {
...
pub(crate) struct DcodeAppsToolsCacheKey {
...
#[derive(Clone)]
struct DcodeAppsToolsCacheContext {
    dcode_home: PathBuf,
    user_key: DcodeAppsToolsCacheKey,
}

impl DcodeAppsToolsCacheContext {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct DcodeAppsToolsDiskCache {
    schema_version: u8,
    tools: Vec<ToolInfo>,
}

enum CachedDcodeAppsToolsLoad {
    Hit(Vec<ToolInfo>),
    Missing,
    Invalid,
}
...
struct ManagedClient {
    ...
    dcode_apps_tools_cache_context: Option<DcodeAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.dcode_apps_tools_cache_context.as_ref()
            && let CachedDcodeAppsToolsLoad::Hit(tools) =
                load_cached_dcode_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.dcode_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    fn new(
        ...
        let startup_snapshot = load_startup_cached_dcode_apps_tools_snapshot(
            &server_name,
            dcode_apps_tools_cache_context.as_ref(),
        )
            ...
            let outcome = async {
                ...
                match start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        dcode_apps_tools_cache_context,
                    },
                )
...
pub const MCP_SANDBOX_STATE_CAPABILITY: &str = "dcode/sandbox-state";

/// Custom MCP request to push sandbox state updates.
/// When used, the `params` field of the notification is [`SandboxState`].
pub const MCP_SANDBOX_STATE_METHOD: &str = "dcode/sandbox-state/update";
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        dcode_home: PathBuf,
        dcode_apps_tools_cache_key: DcodeAppsToolsCacheKey,
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            .await;
            let dcode_apps_tools_cache_context = if server_name == CODEX_APPS_MCP_SERVER_NAME {
                Some(DcodeAppsToolsCacheContext {
                    dcode_home: dcode_home.clone(),
                    user_key: dcode_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
    ...
    /// Force-refresh dcode apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_dcode_apps_tools_cache(&self) -> Result<HashMap<String, ToolInfo>> {
        ...
        write_cached_dcode_apps_tools_if_needed(
            CODEX_APPS_MCP_SERVER_NAME,
            managed_client.dcode_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
            MCP_TOOLS_LIST_DURATION_METRIC,
...
pub(crate) fn filter_non_dcode_apps_mcp_tools_only(
...
fn normalize_dcode_apps_tool_title(
    server_name: &str,
...
fn normalize_dcode_apps_tool_name(
    server_name: &str,
    tool_name: &str,
...
fn normalize_dcode_apps_namespace(server_name: &str, connector_name: Option<&str>) -> String {
    if server_name != CODEX_APPS_MCP_SERVER_NAME {
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "dcode-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Dcode".into()),
            description: None,
            icons: None,
            website_url: None,
        },
...
fn write_cached_dcode_apps_tools_if_needed(
    server_name: &str,
    cache_context: Option<&DcodeAppsToolsCacheContext>,
    tools: &[ToolInfo],
) {
    if server_name != CODEX_APPS_MCP_SERVER_NAME {
        return;
    ...
    if let Some(cache_context) = cache_context {
        ...
        write_cached_dcode_apps_tools(cache_context, tools);
        emit_duration(
...
fn load_startup_cached_dcode_apps_tools_snapshot(
    server_name: &str,
    cache_context: Option<&DcodeAppsToolsCacheContext>,
) -> Option<Vec<ToolInfo>> {
    if server_name != CODEX_APPS_MCP_SERVER_NAME {
    ...
    match load_cached_dcode_apps_tools(cache_context) {
        CachedDcodeAppsToolsLoad::Hit(tools) => Some(tools),
        CachedDcodeAppsToolsLoad::Missing | CachedDcodeAppsToolsLoad::Invalid => None,
    }
}

#[cfg(test)]
fn read_cached_dcode_apps_tools(
    cache_context: &DcodeAppsToolsCacheContext,
) -> Option<Vec<ToolInfo>> {
    match load_cached_dcode_apps_tools(cache_context) {
        CachedDcodeAppsToolsLoad::Hit(tools) => Some(tools),
        CachedDcodeAppsToolsLoad::Missing | CachedDcodeAppsToolsLoad::Invalid => None,
...
fn load_cached_dcode_apps_tools(
    cache_context: &DcodeAppsToolsCacheContext,
) -> CachedDcodeAppsToolsLoad {
    let cache_path = cache_context.cache_path();
    let bytes = match std::fs::read(cache_path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return CachedDcodeAppsToolsLoad::Missing;
        }
        Err(_) => return CachedDcodeAppsToolsLoad::Invalid,
    };
    let cache: DcodeAppsToolsDiskCache = match serde_json::from_slice(&bytes) {
        Ok(cache) => cache,
        Err(_) => return CachedDcodeAppsToolsLoad::Invalid,
    };
    if cache.schema_version != CODEX_APPS_TOOLS_CACHE_SCHEMA_VERSION {
        return CachedDcodeAppsToolsLoad::Invalid;
    }
    CachedDcodeAppsToolsLoad::Hit(filter_disallowed_dcode_apps_tools(cache.tools))
}

fn write_cached_dcode_apps_tools(cache_context: &DcodeAppsToolsCacheContext, tools: &[ToolInfo]) {
    let cache_path = cache_context.cache_path();
    ...
    let tools = filter_disallowed_dcode_apps_tools(tools.to_vec());
    let Ok(bytes) = serde_json::to_vec_pretty(&DcodeAppsToolsDiskCache {
...
fn filter_disallowed_dcode_apps_tools(tools: Vec<ToolInfo>) -> Vec<ToolInfo> {
    tools
        .into_iter()
        .filter(|tool| {
            tool.connector_id
...
fn emit_duration(metric: &str, duration: Duration, tags: &[(&str, &str)]) {
    if let Some(metrics) = dcode_otel::metrics::global() {
...
async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .into_iter()
        .map(|tool| {
            let tool_name = normalize_dcode_apps_tool_name(
                server_name,
                &tool.tool.name,
            ...
            let tool_namespace =
                normalize_dcode_apps_namespace(server_name, tool.connector_name.as_deref());
            let connector_name = tool.connector_name;
            let connector_description = tool.connector_description;
            let mut tool_def = tool.tool;
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_dcode_apps_tool_title(server_name, connector_name.as_deref(), title);
        ...
        .collect();
    if server_name == CODEX_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_dcode_apps_tools(tools));
    }
    Ok(tools)
}
