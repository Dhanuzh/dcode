...
use chrono::DateTime;
use chrono::Utc;
use dcode_api::AuthProvider as ApiAuthProvider;
use dcode_api::TransportError;
use dcode_api::error::ApiError;
use dcode_api::rate_limits::parse_promo_message;
use dcode_api::rate_limits::parse_rate_limit_for_limit;
use http::HeaderMap;
...
pub(crate) fn map_api_error(err: ApiError) -> DcodeErr {
    match err {
        ApiError::ContextWindowExceeded => DcodeErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => DcodeErr::QuotaExceeded,
        ApiError::UsageNotIncluded => DcodeErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => DcodeErr::Stream(message, delay),
        ApiError::Stream(msg) => DcodeErr::Stream(msg, None),
        ApiError::ServerOverloaded => DcodeErr::ServerOverloaded,
        ApiError::Api { status, message } => DcodeErr::UnexpectedStatus(UnexpectedResponseError {
            status,
            body: message,
            url: None,
            cf_ray: None,
        ...
        ApiError::InvalidRequest { message } => DcodeErr::InvalidRequest(message),
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return DcodeErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                        ...
                        .contains("The image data you provided does not represent a valid image")
                    {
                        ...
                        DcodeErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return DcodeErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return DcodeErr::UsageNotIncluded;
                        }
                    }

                    DcodeErr::RetryLimit(RetryLimitReachedError {
                        status,
                        request_id: extract_request_tracking_id(headers.as_ref()),
                    })
                } else {
                    DcodeErr::UnexpectedStatus(UnexpectedResponseError {
                        status,
                        body: body_text,
                        url,
            ...
            TransportError::RetryLimit => DcodeErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => DcodeErr::Timeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                DcodeErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => DcodeErr::Stream(msg, None),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-dcode-active-limit";
...
pub(crate) fn auth_provider_from_auth(
    ...
    match provider.api_key() {
        ...
        Err(crate::error::DcodeErr::EnvVar(_)) => {}
        Ok(None) => {}
        Err(e) => return Err(e),
    }
