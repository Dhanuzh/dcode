...
use dcode_utils_stream_parser::strip_proposed_plan_blocks;
use futures::Future;
use tracing::debug;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<dcode_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                dcode_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            DcodeErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(dcode_home, session_id, call_id);
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(ctx.sess.as_ref(), item.clone()).await {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(DcodeErr::Fatal(message));
        }
    }
        ...
        | ResponseItem::ImageGenerationCall { .. } => {
            ...
            if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                ...
                agent_message.content =
                    vec![dcode_protocol::items::AgentMessageContent::Text { text: stripped }];
                ...
                .await
                {
                    Ok(path) => {
                        ...
                        let image_output_dir = image_output_path
                            .parent()
                            .unwrap_or(turn_context.config.dcode_home.as_path());
