...
use dcode_otel::config::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only Dcode-owned events via OTEL.
/// Keeps events that originated from dcode_otel module
pub fn dcode_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("dcode_otel")
}
