...
pub use dcode_app_server_protocol::AppBranding;
pub use dcode_app_server_protocol::AppInfo;
pub use dcode_app_server_protocol::AppMetadata;
...
pub use dcode_connectors::CONNECTORS_CACHE_TTL;
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_dcode_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
    ...
    let dcode_apps_ready = if refreshed_tools_succeeded {
        true
    ...
    if dcode_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
    let accessible_connectors =
...
fn tool_suggest_connector_ids(config: &Config) -> HashSet<String> {
    let mut connector_ids = PluginsManager::new(config.dcode_home.clone())
...
async fn list_directory_connectors_for_tool_suggest_with_auth(
    ...
    dcode_connectors::list_all_connectors_with_options(
        cache_key,
        is_workspace_account,
        /*force_refetch*/ false,
        |path| {
...
pub(crate) fn dcode_app_tool_is_enabled(
    config: &Config,
...
fn plugin_app_to_app_info(connector_id: AppConnectorId) -> AppInfo {
    ...
    // connector_name values from dcode_apps tool discovery.
