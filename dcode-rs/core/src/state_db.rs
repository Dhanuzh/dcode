...
pub type StateDbHandle = Arc<dcode_state::StateRuntime>;
...
pub(crate) async fn init(config: &Config) -> Option<StateDbHandle> {
    let runtime = match dcode_state::StateRuntime::init(
    ...
    let backfill_state = match runtime.get_backfill_state().await {
        ...
        Err(err) => {
            warn!(
                ...
                config.dcode_home.display()
            );
            return None;
        }
    };
    if backfill_state.status != dcode_state::BackfillStatus::Complete {
...
pub async fn get_state_db(config: &Config) -> Option<StateDbHandle> {
    let state_path = dcode_state::state_db_path(config.sqlite_home.as_path());
...
pub async fn open_if_present(dcode_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = dcode_state::state_db_path(dcode_home);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime =
        dcode_state::StateRuntime::init(dcode_home.to_path_buf(), default_provider.to_string())
            .await
            .ok()?;
    require_backfill_complete(runtime, dcode_home).await
}
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == dcode_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<dcode_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&dcode_state::StateRuntime>,
    dcode_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.dcode_home() != dcode_home {
        warn!(
            "state db dcode_home mismatch: expected {}, got {}",
            ctx.dcode_home().display(),
            dcode_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => dcode_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => dcode_state::SortKey::UpdatedAt,
            },
...
) -> Option<dcode_state::ThreadsPage> {
    let ctx = context?;
    if ctx.dcode_home() != dcode_home {
        warn!(
            "state db dcode_home mismatch: expected {}, got {}",
