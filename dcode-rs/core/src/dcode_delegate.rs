...
use dcode_protocol::protocol::RequestUserInputEvent;
...
use crate::dcode::SUBMISSION_CHANNEL_CAPACITY;
...
#[cfg(test)]
use crate::dcode::completed_session_loop_termination;

/// Start an interactive sub-Dcode thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_dcode_thread_interactive(
    config: Config,
...
) -> Result<Dcode, DcodeErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);

    let DcodeSpawnOk { dcode, .. } = Dcode::spawn(DcodeSpawnArgs {
        config,
        auth_manager,
        models_manager,
    ...
    let dcode_for_events = Arc::clone(&dcode);
    ...
    let dcode_for_ops = Arc::clone(&dcode);
    tokio::spawn(async move {
        forward_ops(dcode_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(Dcode {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: dcode.agent_status.clone(),
        session: Arc::clone(&dcode.session),
        session_loop_termination: dcode.session_loop_termination.clone(),
    })
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_dcode_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: Arc<ModelsManager>,
    input: Vec<UserInput>,
    ...
    let io = run_dcode_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&dcode).await;
                break;
            }
            event = dcode.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&dcode, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(dcode: &Dcode) {
    let _ = dcode.submit(Op::Interrupt).await;
    let _ = dcode.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = dcode.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = dcode.submit_with_id(submission).await;
    }
}
        ...
        if let Some(files) = maybe_files {
            ...
            let patch = changes
                ...
                .map(|(path, change)| match change {
                    dcode_protocol::protocol::FileChange::Add { content } => {
                        format!("*** Add File: {}\n{}", path.display(), content)
                    }
                    dcode_protocol::protocol::FileChange::Delete { content } => {
                        format!("*** Delete File: {}\n{}", path.display(), content)
                    }
...
async fn handle_request_user_input(
    ...
    if routes_approval_to_guardian(parent_ctx)
        ...
        .await
    {
        let _ = dcode.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            dcode_protocol::request_user_input::RequestUserInputAnswer {
...
where
    F: core::future::Future<Output = dcode_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, dcode_protocol::protocol::ReviewDecision::Abort)
                .await;
