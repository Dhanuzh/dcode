...
pub use dcode_login::auth::RefreshTokenFailedError;
pub use dcode_login::auth::RefreshTokenFailedReason;
...
pub type Result<T> = std::result::Result<T, DcodeErr>;
...
pub enum DcodeErr {
    ...
    #[error(
        "Dcode ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `DcodeError::Spawn` variant.
    ...
    #[error(
        "To use Dcode with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("dcode-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for DcodeErr {
    fn from(_: CancelErr) -> Self {
        DcodeErr::TurnAborted
    }
}
...
impl DcodeErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            DcodeErr::TurnAborted
            | DcodeErr::Interrupted
            | DcodeErr::EnvVar(_)
            | DcodeErr::Fatal(_)
            | DcodeErr::UsageNotIncluded
            | DcodeErr::QuotaExceeded
            | DcodeErr::InvalidImageRequest()
            | DcodeErr::InvalidRequest(_)
            | DcodeErr::RefreshTokenFailed(_)
            | DcodeErr::UnsupportedOperation(_)
            | DcodeErr::Sandbox(_)
            | DcodeErr::LandlockSandboxExecutableNotProvided
            | DcodeErr::RetryLimit(_)
            | DcodeErr::ContextWindowExceeded
            | DcodeErr::ThreadNotFound(_)
            | DcodeErr::AgentLimitReached { .. }
            | DcodeErr::Spawn
            | DcodeErr::SessionConfiguredNotFirstEvent
            | DcodeErr::UsageLimitReached(_)
            | DcodeErr::ServerOverloaded => false,
            DcodeErr::Stream(..)
            | DcodeErr::Timeout
            | DcodeErr::UnexpectedStatus(_)
            | DcodeErr::ResponseStreamFailed(_)
            | DcodeErr::ConnectionFailed(_)
            | DcodeErr::InternalServerError
            | DcodeErr::InternalAgentDied
            | DcodeErr::Io(_)
            | DcodeErr::Json(_)
            | DcodeErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            DcodeErr::LandlockRuleset(_) | DcodeErr::LandlockPathFd(_) => false,
        }
    }
}
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("dcode")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/dcode/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Dcode (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/dcode/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
impl DcodeErr {
    /// Minimal shim so that existing `e.downcast_ref::<DcodeErr>()` checks continue to compile
    ...
    pub fn to_dcode_protocol_error(&self) -> DcodeErrorInfo {
        match self {
            DcodeErr::ContextWindowExceeded => DcodeErrorInfo::ContextWindowExceeded,
            DcodeErr::UsageLimitReached(_)
            | DcodeErr::QuotaExceeded
            | DcodeErr::UsageNotIncluded => DcodeErrorInfo::UsageLimitExceeded,
            DcodeErr::ServerOverloaded => DcodeErrorInfo::ServerOverloaded,
            DcodeErr::RetryLimit(_) => DcodeErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            DcodeErr::ConnectionFailed(_) => DcodeErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            DcodeErr::ResponseStreamFailed(_) => DcodeErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            DcodeErr::RefreshTokenFailed(_) => DcodeErrorInfo::Unauthorized,
            DcodeErr::SessionConfiguredNotFirstEvent
            | DcodeErr::InternalServerError
            | DcodeErr::InternalAgentDied => DcodeErrorInfo::InternalServerError,
            DcodeErr::UnsupportedOperation(_)
            | DcodeErr::ThreadNotFound(_)
            | DcodeErr::AgentLimitReached { .. } => DcodeErrorInfo::BadRequest,
            DcodeErr::Sandbox(_) => DcodeErrorInfo::SandboxError,
            _ => DcodeErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            dcode_error_info: Some(self.to_dcode_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            DcodeErr::RetryLimit(err) => Some(err.status),
            DcodeErr::UnexpectedStatus(err) => Some(err.status),
            DcodeErr::ConnectionFailed(err) => err.source.status(),
            DcodeErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
pub fn get_error_message_ui(e: &DcodeErr) -> String {
    let message = match e {
        DcodeErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        DcodeErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
