...
use dcode_api::endpoint::realtime_websocket::RealtimeWebsocketEvents;
use dcode_api::endpoint::realtime_websocket::RealtimeWebsocketWriter;
...
impl RealtimeConversationManager {
    ...
    ) -> DcodeResult<(Receiver<RealtimeEvent>, Arc<AtomicBool>)> {
        let previous_state = {
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> DcodeResult<()> {
        let sender = {
        ...
        let Some(sender) = sender else {
            return Err(DcodeErr::InvalidRequest(
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(DcodeErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> DcodeResult<()> {
        let sender = {
            let guard = self.state.lock().await;
        ...
        sender
            .send(text)
            .await
            .map_err(|_| DcodeErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> DcodeResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> DcodeResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> DcodeResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> DcodeResult<PreparedRealtimeConversationStart> {
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), DcodeErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(
    ...
    if let Some(api_key) = auth.and_then(DcodeAuth::api_key) {
...
fn realtime_request_headers(
    ...
    let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
        DcodeErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
    })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    dcode_error_info: DcodeErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
