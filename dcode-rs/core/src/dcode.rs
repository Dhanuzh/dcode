...
use chrono::Local;
use chrono::Utc;
use dcode_app_server_protocol::McpServerElicitationRequest;
use dcode_app_server_protocol::McpServerElicitationRequestParams;
use dcode_exec_server::Environment;
use dcode_features::FEATURES;
use dcode_features::Feature;
use dcode_features::unstable_features_warning_event;
use dcode_hooks::HookEvent;
use dcode_hooks::HookEventAfterAgent;
use dcode_hooks::HookPayload;
use dcode_hooks::HookResult;
use dcode_hooks::Hooks;
use dcode_hooks::HooksConfig;
use dcode_network_proxy::NetworkProxy;
use dcode_network_proxy::NetworkProxyAuditMetadata;
use dcode_network_proxy::normalize_host;
use dcode_otel::current_span_trace_id;
use dcode_otel::current_span_w3c_trace_context;
use dcode_otel::set_parent_from_w3c_trace_context;
use dcode_protocol::ThreadId;
use dcode_protocol::approvals::ElicitationRequestEvent;
use dcode_protocol::approvals::ExecApprovalRequestSkillMetadata;
use dcode_protocol::approvals::ExecPolicyAmendment;
use dcode_protocol::approvals::NetworkPolicyAmendment;
use dcode_protocol::approvals::NetworkPolicyRuleAction;
use dcode_protocol::config_types::ApprovalsReviewer;
use dcode_protocol::config_types::ModeKind;
use dcode_protocol::config_types::Settings;
use dcode_protocol::config_types::WebSearchMode;
use dcode_protocol::dynamic_tools::DynamicToolResponse;
use dcode_protocol::dynamic_tools::DynamicToolSpec;
use dcode_protocol::items::PlanItem;
use dcode_protocol::items::TurnItem;
use dcode_protocol::items::UserMessageItem;
...
use dcode_protocol::models::PermissionProfile;
use dcode_protocol::models::format_allow_prefixes;
...
use dcode_protocol::protocol::HasLegacyEvent;
use dcode_protocol::protocol::ItemCompletedEvent;
use dcode_protocol::protocol::ItemStartedEvent;
use dcode_protocol::protocol::RawResponseItemEvent;
...
use dcode_protocol::protocol::TurnContextItem;
use dcode_protocol::protocol::TurnContextNetworkItem;
use dcode_protocol::protocol::W3cTraceContext;
use dcode_protocol::request_permissions::PermissionGrantScope;
use dcode_protocol::request_permissions::RequestPermissionProfile;
use dcode_protocol::request_permissions::RequestPermissionsArgs;
use dcode_protocol::request_permissions::RequestPermissionsEvent;
use dcode_protocol::request_permissions::RequestPermissionsResponse;
use dcode_protocol::request_user_input::RequestUserInputArgs;
use dcode_protocol::request_user_input::RequestUserInputResponse;
use dcode_rmcp_client::ElicitationResponse;
use dcode_rmcp_client::OAuthCredentialsStoreMode;
use dcode_terminal_detection::user_agent;
use dcode_utils_stream_parser::AssistantTextChunk;
use dcode_utils_stream_parser::AssistantTextStreamParser;
use dcode_utils_stream_parser::ProposedPlanSegment;
use dcode_utils_stream_parser::extract_proposed_plan_text;
use dcode_utils_stream_parser::strip_citations;
...
use crate::dcode_thread::ThreadConfigSnapshot;
...
use crate::error::Result as DcodeResult;
#[cfg(test)]
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                dcode_error_info: Some(DcodeErrorInfo::BadRequest),
            },
            ...
            Self::ActiveTurnNotSteerable { turn_kind } => {
                ...
                ErrorEvent {
                    ...
                    dcode_error_info: Some(DcodeErrorInfo::ActiveTurnNotSteerable {
                        turn_kind: *turn_kind,
                    }),
                }
            }
...
use crate::mcp::with_dcode_apps_mcp;
use crate::mcp_connection_manager::McpConnectionManager;
use crate::mcp_connection_manager::dcode_apps_tools_cache_key;
use crate::mcp_connection_manager::filter_non_dcode_apps_mcp_tools_only;
use crate::memories;
...
use dcode_async_utils::OrCancelExt;
use dcode_otel::SessionTelemetry;
use dcode_otel::TelemetryAuthMode;
use dcode_otel::metrics::names::THREAD_STARTED_METRIC;
...
use dcode_protocol::models::DeveloperInstructions;
...
use dcode_protocol::protocol::NonSteerableTurnKind;
use dcode_protocol::user_input::UserInput;
use dcode_utils_absolute_path::AbsolutePathBuf;
use dcode_utils_readiness::Readiness;
use dcode_utils_readiness::ReadinessFlag;

/// The high-level interface to the Dcode system.
...
/// Wrapper returned by [`Dcode::spawn`] containing the spawned [`Dcode`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct DcodeSpawnOk {
    pub dcode: Dcode,
...
pub(crate) struct DcodeSpawnArgs {
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/dcode/concepts/cyber-safety";
const DIRECT_APP_TOOL_EXPOSURE_THRESHOLD: usize = 100;

impl Dcode {
    /// Spawn a new [`Dcode`] and initialize the session.
    pub(crate) async fn spawn(args: DcodeSpawnArgs) -> DcodeResult<DcodeSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if dcode_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(DcodeSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: DcodeSpawnArgs) -> DcodeResult<DcodeSpawnOk> {
        let DcodeSpawnArgs {
            mut config,
            auth_manager,
            models_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| DcodeErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };
        ...
        let persisted_tools = if dynamic_tools.is_empty() {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "dcode_spawn")
                        .await
                }
                None => None,
            }
        ...
        // Generate a unique ID for the lifetime of this Dcode session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.dcode_home)
        })?;
    ...
    pub async fn submit(&self, op: Op) -> DcodeResult<String> {
    ...
    /// Use sparingly: prefer `submit()` so Dcode is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> DcodeResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| DcodeErr::InternalAgentDied)?;
        Ok(())
    }

    pub async fn shutdown_and_wait(&self) -> DcodeResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(DcodeErr::InternalAgentDied) => {}
    ...
    pub async fn next_event(&self) -> DcodeResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
pub(crate) struct TurnContext {
    ...
    pub(crate) dcode_linux_sandbox_exe: Option<PathBuf>,
...
impl TurnContext {
    ...
    pub(crate) async fn with_model(&self, model: String, models_manager: &ModelsManager) -> Self {
        ...
        Self {
            ...
            dcode_linux_sandbox_exe: self.dcode_linux_sandbox_exe.clone(),
...
pub(crate) struct SessionConfiguration {
    ...
    cwd: PathBuf,
    /// Directory containing all Dcode state for this session.
    dcode_home: PathBuf,
...
impl SessionConfiguration {
    pub(crate) fn dcode_home(&self) -> &PathBuf {
        &self.dcode_home
    }
...
impl Session {
    /// Builds the `x-dcode-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &dcode_execpolicy::Policy,
        sandbox_policy: &SandboxPolicy,
        network_policy_decider: Option<Arc<dyn dcode_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn dcode_network_proxy::BlockedRequestObserver>>,
    ...
    pub(crate) async fn dcode_home(&self) -> PathBuf {
        let state = self.state.lock().await;
        state.session_configuration.dcode_home().clone()
    }
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            dcode_linux_sandbox_exe: per_turn_config.dcode_linux_sandbox_exe.clone(),
            tool_call_gate: Arc::new(ReadinessFlag::new()),
    ...
    async fn new(
        ...
        let config_path = config.dcode_home.join(CONFIG_TOML_FILE);
        ...
        let auth_mode = auth.map(DcodeAuth::auth_mode).map(TelemetryAuthMode::from);
        let account_id = auth.and_then(DcodeAuth::get_account_id);
        let account_email = auth.and_then(DcodeAuth::get_account_email);
        let originator = crate::default_client::originator().value;
        let terminal_type = user_agent();
        let session_model = session_configuration.collaboration_mode.model().to_string();
        let auth_env_telemetry = collect_auth_env_telemetry(
            &session_configuration.provider,
            auth_manager.dcode_api_key_env_enabled(),
        );
        ...
        let thread_name =
            match session_index::find_thread_name_by_id(&config.dcode_home, &conversation_id)
                .instrument(info_span!(
        ...
        let hooks = Hooks::new(HooksConfig {
            ...
            feature_enabled: config.features.enabled(Feature::DcodeHooks),
        ...
        let sandbox_state = SandboxState {
            sandbox_policy: session_configuration.sandbox_policy.get().clone(),
            dcode_linux_sandbox_exe: config.dcode_linux_sandbox_exe.clone(),
        ...
        let (mcp_connection_manager, cancel_token) = McpConnectionManager::new(
            ...
            sandbox_state,
            config.dcode_home.clone(),
            dcode_apps_tools_cache_key(auth),
        ...
        let session_start_source = match &initial_history {
            InitialHistory::Resumed(_) => dcode_hooks::SessionStartSource::Resume,
            InitialHistory::New | InitialHistory::Forked(_) => {
                dcode_hooks::SessionStartSource::Startup
            }
        };
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Dcode performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        match state.session_configuration.apply(&updates) {
            Ok(updated) => {
                ...
                let dcode_home = updated.dcode_home.clone();
        ...
        let (
            ...
            match state.session_configuration.clone().apply(&updates) {
                Ok(next) => {
                    ...
                    let dcode_home = next.dcode_home.clone();
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&dcode_home, amendment)
            .await?;
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        let request = match params.request {
            McpServerElicitationRequest::Form {
                ...
                dcode_protocol::approvals::ElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema,
                }
            ...
            McpServerElicitationRequest::Url {
                ...
                url,
                elicitation_id,
            } => dcode_protocol::approvals::ElicitationRequest::Url {
                meta,
                message,
                url,
                elicitation_id,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                dcode_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                dcode_protocol::mcp::RequestId::Integer(value)
            }
        };
    ...
    pub(crate) async fn record_model_warning(&self, message: impl Into<String>, ctx: &TurnContext) {
        self.services
            .session_telemetry
            .counter("dcode.model_warning", /*inc*/ 1, &[]);
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-dcode, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn build_initial_context(
        ...
        if turn_context.features.enabled(Feature::MemoryTool)
            ...
            && let Some(memory_prompt) =
                build_memory_tool_developer_instructions(&turn_context.config.dcode_home).await
        {
        ...
        if turn_context.features.enabled(Feature::DcodeGitCommit)
    ...
    pub(crate) async fn notify_stream_error(
        ...
        turn_context: &TurnContext,
        message: impl Into<String>,
        dcode_error: DcodeErr,
    ) {
        let additional_details = dcode_error.to_string();
        let dcode_error_info = DcodeErrorInfo::ResponseStreamDisconnected {
            http_status_code: dcode_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            dcode_error_info: Some(dcode_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<dcode_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let mcp_servers = with_dcode_apps_mcp(
            mcp_servers,
        ...
        let sandbox_state = SandboxState {
            ...
            dcode_linux_sandbox_exe: turn_context.dcode_linux_sandbox_exe.clone(),
        ...
        let (refreshed_manager, cancel_token) = McpConnectionManager::new(
            ...
            sandbox_state,
            config.dcode_home.clone(),
            dcode_apps_tools_cache_key(auth.as_ref()),
            tool_plugin_provenance,
        )
        .await;
        {
                ...
                Op::RealtimeConversationStart(params) => {
                        ...
                        handle_realtime_conversation_start(&sess, sub.id.clone(), params).await
                    {
                        sess.send_event_raw(Event {
                            ...
                            msg: EventMsg::Error(ErrorEvent {
                                message: err.to_string(),
                                dcode_error_info: Some(DcodeErrorInfo::Other),
                            }),
                        })
                        .await;
                    }
...
mod handlers {
    ...
    use crate::dcode::SessionSettingsUpdate;
    use crate::dcode::SteerInputError;

    use crate::dcode::spawn_review_thread;
    use crate::config::Config;
    ...
    use dcode_protocol::protocol::ReviewRequest;
    use dcode_protocol::protocol::RolloutItem;
    use dcode_protocol::protocol::SkillsListEntry;
    use dcode_protocol::protocol::ThreadNameUpdatedEvent;
    ...
    use dcode_protocol::config_types::Settings;
    use dcode_protocol::dynamic_tools::DynamicToolResponse;
    use dcode_protocol::mcp::RequestId as ProtocolRequestId;
    use dcode_protocol::user_input::UserInput;
    use dcode_rmcp_client::ElicitationAction;
    use dcode_rmcp_client::ElicitationResponse;
    use serde_json::Value;
    use std::path::PathBuf;
    use std::sync::Arc;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: dcode_protocol::approvals::ElicitationAction,
        content: Option<Value>,
        meta: Option<Value>,
    ) {
        let action = match decision {
            dcode_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            dcode_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            dcode_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| dcode_protocol::message_history::HistoryEntry {
    ...
    pub async fn drop_memories(sess: &Arc<Session>, config: &Arc<Config>, sub_id: String) {
        ...
        let memory_root = crate::memories::memory_root(&config.dcode_home);
        ...
        if num_turns == 0 {
            sess.send_event_raw(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    dcode_error_info: Some(DcodeErrorInfo::ThreadRollbackFailed),
                }),
            })
            .await;
            return;
    ...
    pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
        ...
        let dcode_home = sess.dcode_home().await;
        if let Err(e) =
            session_index::append_thread_name(&dcode_home, sess.conversation_id, &name).await
        {
            let event = Event {
                id: sub_id,
                msg: EventMsg::Error(ErrorEvent {
    ...
    pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
        ...
        sess.services
            ...
            .await;
        sess.guardian_review_session.shutdown().await;
        info!("Shutting down Dcode instance");
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        dcode_linux_sandbox_exe: parent_turn_context.dcode_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
...
pub(crate) async fn run_turn(
    ...
    // Although from the perspective of dcode.rs, TurnDiffTracker has the lifecycle of a Task which contains
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = dcode_hooks::StopRequest {
            ...
            Err(DcodeErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(DcodeErr::InvalidImageRequest()) => {
                let mut state = sess.state.lock().await;
                error_or_panic(
...
fn filter_dcode_apps_mcp_tools(
    ...
    mcp_tools
        ...
        .filter(|(_, tool)| {
            ...
            let Some(connector_id) = dcode_apps_connector_id(tool) else {
                return false;
            };
            allowed.contains(connector_id) && connectors::dcode_app_tool_is_enabled(config, tool)
        })
        .map(|(name, tool)| (name.clone(), tool.clone()))
        .collect()
}

fn dcode_apps_connector_id(tool: &crate::mcp_connection_manager::ToolInfo) -> Option<&str> {
...
async fn run_sampling_request(
    ...
    cancellation_token: CancellationToken,
) -> DcodeResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(DcodeErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(DcodeErr::ContextWindowExceeded);
            }
            Err(DcodeErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(DcodeErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                DcodeErr::Stream(_, requested_delay) => {
...
pub(crate) async fn built_tools(
    ...
    let app_tools = connectors.as_ref().map(|connectors| {
        filter_dcode_apps_mcp_tools(&mcp_tools, connectors, &turn_context.config)
    });
    ...
    if let Some(connectors) = connectors.as_ref() {
        ...
        let mut selected_mcp_tools = filter_non_dcode_apps_mcp_tools_only(&mcp_tools);
        selected_mcp_tools.extend(filter_dcode_apps_mcp_tools(
            &mcp_tools,
...
fn agent_message_text(item: &dcode_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            dcode_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: dcode_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(dcode_protocol::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, DcodeResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> DcodeResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, DcodeResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: DcodeResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(dcode_async_utils::CancelErr::Cancelled) => break Err(DcodeErr::TurnAborted),
        };

        let event = match event {
            Some(res) => res?,
            None => {
                break Err(DcodeErr::Stream(
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![dcode_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(DcodeErr::TurnAborted);
    }
