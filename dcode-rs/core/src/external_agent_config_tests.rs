...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let dcode_home = root.path().join(".dcode");
    (root, claude_home, dcode_home)
}

fn service_for_paths(claude_home: PathBuf, dcode_home: PathBuf) -> ExternalAgentConfigService {
    ExternalAgentConfigService::new_for_test(dcode_home, claude_home)
}

#[test]
fn detect_home_lists_config_skills_and_agents_md() {
    let (_root, claude_home, dcode_home) = fixture_paths();
    let agents_skills = dcode_home
        .parent()
    ...
    let items = service_for_paths(claude_home.clone(), dcode_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                dcode_home.join("config.toml").display()
            ),
            cwd: None,
        },
        ExternalAgentConfigMigrationItem {
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                dcode_home.join("AGENTS.md").display()
            ),
            cwd: None,
        },
    ];
...
fn detect_repo_lists_agents_md_for_each_cwd() {
    ...
    let items = service_for_paths(root.path().join(".claude"), root.path().join(".dcode"))
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
...
fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    fs::write(
            claude_home.join("settings.json"),
            r#"{"model":"claude","permissions":{"ask":["git push"]},"env":{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"dcode","IGNORED":null,"LIST":["a","b"],"MAP":{"x":1}},"sandbox":{"enabled":true,"network":{"allowLocalBinding":true}}}"#,
        )
        .expect("write settings");
    fs::write(
    ...
    service_for_paths(claude_home, dcode_home.clone())
        .import(vec![
            ExternalAgentConfigMigrationItem {
                item_type: ExternalAgentConfigMigrationItemType::AgentsMd,
    ...
    assert_eq!(
        fs::read_to_string(dcode_home.join("AGENTS.md")).expect("read agents"),
        "Dcode guidance"
    );

    assert_eq!(
        fs::read_to_string(dcode_home.join("config.toml")).expect("read config"),
        "sandbox_mode = \"workspace-write\"\n\n[shell_environment_policy]\ninherit = \"core\"\n\n[shell_environment_policy.set]\nCI = \"false\"\nFOO = \"bar\"\nMAX_RETRIES = \"3\"\nMY_TEAM = \"dcode\"\n"
    );
    assert_eq!(
        fs::read_to_string(agents_skills.join("skill-a").join("SKILL.md"))
            .expect("read copied skill"),
        "Use Dcode and Dcode utilities."
    );
}
...
fn import_home_skips_empty_config_migration() {
    ...
    assert!(!dcode_home.join("config.toml").exists());
}
...
fn detect_home_skips_config_when_target_already_has_supported_fields() {
    let (_root, claude_home, dcode_home) = fixture_paths();
    fs::create_dir_all(&claude_home).expect("create claude home");
    fs::create_dir_all(&dcode_home).expect("create dcode home");
    fs::write(
        claude_home.join("settings.json"),
    ...
    let items = service_for_paths(claude_home, dcode_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
fn import_repo_agents_md_rewrites_terms_and_skips_non_empty_targets() {
    ...
    service_for_paths(root.path().join(".claude"), root.path().join(".dcode"))
        .import(vec![
            ExternalAgentConfigMigrationItem {
                item_type: ExternalAgentConfigMigrationItemType::AgentsMd,
                description: String::new(),
...
fn import_skills_returns_only_new_skill_directory_count() {
    ...
    let copied_count = service_for_paths(claude_home, dcode_home)
        .import_skills(None)
