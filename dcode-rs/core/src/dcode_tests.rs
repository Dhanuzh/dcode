...
use dcode_protocol::models::FunctionCallOutputBody;
...
use dcode_execpolicy::NetworkRuleProtocol;
use dcode_execpolicy::Policy;
...
use dcode_protocol::protocol::Submission;
...
use dcode_protocol::mcp::CallToolResult as McpCallToolResult;
...
#[path = "dcode_tests_guardian.rs"]
mod guardian_tests;

use dcode_protocol::models::function_call_output_content_items_to_text;
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    crate::client::ModelClient::new(
        ...
        dcode_protocol::protocol::SessionSource::Exec,
        None,
        false,
        false,
        None,
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let dcode_home = session.dcode_home().await;
    std::fs::create_dir_all(&dcode_home).expect("create dcode home");
    let config_toml_path = dcode_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
...
fn non_app_mcp_tools_remain_visible_without_search_selection() {
    let mcp_tools = HashMap::from([
        (
            "mcp__dcode_apps__calendar_create_event".to_string(),
            make_mcp_tool(
...
#[test]
fn search_tool_selection_keeps_dcode_apps_tools_without_mentions() {
...
#[test]
fn apps_mentions_add_dcode_apps_tools_to_search_selected_set() {
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_dcode().with_config(|config| {
        config.permissions.approval_policy =
            dcode_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    let mut fork_config = initial.config.clone();
    fork_config.permissions.approval_policy =
        dcode_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "dcode_core__dcode_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            dcode_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            dcode_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_rollout_path() {
    ...
    assert_eq!(
        error_event.dcode_error_info,
        Some(DcodeErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let config = build_test_config(dcode_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(dcode_protocol::account::PlanType::Plus),
    };
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(dcode_protocol::account::PlanType::Pro),
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.dcode_error_info == Some(DcodeErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(dcode_home: &Path) -> Config {
...
async fn session_configuration_apply_preserves_split_file_system_policy_on_cwd_only_update() {
    ...
    let docs_dir =
        dcode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs_dir).expect("docs");

    session_configuration.cwd = original_cwd;
    session_configuration.sandbox_policy =
        dcode_config::Constrained::allow_any(SandboxPolicy::WorkspaceWrite {
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let dcode_home = parent_config.dcode_home.clone();
    let skill_dir = dcode_home.join("skills").join("demo");
    ...
    let role_path = dcode_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    let dcode_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(dcode_home.path()).await;
    config
        .features
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.dcode_home.clone()));
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        dcode_exec_server::Environment::create(None)
            .await
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            dcode_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(dcode_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = dcode_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(dcode_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    turn_context.as_ref(),
                    call_id,
                    dcode_protocol::request_permissions::RequestPermissionsArgs {
...
async fn submit_with_id_captures_current_span_trace_context() {
    ...
    let _trace_test_context = install_test_tracing("dcode-core-tests");
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        dcode_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = dcode_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let waiter_1 = {
        let dcode = Arc::clone(&dcode);
        tokio::spawn(async move { dcode.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let dcode = Arc::clone(&dcode);
        tokio::spawn(async move { dcode.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_dcode)
        .await;
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let expected_saved_path = crate::stream_events_utils::image_generation_artifact_path(
        turn_context.config.dcode_home.as_path(),
