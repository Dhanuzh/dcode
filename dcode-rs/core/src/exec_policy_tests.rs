...
use dcode_config::RequirementsExecPolicy;
...
fn config_stack_for_dot_dcode_folder(dot_dcode_folder: &Path) -> ConfigLayerStack {
    let dot_dcode_folder =
        AbsolutePathBuf::from_absolute_path(dot_dcode_folder).expect("absolute dot_dcode_folder");
...
async fn test_config() -> (TempDir, Config) {
    ...
    let config = ConfigBuilder::default()
        .dcode_home(home.path().to_path_buf())
...
async fn returns_empty_policy_when_no_policy_files_exist() {
    ...
    let config_stack = config_stack_for_dot_dcode_folder(temp_dir.path());
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        dcode_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(dcode_config::Sourced::new(
            dcode_config::RequirementsExecPolicy::new(requirements_exec_policy),
            dcode_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
    let dot_dcode_folder = AbsolutePathBuf::from_absolute_path(temp_dir.path())?;
    let layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project { dot_dcode_folder },
        TomlValue::Table(Default::default()),
    );
...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let project_dot_dcode_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
    let layers = vec![ConfigLayerEntry::new_disabled(
        ConfigLayerSource::Project {
            dot_dcode_folder: project_dot_dcode_folder,
        },
        TomlValue::Table(Default::default()),
        "marked untrusted",
    )];
...
async fn request_rule_falls_back_when_prefix_rule_does_not_approve_all_commands() {
    let command = vec![
        "bash".to_string(),
        "-lc".to_string(),
        "cargo install cargo-insta && rm -rf /tmp/dcode".to_string(),
    ];
    let manager = ExecPolicyManager::default();
...
async fn append_execpolicy_amendment_updates_policy_and_file() {
    let dcode_home = tempdir().expect("create temp dir");
    let prefix = vec!["echo".to_string(), "hello".to_string()];
    let manager = ExecPolicyManager::default();

    manager
        .append_amendment_and_update(dcode_home.path(), &ExecPolicyAmendment::from(prefix))
        .await
    ...
    let contents = fs::read_to_string(default_policy_path(dcode_home.path()))
...
async fn append_execpolicy_amendment_rejects_empty_prefix() {
    ...
    let result = manager
        .append_amendment_and_update(dcode_home.path(), &ExecPolicyAmendment::from(vec![]))
        .await;
