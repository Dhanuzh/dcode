...
use crate::dcode_delegate::run_dcode_thread_interactive;
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            dcode_linux_sandbox_exe: spawn_config.dcode_linux_sandbox_exe.clone(),
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.dcode.shutdown_and_wait().await;
    }
    ...
    async fn refresh_last_committed_rollout_items(&self) {
        match load_rollout_items_for_fork(&self.dcode.session).await {
            Ok(Some(items)) => {
...
impl GuardianReviewSessionManager {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, dcode: Dcode) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            dcode.session.get_config().await.as_ref(),
        );
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, dcode: Dcode) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            dcode.session.get_config().await.as_ref(),
        );
        self.state
...
async fn spawn_guardian_review_session(
    ...
    let dcode = run_dcode_thread_interactive(
        spawn_config,
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.dcode.session.services.network_approval,
                )
                .await;
...
async fn append_guardian_followup_reminder(review_session: &GuardianReviewSession) {
    let turn_context = review_session.dcode.session.new_default_turn().await;
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            _ = &mut timeout => {
                let keep_review_session = interrupt_and_drain_turn(&review_session.dcode).await.is_ok();
            ...
            event = review_session.dcode.next_event() => {
                match event {
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<dcode_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
async fn interrupt_and_drain_turn(dcode: &Dcode) -> anyhow::Result<()> {
