...
async fn guardian_test_session_and_turn_with_base_url(
    ...
    let (mut session, mut turn) = crate::dcode::make_session_and_context().await;
        ...
        .record_into_history(
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"openai/dcode\"}".to_string(),
                    call_id: "call-1".to_string(),
                },
                ResponseItem::FunctionCallOutput {
                    call_id: "call-1".to_string(),
                    output: dcode_protocol::models::FunctionCallOutputPayload::from_text(
...
async fn cancelled_guardian_review_emits_terminal_abort_without_warning() {
    let (session, turn, rx) = crate::dcode::make_session_and_context_with_rx().await;
...
async fn routes_approval_to_guardian_requires_auto_only_review_policy() {
    let (_session, mut turn) = crate::dcode::make_session_and_context().await;
    let mut config = (*turn.config).clone();
...
-> anyhow::Result<()> {
    ...
    let prompt = build_guardian_prompt_items(
        ...
        GuardianApprovalRequest::Shell {
            ...
            cwd: PathBuf::from("/repo/dcode-rs/core"),
    ...
    settings.bind(|| {
        assert_snapshot!(
            "dcode_core__guardian__tests__guardian_review_request_layout",
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "dcode_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn guardian_review_surfaces_responses_api_errors_in_rejection_reason() -> anyhow::Result<()> {
    ...
    let (mut session, mut turn, rx) = crate::dcode::make_session_and_context_with_rx().await;
    let mut config = (*turn.config).clone();
    config.model_provider.base_url = Some(format!("{}/v1", server.uri()));
    config.user_instructions = None;
    let config = Arc::new(config);
...
fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        None,
        "parent-active-model",
        Some(dcode_protocol::openai_models::ReasoningEffort::Low),
    )
