...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &dcode_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        dcode_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(name))
...
pub(super) async fn run_guardian_review_session(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    prompt_items: Vec<dcode_protocol::user_input::UserInput>,
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == dcode_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == dcode_protocol::openai_models::ReasoningEffort::Low),
