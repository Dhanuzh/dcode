...
async fn refresh_accessible_connectors_cache_from_mcp_tools_writes_latest_installed_apps() {
    ...
    let tools = HashMap::from([
        ...
        (
            "mcp__dcode_apps__openai_hidden".to_string(),
            dcode_app_tool(
                "openai_hidden",
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let dcode_home = tempdir().expect("tempdir should succeed");
    std::fs::write(dcode_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(dcode_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .dcode_home(dcode_home.path().to_path_buf())
        .fallback_cwd(Some(dcode_home.path().to_path_buf()))
        .build()
