...
pub(crate) async fn handle_mcp_tool_call(
    ...
    .await;
    maybe_track_dcode_app_used(sess.as_ref(), turn_context.as_ref(), &server, &tool_name).await;

    let status = if result.is_ok() { "ok" } else { "error" };
    turn_context
        .session_telemetry
        .counter("dcode.mcp.call", /*inc*/ 1, &[("status", status)]);
...
async fn maybe_track_dcode_app_used(
    sess: &Session,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    dcode_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
}

const MCP_TOOL_CODEX_APPS_META_KEY: &str = "_dcode_apps";
...
fn build_mcp_tool_call_request_meta(
    ...
    if server == CODEX_APPS_MCP_SERVER_NAME
        && let Some(dcode_apps_meta) =
            metadata.and_then(|metadata| metadata.dcode_apps_meta.clone())
    {
        request_meta.insert(
            MCP_TOOL_CODEX_APPS_META_KEY.to_string(),
            serde_json::Value::Object(dcode_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__dcode_mcp_decline__";
const MCP_TOOL_APPROVAL_ACCEPT_AND_REMEMBER: &str = "Allow and don't ask me again";
const MCP_TOOL_APPROVAL_CANCEL: &str = "Cancel";
const MCP_TOOL_APPROVAL_KIND_KEY: &str = "dcode_approval_kind";
...
async fn maybe_persist_mcp_tool_approval(
    ...
    if let Err(err) =
        persist_dcode_app_tool_approval(&turn_context.config.dcode_home, &connector_id, &tool_name)
            .await
    {
        error!(
            error = %err,
            connector_id,
            tool_name,
            "failed to persist dcode app tool approval"
        );
        remember_mcp_tool_approval(sess, key).await;
        return;
    }
...
async fn persist_dcode_app_tool_approval(
    dcode_home: &Path,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
