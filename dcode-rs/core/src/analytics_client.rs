...
enum TrackEventRequest {
    ...
    AppMentioned(DcodeAppMentionedEventRequest),
    AppUsed(DcodeAppUsedEventRequest),
    PluginUsed(DcodePluginUsedEventRequest),
    PluginInstalled(DcodePluginEventRequest),
    PluginUninstalled(DcodePluginEventRequest),
    PluginEnabled(DcodePluginEventRequest),
    PluginDisabled(DcodePluginEventRequest),
}
...
#[derive(Serialize)]
struct DcodeAppMentionedEventRequest {
    event_type: &'static str,
    event_params: DcodeAppMetadata,
}

#[derive(Serialize)]
struct DcodeAppUsedEventRequest {
    event_type: &'static str,
    event_params: DcodeAppMetadata,
}

#[derive(Serialize)]
struct DcodePluginMetadata {
...
#[derive(Serialize)]
struct DcodePluginUsedMetadata {
    #[serde(flatten)]
    plugin: DcodePluginMetadata,
    thread_id: Option<String>,
    turn_id: Option<String>,
    model_slug: Option<String>,
}
...
#[derive(Serialize)]
struct DcodePluginEventRequest {
    event_type: &'static str,
    event_params: DcodePluginMetadata,
}

#[derive(Serialize)]
struct DcodePluginUsedEventRequest {
    event_type: &'static str,
    event_params: DcodePluginUsedMetadata,
}
...
async fn send_track_app_mentioned(auth_manager: &AuthManager, job: TrackAppMentionedJob) {
    ...
    let events = mentions
        .into_iter()
        .map(|mention| {
            let event_params = dcode_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(DcodeAppMentionedEventRequest {
                event_type: "dcode_app_mentioned",
                event_params,
            })
        })
        .collect::<Vec<_>>();
...
async fn send_track_app_used(auth_manager: &AuthManager, job: TrackAppUsedJob) {
    ...
    let event_params = dcode_app_metadata(&tracking, app);
    let events = vec![TrackEventRequest::AppUsed(DcodeAppUsedEventRequest {
        event_type: "dcode_app_used",
        event_params,
    })];
...
async fn send_track_plugin_used(auth_manager: &AuthManager, job: TrackPluginUsedJob) {
    ...
    let events = vec![TrackEventRequest::PluginUsed(DcodePluginUsedEventRequest {
        event_type: "dcode_plugin_used",
        event_params: dcode_plugin_used_metadata(&tracking, plugin),
    })];
...
async fn send_track_plugin_installed(auth_manager: &AuthManager, job: TrackPluginManagementJob) {
    send_track_plugin_management_event(auth_manager, job, "dcode_plugin_installed").await;
}

async fn send_track_plugin_uninstalled(auth_manager: &AuthManager, job: TrackPluginManagementJob) {
    send_track_plugin_management_event(auth_manager, job, "dcode_plugin_uninstalled").await;
}

async fn send_track_plugin_enabled(auth_manager: &AuthManager, job: TrackPluginManagementJob) {
    send_track_plugin_management_event(auth_manager, job, "dcode_plugin_enabled").await;
}

async fn send_track_plugin_disabled(auth_manager: &AuthManager, job: TrackPluginManagementJob) {
    send_track_plugin_management_event(auth_manager, job, "dcode_plugin_disabled").await;
}
...
async fn send_track_plugin_management_event(
    ...
    let event_params = dcode_plugin_metadata(plugin);
    let event = DcodePluginEventRequest {
        event_type,
        event_params,
    };
    let events = vec![match event_type {
        "dcode_plugin_installed" => TrackEventRequest::PluginInstalled(event),
        "dcode_plugin_uninstalled" => TrackEventRequest::PluginUninstalled(event),
        "dcode_plugin_enabled" => TrackEventRequest::PluginEnabled(event),
        "dcode_plugin_disabled" => TrackEventRequest::PluginDisabled(event),
...
fn dcode_app_metadata(tracking: &TrackEventsContext, app: AppInvocation) -> DcodeAppMetadata {
    DcodeAppMetadata {
...
fn dcode_plugin_metadata(plugin: PluginTelemetryMetadata) -> DcodePluginMetadata {
...
fn dcode_plugin_used_metadata(
    tracking: &TrackEventsContext,
    plugin: PluginTelemetryMetadata,
) -> DcodePluginUsedMetadata {
    DcodePluginUsedMetadata {
        plugin: dcode_plugin_metadata(plugin),
        thread_id: Some(tracking.thread_id.clone()),
        turn_id: Some(tracking.turn_id.clone()),
        model_slug: Some(tracking.model_slug.clone()),
    }
...
async fn send_track_events(
    ...
    let url = format!("{base_url}/dcode/analytics-events/events");
