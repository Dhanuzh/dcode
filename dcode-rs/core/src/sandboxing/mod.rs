...
pub use dcode_protocol::models::SandboxPermissions;
...
use dcode_sandboxing::policy_transforms::EffectiveSandboxPermissions;
use dcode_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use dcode_sandboxing::policy_transforms::effective_network_sandbox_policy;
use dcode_sandboxing::policy_transforms::should_require_platform_sandbox;
#[cfg(target_os = "macos")]
use dcode_sandboxing::seatbelt::MACOS_PATH_TO_SEATBELT_EXECUTABLE;
#[cfg(target_os = "macos")]
...
pub(crate) struct SandboxTransformRequest<'a> {
    ...
    pub dcode_linux_sandbox_exe: Option<&'a PathBuf>,
...
pub(crate) enum SandboxTransformError {
    #[error("missing dcode-linux-sandbox executable path")]
    ...
    pub(crate) fn transform(
        ...
        let (command, sandbox_env, arg0_override) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let exe = dcode_linux_sandbox_exe
                ...
                (
                    full_command,
                    HashMap::new(),
                    Some("dcode-linux-sandbox".to_string()),
                )
            }
            // On Windows, the restricted token sandbox executes in-process via the
            // dcode-windows-sandbox crate. We leave the command unchanged here and
