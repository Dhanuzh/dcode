...
//!
//! The history is stored at `~/.dcode/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.dcode`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.dcode_home.clone();
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.dcode/history.jsonl` and ensure the parent directory exists.
