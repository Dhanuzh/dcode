...
async fn forward_events_cancelled_while_send_blocked_shuts_down_delegate() {
    ...
    let (session, ctx, _rx_evt) = crate::dcode::make_session_and_context_with_rx().await;
...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    let (_agent_status_tx, agent_status) = watch::channel(AgentStatus::PendingInit);
    let (session, _ctx, _rx_evt) = crate::dcode::make_session_and_context_with_rx().await;
    let dcode = Arc::new(Dcode {
        tx_sub,
        rx_event: rx_events,
        agent_status,
    ...
    let cancel = CancellationToken::new();
    let forward = tokio::spawn(forward_ops(Arc::clone(&dcode), rx_ops, cancel));

    let submission = Submission {
        id: "sub-1".to_string(),
        op: Op::Interrupt,
        trace: Some(dcode_protocol::protocol::W3cTraceContext {
            traceparent: Some(
...
async fn handle_request_permissions_uses_tool_call_id_for_round_trip() {
    let (parent_session, parent_ctx, rx_events) =
        crate::dcode::make_session_and_context_with_rx().await;
