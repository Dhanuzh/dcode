...
pub(crate) const CODEX_APPS_MCP_SERVER_NAME: &str = "dcode_apps";
...
fn dcode_apps_mcp_bearer_token_env_var() -> Option<String> {
...
fn dcode_apps_mcp_bearer_token(auth: Option<&DcodeAuth>) -> Option<String> {
...
fn dcode_apps_mcp_http_headers(auth: Option<&DcodeAuth>) -> Option<HashMap<String, String>> {
    let mut headers = HashMap::new();
    if let Some(token) = dcode_apps_mcp_bearer_token(auth) {
        headers.insert("Authorization".to_string(), format!("Bearer {token}"));
    }
    if let Some(account_id) = auth.and_then(DcodeAuth::get_account_id) {
...
fn normalize_dcode_apps_base_url(base_url: &str) -> String {
...
fn dcode_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_dcode_apps_base_url(base_url);
    if base_url.contains("/backend-api") {
        format!("{base_url}/wham/apps")
    } else if base_url.contains("/api/dcode") {
        format!("{base_url}/apps")
    } else {
        format!("{base_url}/api/dcode/apps")
    }
}

pub(crate) fn dcode_apps_mcp_url(config: &Config) -> String {
    dcode_apps_mcp_url_for_base_url(&config.chatgpt_base_url)
}

fn dcode_apps_mcp_server_config(config: &Config, auth: Option<&DcodeAuth>) -> McpServerConfig {
    let bearer_token_env_var = dcode_apps_mcp_bearer_token_env_var();
    let http_headers = if bearer_token_env_var.is_some() {
        None
    } else {
        dcode_apps_mcp_http_headers(auth)
    };
    let url = dcode_apps_mcp_url(config);
...
pub(crate) fn with_dcode_apps_mcp(
    ...
    if connectors_enabled {
        servers.insert(
            ...
            dcode_apps_mcp_server_config(config, auth),
        );
    } else {
