...
#[test]
fn dcode_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
        dcode_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/wham/apps"
    );
    assert_eq!(
        dcode_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/wham/apps"
    );
    assert_eq!(
        dcode_apps_mcp_url_for_base_url("http://localhost:8080/api/dcode"),
        "http://localhost:8080/api/dcode/apps"
    );
    assert_eq!(
        dcode_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/dcode/apps"
    );
}

#[test]
fn dcode_apps_mcp_url_uses_legacy_dcode_apps_path() {
    let mut config = crate::config::test_config();
    config.chatgpt_base_url = "https://chatgpt.com".to_string();

    assert_eq!(
        dcode_apps_mcp_url(&config),
        "https://chatgpt.com/backend-api/wham/apps"
    );
}

#[test]
fn dcode_apps_server_config_uses_legacy_dcode_apps_path() {
    let mut config = crate::config::test_config();
    config.chatgpt_base_url = "https://chatgpt.com".to_string();

    let mut servers = with_dcode_apps_mcp(HashMap::new(), false, None, &config);
    ...
    servers = with_dcode_apps_mcp(servers, true, None, &config);
    let server = servers
        .get(CODEX_APPS_MCP_SERVER_NAME)
        .expect("dcode apps should be present when apps is enabled");
    let url = match &server.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => url,
        _ => panic!("expected streamable http transport for dcode apps"),
    };
