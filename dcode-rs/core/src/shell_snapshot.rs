...
impl ShellSnapshot {
    ...
    fn spawn_snapshot_task(
        ...
        tokio::spawn(
            async move {
                let timer = session_telemetry.start_timer("dcode.shell_snapshot.duration_ms", &[]);
                ...
                session_telemetry.counter("dcode.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_new(
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&dcode_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            dcode_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(dcode_home: &Path, active_session_id: ThreadId) -> Result<()> {
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(dcode_home, session_id).await?;
