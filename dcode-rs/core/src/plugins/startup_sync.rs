...
pub(crate) fn curated_plugins_repo_path(dcode_home: &Path) -> PathBuf {
    dcode_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub(crate) fn read_curated_plugins_sha(dcode_home: &Path) -> Option<String> {
    read_sha_file(dcode_home.join(CURATED_PLUGINS_SHA_FILE).as_path())
}

pub(crate) fn sync_openai_plugins_repo(dcode_home: &Path) -> Result<String, String> {
    sync_openai_plugins_repo_with_transport_overrides(dcode_home, "git", GITHUB_API_BASE_URL)
}
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    match sync_openai_plugins_repo_via_git(dcode_home, git_binary) {
        ...
        Err(err) => {
            ...
            sync_openai_plugins_repo_via_http(dcode_home, api_base_url)
        }
    }
}

fn sync_openai_plugins_repo_via_git(dcode_home: &Path, git_binary: &str) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(dcode_home);
    let sha_path = dcode_home.join(CURATED_PLUGINS_SHA_FILE);
...
pub(super) fn start_startup_remote_plugin_sync_once(
    ...
    let marker_path = startup_remote_plugin_sync_marker_path(dcode_home.as_path());
    ...
    tokio::spawn(async move {
        ...
        if !wait_for_startup_remote_plugin_sync_prerequisites(dcode_home.as_path()).await {
            warn!(
                dcode_home = %dcode_home.display(),
            ...
            .await
        {
            Ok(sync_result) => {
                ...
                if let Err(err) =
                    write_startup_remote_plugin_sync_marker(dcode_home.as_path()).await
                {
                    warn!(
                        error = %err,
...
fn startup_remote_plugin_sync_marker_path(dcode_home: &Path) -> PathBuf {
    dcode_home.join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE)
}

fn startup_remote_plugin_sync_prerequisites_ready(dcode_home: &Path) -> bool {
    dcode_home
        .join(".tmp/plugins/.agents/plugins/marketplace.json")
        .is_file()
        && dcode_home.join(".tmp/plugins.sha").is_file()
}

async fn wait_for_startup_remote_plugin_sync_prerequisites(dcode_home: &Path) -> bool {
    let deadline = tokio::time::Instant::now() + STARTUP_REMOTE_PLUGIN_SYNC_PREREQUISITE_TIMEOUT;
    loop {
        if startup_remote_plugin_sync_prerequisites_ready(dcode_home) {
            return true;
        }
...
async fn write_startup_remote_plugin_sync_marker(dcode_home: &Path) -> std::io::Result<()> {
    let marker_path = startup_remote_plugin_sync_marker_path(dcode_home);
