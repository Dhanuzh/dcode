...
fn load_plugins_from_config(config_toml: &str, dcode_home: &Path) -> PluginLoadOutcome {
    write_file(&dcode_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config_blocking(dcode_home, dcode_home);
    PluginsManager::new(dcode_home.to_path_buf()).plugins_for_config(&config)
}

async fn load_config(dcode_home: &Path, cwd: &Path) -> crate::config::Config {
...
fn load_config_blocking(dcode_home: &Path, cwd: &Path) -> crate::config::Config {
    tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .expect("tokio runtime should build")
        .block_on(load_config(dcode_home, cwd))
}
...
fn load_plugins_loads_default_skills_and_mcp_servers() {
  ...
  "apps": {
    ...
    let outcome = load_plugins_from_config(&plugin_config_toml(true, true), dcode_home.path());
...
enabled = true
"#;
    let outcome = load_plugins_from_config(config_toml, dcode_home.path());
...
fn load_plugins_preserves_disabled_plugins_without_effective_contributions() {
  ...
  "mcpServers": {
    ...
    let outcome = load_plugins_from_config(&plugin_config_toml(false, true), dcode_home.path());
...
fn effective_apps_dedupes_connector_ids_across_plugins() {
    let dcode_home = TempDir::new().unwrap();
    let plugin_a_root = dcode_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = dcode_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".dcode-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".dcode-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(dcode_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![dcode_home.path().join("skills-plugin/skills")],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![dcode_home.path().join("disabled-plugin/skills")],
...
fn plugin_namespace_for_skill_path_uses_manifest_name() {
    let dcode_home = TempDir::new().unwrap();
    let plugin_root = dcode_home.path().join("plugins/sample");
...
fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config_blocking(dcode_home.path(), dcode_home.path());
    let outcome = PluginsManager::new(dcode_home.path().to_path_buf()).plugins_for_config(&config);
...
"#,
    ...
    write_file(
        &repo_root.join(".dcode/config.toml"),
        r#"[[skills.config]]
  ...
  {"id":"2","name":"gmail","marketplace_name":"openai-curated","version":"1.0.0","enabled":false}
    ...
    let result = manager
        .sync_plugins_from_remote(
            &config,
            Some(&DcodeAuth::create_dummy_chatgpt_auth_for_testing()),
            /*additive_only*/ false,
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_dcode() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["dcode-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["dcode-plugin".to_string()]);
}
...
fn load_plugins_ignores_project_config_files() {
    let dcode_home = TempDir::new().unwrap();
    let project_root = dcode_home.path().join("project");
    let plugin_root = dcode_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".dcode/config.toml"),
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_dcode_folder: AbsolutePathBuf::try_from(project_root.join(".dcode")).unwrap(),
            },
    ...
    let outcome = load_plugins_from_layer_stack(
        &stack,
        &PluginStore::new(dcode_home.path().to_path_buf()),
        Some(Product::Dcode),
    );
