...
#[test]
fn curated_plugins_repo_path_uses_dcode_home_tmp_dir() {
...
if [ "$1" = "clone" ]; then
  dest="$5"
  mkdir -p "$dest/.git" "$dest/.agents/plugins" "$dest/plugins/gmail/.dcode-plugin"
...
EOF
  printf '%s\n' '{{"name":"gmail"}}' > "$dest/plugins/gmail/.dcode-plugin/plugin.json"
  exit 0
fi
...
async fn sync_openai_plugins_repo_falls_back_to_http_when_git_is_unavailable() {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.dcode-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(read_curated_plugins_sha(tmp.path()).as_deref(), Some(sha));
}
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.dcode-plugin/plugin.json"),
            options,
        )
