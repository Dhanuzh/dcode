...
impl PluginsManager {
    pub fn new(dcode_home: PathBuf) -> Self {
        Self::new_with_restriction_product(dcode_home, Some(Product::Dcode))
    }
    ...
    pub fn new_with_restriction_product(
        ...
        Self {
            ...
            store: PluginStore::new(dcode_home),
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if resolved.plugin_id.marketplace_name == OPENAI_CURATED_MARKETPLACE_NAME {
                Some(
                    read_curated_plugins_sha(self.dcode_home.as_path()).ok_or_else(|| {
        ...
        ConfigService::new_with_defaults(self.dcode_home.clone())
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        let plugin_telemetry = self
            ...
            .map(|_| installed_plugin_telemetry_metadata(self.dcode_home.as_path(), &plugin_id));
        let store = self.store.clone();
        ...
        ConfigEditsBuilder::new(&self.dcode_home)
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let curated_marketplace_root = curated_plugins_repo_path(self.dcode_home.as_path());
        ...
        let curated_plugin_version = read_curated_plugins_sha(self.dcode_home.as_path())
            .ok_or_else(|| {
                PluginStoreError::Invalid(
                    "local curated marketplace sha is not available".to_string(),
                )
    ...
    pub fn read_plugin_for_config(
        ...
        let manifest = load_plugin_manifest(source_path.as_path()).ok_or_else(|| {
            MarketplaceError::InvalidPlugin(
                "missing or invalid .dcode-plugin/plugin.json".to_string(),
            )
        })?;
    ...
    fn start_curated_repo_sync(self: &Arc<Self>, configured_curated_plugin_ids: Vec<PluginId>) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(
                move || match sync_openai_plugins_repo(dcode_home.as_path()) {
    ...
    fn marketplace_roots(&self, additional_roots: &[AbsolutePathBuf]) -> Vec<AbsolutePathBuf> {
        ...
        let curated_repo_root = curated_plugins_repo_path(self.dcode_home.as_path());
...
fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::new(dcode_home.to_path_buf());
    let curated_marketplace_path = AbsolutePathBuf::try_from(
        curated_plugins_repo_path(dcode_home).join(".agents/plugins/marketplace.json"),
    )
...
fn load_plugin(
    ...
    let Some(manifest) = load_plugin_manifest(plugin_root.as_path()) else {
        loaded_plugin.error = Some("missing or invalid .dcode-plugin/plugin.json".to_string());
        return loaded_plugin;
    };
