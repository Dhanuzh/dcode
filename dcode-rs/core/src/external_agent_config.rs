...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "dcode.external_agent_config.detect";
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "dcode.external_agent_config.import";
...
impl ExternalAgentConfigService {
    ...
    #[cfg(test)]
    fn new_for_test(dcode_home: PathBuf, claude_home: PathBuf) -> Self {
        Self {
            dcode_home,
            claude_home,
        }
    ...
    fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.dcode_home.join("config.toml"),
            |repo_root| repo_root.join(".dcode").join("config.toml"),
        );
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.dcode_home.join("AGENTS.md"),
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<()> {
        let (source_settings, target_config) = if let Some(repo_root) = find_repo_root(cwd)? {
            (
                ...
                repo_root.join(".dcode").join("config.toml"),
            )
...
fn rewrite_claude_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "Dcode");
    }
    rewritten
}
