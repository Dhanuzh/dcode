...
pub async fn maybe_migrate_personality(
    ...
    let marker_path = dcode_home.join(PERSONALITY_MIGRATION_FILENAME);
    ...
    if !has_recorded_sessions(dcode_home, model_provider_id.as_str()).await? {
        create_marker(&marker_path).await?;
...
async fn has_recorded_sessions(dcode_home: &Path, default_provider: &str) -> io::Result<bool> {
    let allowed_sources: &[SessionSource] = &[];

    if let Some(state_db_ctx) = state_db::open_if_present(dcode_home, default_provider).await
    ...
    let sessions = get_threads_in_root(
        dcode_home.join(SESSIONS_SUBDIR),
        /*page_size*/ 1,
        /*cursor*/ None,
        ThreadSortKey::CreatedAt,
        ThreadListConfig {
    ...
    let archived_sessions = get_threads_in_root(
        dcode_home.join(ARCHIVED_SESSIONS_SUBDIR),
        /*page_size*/ 1,
        /*cursor*/ None,
        ThreadSortKey::CreatedAt,
        ThreadListConfig {
