...
///
/// 1. The process was spawned by Dcode as part of a shell tool call.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    // If this Dcode process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                dcode_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            Ok(())
        });
    }
