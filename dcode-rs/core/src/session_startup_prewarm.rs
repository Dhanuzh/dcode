...
use crate::dcode::build_prompt;
use crate::dcode::built_tools;
use crate::error::Result as DcodeResult;
use dcode_otel::SessionTelemetry;
use dcode_otel::metrics::names::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use dcode_otel::metrics::names::STARTUP_PREWARM_DURATION_METRIC;
use dcode_protocol::models::BaseInstructions;

pub(crate) struct SessionStartupPrewarmHandle {
    task: JoinHandle<DcodeResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<DcodeResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
) -> DcodeResult<ModelClientSession> {
