...
fn search_capable_model_info() -> ModelInfo {
    ...
    let mut model_info =
        ModelsManager::construct_model_info_offline_for_tests("gpt-5-dcode", &config);
...
fn search_tool_deferred_tools_always_set_defer_loading_true() {
    ...
    let openai_tool =
        mcp_tool_to_deferred_openai_tool("mcp__dcode_apps__lookup_order".to_string(), tool)
...
fn deferred_responses_api_tool_serializes_with_defer_loading() {
    ...
    assert_eq!(
        ...
        serde_json::json!({
            "type": "function",
            "name": "mcp__dcode_apps__lookup_order",
...
fn model_provided_unified_exec_is_blocked_for_windows_sandboxed_policies() {
    let mut model_info = model_info_from_models_json("gpt-5-dcode");
...
#[test]
fn test_full_toolset_specs_for_gpt5_dcode_unified_exec_web_search() {
    let model_info = model_info_from_models_json("gpt-5-dcode");
...
fn test_build_specs_collab_tools_enabled() {
    let config = test_config();
    let model_info = ModelsManager::construct_model_info_offline_for_tests("gpt-5-dcode", &config);
    let mut features = Features::with_defaults();
...
fn test_build_specs_artifact_tool_enabled() {
    ...
    let runtime_root = tempfile::TempDir::new().expect("create temp dcode home");
    config.dcode_home = runtime_root.path().to_path_buf();
    let model_info = ModelsManager::construct_model_info_offline_for_tests("gpt-5-dcode", &config);
    let mut features = Features::with_defaults();
...
fn js_repl_freeform_grammar_blocks_common_non_js_prefixes() {
    ...
    assert!(format.definition.contains("dcode-js-repl:"));
    assert!(!format.definition.contains("(?!"));
}
...
fn web_search_config_is_forwarded_to_tool_spec() {
    ...
    let web_search_config = WebSearchConfig {
        filters: Some(dcode_protocol::config_types::WebSearchFilters {
            allowed_domains: Some(vec!["example.com".to_string()]),
        }),
        user_location: Some(dcode_protocol::config_types::WebSearchUserLocation {
            r#type: dcode_protocol::config_types::WebSearchUserLocationType::Approximate,
            country: Some("US".to_string()),
            region: Some("California".to_string()),
            city: Some("San Francisco".to_string()),
            timezone: Some("America/Los_Angeles".to_string()),
        }),
        search_context_size: Some(dcode_protocol::config_types::WebSearchContextSize::High),
    };
...
#[test]
fn test_build_specs_gpt5_dcode_default() {
    let features = Features::with_defaults();
...
#[test]
fn test_build_specs_gpt51_dcode_default() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.1-dcode",
        &features,
...
#[test]
fn test_build_specs_gpt5_dcode_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5-dcode",
...
#[test]
fn test_build_specs_gpt51_dcode_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.1-dcode",
...
#[test]
fn test_gpt_5_1_dcode_max_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.1-dcode-max",
        &features,
...
#[test]
fn test_dcode_5_1_mini_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.1-dcode-mini",
        &features,
...
#[test]
fn test_gpt_5_1_dcode_max_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.1-dcode-max",
    ...
    assert_eq!(
        tools_config
            .with_unified_exec_shell_mode_for_session(
                ...
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\dcode\dcode-execve-wrapper"
                } else {
                    "/opt/dcode/dcode-execve-wrapper"
                })),
            )
            .unified_exec_shell_mode,
        if cfg!(unix) {
            UnifiedExecShellMode::ZshFork(ZshForkConfig {
                shell_zsh_path: AbsolutePathBuf::from_absolute_path("/opt/dcode/zsh").unwrap(),
...
#[test]
fn search_tool_description_lists_each_dcode_apps_connector_once() {
        ...
        Some(HashMap::from([
            (
                ...
                ToolInfo {
                    ...
                    tool_namespace: "mcp__dcode_apps__calendar".to_string(),
                    tool: mcp_tool(
            ...
            (
                "mcp__dcode_apps__calendar_list_events".to_string(),
                ToolInfo {
                    server_name: crate::mcp::CODEX_APPS_MCP_SERVER_NAME.to_string(),
            ...
            (
                "mcp__dcode_apps__gmail_search_threads".to_string(),
                ToolInfo {
                    server_name: crate::mcp::CODEX_APPS_MCP_SERVER_NAME.to_string(),
                    tool_name: "_search_threads".to_string(),
                    tool_namespace: "mcp__dcode_apps__gmail".to_string(),
                    tool: mcp_tool(
...
fn search_tool_registers_namespaced_app_tool_aliases() {
    ...
    let alias = tool_handler_key("_create_event", Some("mcp__dcode_apps__calendar"));
