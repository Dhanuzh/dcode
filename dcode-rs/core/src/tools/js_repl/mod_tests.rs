use super::*;
use crate::dcode::make_session_and_context;
use crate::dcode::make_session_and_context_with_dynamic_tools_and_rx;
use crate::protocol::AskForApproval;
use crate::protocol::EventMsg;
use crate::protocol::SandboxPolicy;
use crate::turn_diff_tracker::TurnDiffTracker;
use dcode_features::Feature;
use dcode_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
fn set_danger_full_access(turn: &mut crate::dcode::TurnContext) {
    turn.sandbox_policy
...
fn validate_emitted_image_url_rejects_non_data_scheme() {
    assert_eq!(
        ...
        Err("dcode.emitImage only accepts data URLs".to_string())
    );
}
...
async fn can_run_js_repl_runtime_tests() -> bool {
    ...
    // the dcode-linux-sandbox arg0 dispatch path, which is exercised in
...
async fn js_repl_uncaught_exception_returns_exec_error_and_recovers() -> anyhow::Result<()> {
    ...
    let (session, turn) = crate::dcode::make_session_and_context().await;
    let session = Arc::new(session);
    let turn = Arc::new(turn);
    let tracker = Arc::new(tokio::sync::Mutex::new(TurnDiffTracker::default()));
    let manager = turn.js_repl.manager().await?;
...
void dcode.tool("shell_command", {{ command: `sleep 0.35; printf js_repl_unawaited_done > "${{marker}}"` }});
...
const imagePath = path.join(dcode.tmpDir, "js-repl-view-image.png");
const png = Buffer.from(
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==",
  "base64"
);
await fs.writeFile(imagePath, png);
const out = await dcode.tool("view_image", { path: imagePath });
...
const fs = await import("node:fs/promises");
const path = await import("node:path");
const imagePath = path.join(dcode.tmpDir, "js-repl-view-image-explicit.png");
const png = Buffer.from(
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==",
  "base64"
);
...
await dcode.emitImage({ bytes: png, mimeType: "image/png" });
"#;
...
const savedEmitImage = dcode.emitImage;
...
async fn js_repl_unawaited_emit_image_errors_fail_cell() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
void dcode.emitImage({ bytes: new Uint8Array(), mimeType: "image/png" });
console.log("cell-complete");
"#;
...
async fn js_repl_caught_emit_image_error_does_not_fail_cell() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let code = r#"
try {
  await dcode.emitImage({ bytes: new Uint8Array(), mimeType: "image/png" });
} catch (error) {
...
await dcode.emitImage({ bytes: png });
"#;
...
async fn js_repl_emit_image_rejects_non_data_url() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
await dcode.emitImage("https://example.com/image.png");
"#;
...
async fn js_repl_emit_image_accepts_case_insensitive_data_url() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
await dcode.emitImage("DATA:image/png;base64,AAA");
"#;
...
await dcode.emitImage({ bytes: png, mimeType: "image/png", detail: "ultra" });
"#;
...
await dcode.emitImage({ bytes: png, mimeType: "image/png", detail: null });
"#;
...
async fn js_repl_emit_image_rejects_mixed_content() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
const out = await dcode.tool("inline_image", {});
await dcode.emitImage(out);
"#;
...
async fn js_repl_dynamic_tool_response_preserves_js_line_separator_text() -> anyhow::Result<()> {
    ...
    for (tool_name, description, expected_text, literal) in [
        ...
        *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
        ...
        let code = format!(
            r#"
const out = await dcode.tool("{tool_name}", {{}});
...
async fn js_repl_can_call_hidden_dynamic_tools() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
const out = await dcode.tool("hidden_dynamic_tool", { city: "Paris" });
...
async fn js_repl_imported_local_files_can_access_repl_globals() -> anyhow::Result<()> {
    ...
    let cwd_dir = tempdir()?;
    let expected_home_dir = serde_json::to_string("/tmp/dcode-home")?;
    write_js_repl_test_module(
        cwd_dir.path(),
        "globals.js",
        &format!(
            "const expectedHomeDir = {expected_home_dir};\nconsole.log(`tmp:${{dcode.tmpDir === tmpDir}}`);\nconsole.log(`cwd:${{typeof dcode.cwd}}:${{dcode.cwd.length > 0}}`);\nconsole.log(`home:${{dcode.homeDir === expectedHomeDir}}`);\nconsole.log(`tool:${{typeof dcode.tool}}`);\nconsole.log(\"local-file-console-ok\");\n"
        ),
    )?;
