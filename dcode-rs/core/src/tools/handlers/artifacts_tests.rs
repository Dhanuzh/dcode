...
fn parse_freeform_args_with_artifact_tool_pragma() {
    let args = parse_freeform_args("// dcode-artifact-tool: timeout_ms=45000\nconsole.log('ok');")
        .expect("parse args");
    assert_eq!(args.source, "console.log('ok');");
...
#[test]
fn default_runtime_manager_uses_openai_dcode_release_base() {
    let dcode_home = TempDir::new().expect("create temp dcode home");
    let manager = default_runtime_manager(dcode_home.path().to_path_buf());
...
fn load_cached_runtime_reads_pinned_cache_path() {
    let dcode_home = TempDir::new().expect("create temp dcode home");
    let platform =
        dcode_artifacts::ArtifactRuntimePlatform::detect_current().expect("detect platform");
    let install_dir = dcode_home
        .path()
        .join("packages")
        .join("artifacts")
    ...
    let runtime = dcode_artifacts::load_cached_runtime(
        &dcode_home
            .path()
            .join(dcode_artifacts::DEFAULT_CACHE_ROOT_RELATIVE),
        versions::ARTIFACT_RUNTIME,
    )
