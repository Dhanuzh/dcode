...
struct ToolSuggestMeta<'a> {
    dcode_approval_kind: &'static str,
    tool_type: DiscoverableToolType,
...
impl ToolHandler for ToolSuggestHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("dcode-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin tool suggestions are not available in dcode-tui yet".to_string(),
            ));
        }
...
fn build_tool_suggestion_meta<'a>(
    ...
    ToolSuggestMeta {
        dcode_approval_kind: TOOL_SUGGEST_APPROVAL_KIND_VALUE,
        tool_type,
...
async fn verify_tool_suggestion_completed(
    session: &crate::dcode::Session,
    turn: &crate::dcode::TurnContext,
    tool: &DiscoverableTool,
    ...
    match tool {
        DiscoverableTool::Connector(connector) => {
            let manager = session.services.mcp_connection_manager.read().await;
            match manager.hard_refresh_dcode_apps_tools_cache().await {
                Ok(mcp_tools) => {
                    let accessible_connectors = connectors::with_app_enabled_state(
                        connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                        &turn.config,
                ...
                Err(err) => {
                    warn!(
                        "failed to refresh dcode apps tools cache after tool suggestion for {}: {err:#}",
                        connector.id
                    );
                    false
                }
