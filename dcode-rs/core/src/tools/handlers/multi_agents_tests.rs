        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    dcode_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_send_input_interrupts_busy_child_without_losing_message() {
    ...
    let active_turn = thread.dcode.session.new_default_turn().await;
    thread
        .dcode
        .session
        .spawn_task(
...
async fn spawn_agent_reapplies_runtime_sandbox_after_role_config() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("spawned agent thread should exist");
    let child_turn = child_thread.dcode.session.new_default_turn().await;
    assert_eq!(
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(DcodeAuth::from_api_key("dummy")),
            false,
            None,
        )
        .await
...
async fn multi_agent_v2_wait_agent_does_not_return_completed_content() {
    ...
    let thread = manager.start_thread(config).await.expect("start thread");
    let agent_id = thread.thread_id;
    let child_turn = thread.thread.dcode.session.new_default_turn().await;
    thread
        .thread
        .dcode
        .session
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.dcode.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.dcode.session.clone();
    ...
    let operator_session = operator.thread.dcode.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.dcode.session.new_default_turn().await,
            "resume_agent",
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    turn.dcode_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    ...
    expected.dcode_linux_sandbox_exe = turn.dcode_linux_sandbox_exe.clone();
