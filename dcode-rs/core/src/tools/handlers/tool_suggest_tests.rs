...
#[test]
fn filter_tool_suggest_discoverable_tools_for_dcode_tui_omits_plugins() {
    ...
    assert_eq!(
        filter_tool_suggest_discoverable_tools_for_client(discoverable_tools, Some("dcode-tui"),),
...
async fn verified_plugin_suggestion_completed_requires_installed_plugin() {
    let dcode_home = tempdir().expect("tempdir should succeed");
    let curated_root = crate::plugins::curated_plugins_repo_path(dcode_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample"]);
    write_curated_plugin_sha(dcode_home.path());
    write_plugins_feature_config(dcode_home.path());

    let config = load_plugins_config(dcode_home.path()).await;
    let plugins_manager = PluginsManager::new(dcode_home.path().to_path_buf());
    ...
    let refreshed_config = load_plugins_config(dcode_home.path()).await;
    assert!(verified_plugin_suggestion_completed(
        "sample@openai-curated",
        &refreshed_config,
        &plugins_manager,
