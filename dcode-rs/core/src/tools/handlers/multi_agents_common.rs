...
pub(crate) fn collab_spawn_error(err: DcodeErr) -> FunctionCallError {
    match err {
        DcodeErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        DcodeErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: DcodeErr) -> FunctionCallError {
    match err {
        DcodeErr::ThreadNotFound(id) => {
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.dcode_linux_sandbox_exe = turn.dcode_linux_sandbox_exe.clone();
...
fn find_spawn_agent_model_name(
    available_models: &[dcode_protocol::openai_models::ModelPreset],
    requested_model: &str,
