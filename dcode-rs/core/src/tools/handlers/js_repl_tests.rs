...
use crate::dcode::make_session_and_context_with_rx;
...
fn parse_freeform_args_with_pragma() {
    let input = "// dcode-js-repl: timeout_ms=15000\nconsole.log('ok');";
...
fn parse_freeform_args_rejects_unknown_key() {
    let err = parse_freeform_args("// dcode-js-repl: nope=1\nconsole.log('ok');")
...
fn parse_freeform_args_rejects_reset_key() {
    let err = parse_freeform_args("// dcode-js-repl: reset=true\nconsole.log('ok');")
        .expect_err("expected error");
    assert_eq!(
        err.to_string(),
