...
impl JobProgressEmitter {
    ...
    async fn maybe_emit(
        ...
        turn: &TurnContext,
        job_id: &str,
        progress: &dcode_state::AgentJobProgress,
        force: bool,
    ) -> anyhow::Result<()> {
...
mod spawn_agents_on_csv {
    ...
    pub async fn handle(
        ...
        for (idx, row) in rows.into_iter().enumerate() {
            ...
            items.push(dcode_state::AgentJobItemCreateParams {
                item_id,
        ...
        let _job = db
            .create_agent_job(
                &dcode_state::AgentJobCreateParams {
                    id: job_id.clone(),
                    name: job_name,
        ...
        let failed_item_errors = if progress.failed_items > 0 {
            let items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(dcode_state::AgentJobItemStatus::Failed),
                    Some(5),
                )
                .await
                .unwrap_or_default();
...
fn required_state_db(
    session: &Arc<Session>,
) -> Result<Arc<dcode_state::StateRuntime>, FunctionCallError> {
...
async fn run_agent_job_loop(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    db: Arc<dcode_state::StateRuntime>,
    job_id: String,
    ...
    loop {
        ...
        if !cancel_requested && active_items.len() < options.max_concurrency {
            ...
            let pending_items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(dcode_state::AgentJobItemStatus::Pending),
                    Some(slots),
                )
                .await?;
            for item in pending_items {
                    ...
                    .await
                {
                    ...
                    Err(DcodeErr::AgentLimitReached { .. }) => {
...
async fn recover_running_items(
    ...
    let running_items = db
        .list_agent_job_items(
            job_id,
            Some(dcode_state::AgentJobItemStatus::Running),
            /*limit*/ None,
        )
        .await?;
...
async fn finalize_finished_item(
    ...
    if matches!(item.status, dcode_state::AgentJobItemStatus::Running) {
...
fn job_runtime_timeout(job: &dcode_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &dcode_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &dcode_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();
...
fn render_job_csv(
    headers: &[String],
    items: &[dcode_state::AgentJobItem],
