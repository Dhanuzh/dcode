...
use dcode_apply_patch::ApplyPatchAction;
use dcode_apply_patch::ApplyPatchFileChange;
use dcode_protocol::models::FileSystemPermissions;
use dcode_protocol::models::PermissionProfile;
use dcode_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use dcode_sandboxing::policy_transforms::merge_permission_profiles;
use dcode_sandboxing::policy_transforms::normalize_additional_permissions;
...
async fn effective_patch_permissions(
    ...
    dcode_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        match dcode_apply_patch::maybe_parse_apply_patch_verified(&command, &cwd) {
            dcode_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
                    ...
                    .await
                {
                    ...
                    InternalApplyPatchInvocation::DelegateToExec(apply) => {
                        ...
                        let req = ApplyPatchRequest {
                            ...
                            timeout_ms: None,
                            dcode_exe: turn.dcode_linux_sandbox_exe.clone(),
                        };
            ...
            dcode_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            dcode_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            dcode_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match dcode_apply_patch::maybe_parse_apply_patch_verified(command, cwd) {
        ...
        dcode_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
