...
use dcode_artifacts::ArtifactBuildRequest;
use dcode_artifacts::ArtifactCommandOutput;
use dcode_artifacts::ArtifactRuntimeManager;
use dcode_artifacts::ArtifactRuntimeManagerConfig;
use dcode_artifacts::ArtifactsClient;
use dcode_artifacts::ArtifactsError;
...
const ARTIFACT_TOOL_PRAGMA_PREFIX: &str = "// dcode-artifact-tool:";
...
fn parse_freeform_args(input: &str) -> Result<ArtifactsToolArgs, FunctionCallError> {
    if input.trim().is_empty() {
        return Err(FunctionCallError::RespondToModel(
            "artifacts expects raw JavaScript source text (non-empty) authored against the preloaded @oai/artifact-tool exports. Provide JS only, optionally with first-line `// dcode-artifact-tool: timeout_ms=15000`."
                .to_string(),
        ));
    }
...
fn reject_json_or_quoted_source(code: &str) -> Result<(), FunctionCallError> {
    ...
    if trimmed.starts_with("```") {
        return Err(FunctionCallError::RespondToModel(
            "artifacts expects raw JavaScript source, not markdown code fences. Resend plain JS only (optional first line `// dcode-artifact-tool: ...`)."
                .to_string(),
        ));
    }
    ...
    match value {
        JsonValue::Object(_) | JsonValue::String(_) => Err(FunctionCallError::RespondToModel(
            "artifacts is a freeform tool and expects raw JavaScript source authored against the preloaded @oai/artifact-tool exports. Resend plain JS only (optional first line `// dcode-artifact-tool: ...`); do not send JSON (`{\"code\":...}`), quoted code, or markdown fences."
                .to_string(),
        )),
        _ => Ok(()),
    }
...
fn default_runtime_manager(dcode_home: std::path::PathBuf) -> ArtifactRuntimeManager {
