...
use dcode_hooks::HookEventAfterToolUse;
use dcode_hooks::HookPayload;
use dcode_hooks::HookResult;
use dcode_hooks::HookToolInput;
use dcode_hooks::HookToolInputLocalShell;
use dcode_hooks::HookToolKind;
use dcode_protocol::models::ResponseInputItem;
use dcode_protocol::models::ShellCommandToolCallParams;
use dcode_protocol::models::ShellToolCallParams;
    ...
    match (tool_name, payload) {
        ("shell" | "container.exec", ToolPayload::Function { arguments }) => {
            serde_json::from_str::<ShellToolCallParams>(arguments)
                .ok()
                .map(|params| dcode_shell_command::parse_command::shlex_join(&params.command))
        }
        ("local_shell", ToolPayload::LocalShell { params }) => Some(
            dcode_shell_command::parse_command::shlex_join(&params.command),
        ),
