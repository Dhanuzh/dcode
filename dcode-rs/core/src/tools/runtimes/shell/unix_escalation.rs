...
use dcode_execpolicy::MatchOptions;
...
use dcode_protocol::protocol::ExecApprovalRequestSkillMetadata;
...
use dcode_shell_command::bash::parse_shell_lc_plain_commands;
use dcode_shell_command::bash::parse_shell_lc_single_command_prefix;
use dcode_shell_escalation::EscalateServer;
use dcode_shell_escalation::EscalationDecision;
use dcode_shell_escalation::EscalationExecution;
use dcode_shell_escalation::EscalationPermissions;
use dcode_shell_escalation::EscalationPolicy;
use dcode_shell_escalation::EscalationSession;
use dcode_shell_escalation::ExecParams;
use dcode_shell_escalation::ExecResult;
use dcode_shell_escalation::Permissions as EscalatedPermissions;
use dcode_shell_escalation::PreparedExec;
use dcode_shell_escalation::ShellCommandExecutor;
use dcode_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::Dcode(err.into()))?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        macos_seatbelt_profile_extensions: ctx
            ...
            .clone(),
        dcode_linux_sandbox_exe: ctx.turn.dcode_linux_sandbox_exe.clone(),
...
struct CoreShellActionProvider {
    ...
    session: Arc<crate::dcode::Session>,
    turn: Arc<crate::dcode::TurnContext>,
    call_id: String,
...
struct CoreShellCommandExecutor {
    ...
    network: Option<dcode_network_proxy::NetworkProxy>,
    ...
    macos_seatbelt_profile_extensions: Option<MacOsSeatbeltProfileExtensions>,
    dcode_linux_sandbox_exe: Option<PathBuf>,
    ...
    fn prepare_sandboxed_exec(
        ...
        let mut exec_request =
            sandbox_manager.transform(crate::sandboxing::SandboxTransformRequest {
                ...
                #[cfg(target_os = "macos")]
                macos_seatbelt_profile_extensions,
                dcode_linux_sandbox_exe: self.dcode_linux_sandbox_exe.as_ref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Dcode(DcodeErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::Dcode(DcodeErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
