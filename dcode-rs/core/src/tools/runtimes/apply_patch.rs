...
//! `dcode --dcode-run-as-apply-patch`, and runs under the current
...
impl ApplyPatchRuntime {
    ...
    fn build_command_spec(
        req: &ApplyPatchRequest,
        _dcode_home: &std::path::Path,
    ) -> Result<CommandSpec, ToolError> {
        let exe = if let Some(path) = &req.dcode_exe {
            path.clone()
        } else {
            #[cfg(target_os = "windows")]
            {
                dcode_windows_sandbox::resolve_current_exe_for_launch(_dcode_home, "dcode.exe")
            }
            #[cfg(not(target_os = "windows"))]
            {
                std::env::current_exe().map_err(|e| {
                    ToolError::Rejected(format!("failed to determine dcode exe: {e}"))
                })?
            }
        };
...
impl ToolRuntime<ApplyPatchRequest, ExecToolCallOutput> for ApplyPatchRuntime {
    async fn run(
        ...
        let spec = Self::build_command_spec(req, &ctx.turn.config.dcode_home)?;
        let env = attempt
            .env_for(spec, /*network*/ None)
            .map_err(|err| ToolError::Dcode(err.into()))?;
        let out = execute_env(env, Self::stdout_stream(ctx))
            .await
            .map_err(ToolError::Dcode)?;
        Ok(out)
    }
}
