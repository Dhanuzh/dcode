...
use dcode_protocol::models::VIEW_IMAGE_TOOL_NAME;
use dcode_protocol::openai_models::ApplyPatchToolType;
...
use dcode_protocol::openai_models::WebSearchToolType;
...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams) -> Self {
        ...
        let include_artifact_tools =
            features.enabled(Feature::Artifact) && dcode_artifacts::can_manage_artifact_runtime();
        ...
        let shell_type = if !features.enabled(Feature::ShellTool) {
            ...
            if dcode_utils_pty::conpty_supported() {
...
fn create_wait_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: format!(
            ...
            dcode_code_mode::build_wait_tool_description().trim()
        ),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_test_sync_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Internal synchronization helper used by Dcode integration tests.".to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
            properties,
...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        ...
        description: "Runs JavaScript in a persistent Node kernel with top-level await. This is a freeform tool: send raw JavaScript source text, optionally with a first-line pragma like `// dcode-js-repl: timeout_ms=15000`; do not send JSON/quotes/markdown fences."
            .to_string(),
...
PRAGMA_LINE: /[ \t]*\/\/ dcode-artifact-tool:[^\r\n]*/
NEWLINE: /\r?\n/
PLAIN_JS_SOURCE: /(?:\s*)(?:[^\s{\"`]|`[^`]|``[^`])[\s\S]*/
JS_SOURCE: /(?:\s*)(?:[^\s{\"`]|`[^`]|``[^`])[\s\S]*/
"#;
...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        ...
        description: dcode_code_mode::build_exec_tool_description(
            enabled_tools,
    ...
    if config.code_mode_enabled {
        ...
        let mut enabled_tools = nested_specs
            ...
            .filter_map(|spec| {
                ...
                dcode_code_mode::is_code_mode_nested_tool(&name).then_some((name, description))
            })
            .collect::<Vec<_>>();
