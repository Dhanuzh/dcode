...
fn write_user_skill(dcode_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = dcode_home.path().join("skills").join(dir);
...
fn new_with_disabled_bundled_skills_removes_stale_cached_system_skills() {
    ...
    let stale_system_skill_dir = dcode_home.path().join("skills/.system/stale-skill");
    ...
    let plugins_manager = Arc::new(PluginsManager::new(dcode_home.path().to_path_buf()));
    let _skills_manager =
        SkillsManager::new(dcode_home.path().to_path_buf(), plugins_manager, false);

    assert!(
        !dcode_home.path().join("skills/.system").exists(),
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let plugins_manager = Arc::new(PluginsManager::new(dcode_home.path().to_path_buf()));
    let skills_manager = SkillsManager::new(dcode_home.path().to_path_buf(), plugins_manager, true);

    write_user_skill(&dcode_home, "a", "skill-a", "from a");
    ...
    write_user_skill(&dcode_home, "b", "skill-b", "from b");
...
async fn skills_for_config_disables_plugin_skills_by_name() {
    ...
    fs::write(
        dcode_home.path().join(crate::config::CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = dcode_home.path().join("skills/.system/bundled-skill");
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let skill_dir = dcode_home.path().join("skills").join("demo");
...
"#,
    ...
    let role_path = dcode_home.path().join("enable-role.toml");
    fs::write(
        &role_path,
        format!(
            r#"[[skills.config]]
