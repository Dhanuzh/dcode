...
use dcode_protocol::models::FileSystemPermissions;
use dcode_protocol::models::MacOsAutomationPermission;
use dcode_protocol::models::MacOsContactsPermission;
use dcode_protocol::models::MacOsPreferencesPermission;
use dcode_protocol::models::MacOsSeatbeltProfileExtensions;
use dcode_protocol::models::PermissionProfile;
use dcode_protocol::protocol::Product;
...
const REPO_ROOT_CONFIG_DIR_NAME: &str = ".dcode";

async fn make_config(dcode_home: &TempDir) -> Config {
    make_config_for_cwd(dcode_home, dcode_home.path().to_path_buf()).await
}

async fn make_config_for_cwd(dcode_home: &TempDir, cwd: PathBuf) -> Config {
    let trust_root = cwd
        .ancestors()
...
-> anyhow::Result<()> {
    ...
    let system_folder = tmp.path().join("etc/dcode");
    let home_folder = tmp.path().join("home");
    let user_folder = home_folder.join("dcode");
...
fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
    ...
    fs::create_dir_all(&dot_dcode)?;

    let user_file = AbsolutePathBuf::from_absolute_path(user_folder.join("config.toml"))?;
    let project_dot_dcode = AbsolutePathBuf::from_absolute_path(&dot_dcode)?;
    ...
    let layers = vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_dcode_folder: project_dot_dcode,
            },
            TomlValue::Table(toml::map::Map::new()),
            "marked untrusted",
        ),
    ...
    assert_eq!(
        got,
        vec![
            (SkillScope::Repo, dot_dcode.join("skills")),
            (SkillScope::User, user_folder.join("skills")),
            (
                SkillScope::User,
                home_folder.join(AGENTS_DIR_NAME).join(SKILLS_DIR_NAME)
...
fn write_skill(dcode_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(&dcode_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(dcode_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(
        &dcode_home.path().join("skills/.system"),
        dir,
        name,
        description,
    )
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "dep-skill", "from json");
...
"#,
    ...
    let cfg = make_config(&dcode_home).await;
...
async fn loads_skill_interface_metadata_from_yaml() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::Dcode, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn loads_skill_permissions_from_yaml() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "permissions-skill", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_permissions_do_not_create_profile() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "permissions-empty", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_macos_permissions_from_yaml() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "permissions-macos", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(dcode_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &dcode_home.path().join("skills/shared"));
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    let cycle_dir = dcode_home.path().join("skills/cycle");
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let cfg = make_config_for_cwd(&dcode_home, repo_dir.path().to_path_buf()).await;
    let outcome = load_skills_for_test(&cfg);
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = dcode_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = dcode_home.path().join("skills");
    let outcome = load_skills_from_roots([SkillRoot {
        path: skills_root,
        scope: SkillScope::User,
    }]);
...
async fn loads_valid_skill() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&dcode_home, "demo", "demo-skill", "does things\ncarefully");
    let cfg = make_config(&dcode_home).await;
...
async fn namespaces_plugin_skills_using_plugin_name() {
    ...
    fs::create_dir_all(plugin_root.join(".dcode-plugin")).unwrap();
    fs::write(
        plugin_root.join(".dcode-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )
    .unwrap();
...
async fn skips_hidden_and_invalid() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = dcode_home.path().join("skills/.hidden");
    ...
    let invalid_dir = dcode_home.path().join("skills/invalid");
...
async fn enforces_length_limits() {
    ...
    write_skill(&dcode_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&dcode_home).await;
    ...
    write_skill(&dcode_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg);
    assert_eq!(outcome.skills.len(), 1);
    assert_eq!(outcome.errors.len(), 1);
    assert!(
...
#[tokio::test]
async fn loads_skills_from_agents_dir_without_dcode_dir() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
#[tokio::test]
async fn loads_skills_from_all_dcode_dirs_under_project_root() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
    ...
    let cfg = make_config_for_cwd(&dcode_home, nested_dir).await;
...
#[tokio::test]
async fn loads_skills_from_dcode_dir_when_not_git_repo() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    ...
    let cfg = make_config_for_cwd(&dcode_home, work_dir.path().to_path_buf()).await;
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&dcode_home, "user", "dupe-skill", "from user");
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_dcode_dirs() {
    let dcode_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&dcode_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&dcode_home, file_path).await;
...
async fn loads_skills_from_system_cache_when_present() {
    ...
    let skill_path = write_system_skill(&dcode_home, "system", "system-skill", "from system");
