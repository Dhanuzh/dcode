...
/// This is the single entrypoint that `dcode` uses to trigger memory startup.
///
...
mod phase_one {
    ...
    pub(super) const MODEL: &str = "gpt-5.1-dcode-mini";
...
mod phase_two {
    ...
    pub(super) const MODEL: &str = "gpt-5.3-dcode";
...
mod metrics {
    ...
    pub(super) const MEMORY_PHASE_ONE_JOBS: &str = "dcode.memory.phase1";
    /// End-to-end latency for a single phase-1 startup run.
    pub(super) const MEMORY_PHASE_ONE_E2E_MS: &str = "dcode.memory.phase1.e2e_ms";
    /// Number of raw memories produced by phase-1 startup extraction.
    pub(super) const MEMORY_PHASE_ONE_OUTPUT: &str = "dcode.memory.phase1.output";
    /// Histogram for aggregate token usage across one phase-1 startup run.
    pub(super) const MEMORY_PHASE_ONE_TOKEN_USAGE: &str = "dcode.memory.phase1.token_usage";
    /// Number of phase-2 startup jobs grouped by status.
    pub(super) const MEMORY_PHASE_TWO_JOBS: &str = "dcode.memory.phase2";
    /// End-to-end latency for a single phase-2 consolidation run.
    pub(super) const MEMORY_PHASE_TWO_E2E_MS: &str = "dcode.memory.phase2.e2e_ms";
    /// Number of stage-1 memories included in each phase-2 consolidation step.
    pub(super) const MEMORY_PHASE_TWO_INPUT: &str = "dcode.memory.phase2.input";
    /// Histogram for aggregate token usage across one phase-2 consolidation run.
    pub(super) const MEMORY_PHASE_TWO_TOKEN_USAGE: &str = "dcode.memory.phase2.token_usage";
}

use std::path::Path;
use std::path::PathBuf;

pub fn memory_root(dcode_home: &Path) -> PathBuf {
    dcode_home.join("memories")
}
