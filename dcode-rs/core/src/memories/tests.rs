...
fn memory_root_uses_shared_global_path() {
    ...
    let dcode_home = dir.path().join("dcode");
    assert_eq!(memory_root(&dcode_home), dcode_home.join("memories"));
}
...
keywords: dcode-state, migrations
---
### Task 1: migration-test
task: add-migration-test
task_group: dcode-state
...
- Confirmed no ordering regressions."
    ...
    assert!(raw_memories.contains("task_group: dcode-state"));
...
mod phase2 {
    ...
    use dcode_state::Phase2JobClaimOutcome;
    use dcode_state::Stage1Output;
    ...
    struct DispatchHarness {
        ...
        state_db: Arc<dcode_state::StateRuntime>,
    }

    impl DispatchHarness {
        async fn new() -> Self {
            let dcode_home = tempfile::tempdir().expect("create temp dcode home");
            let mut config = test_config();
            config.dcode_home = dcode_home.path().to_path_buf();
            config.cwd = config.dcode_home.clone();
            let config = Arc::new(config);

            let state_db = dcode_state::StateRuntime::init(
    ...
    async fn dispatch_reclaims_stale_global_lock_and_starts_consolidation() {
        ...
        pretty_assertions::assert_eq!(config_snapshot.cwd, memory_root(&harness.config.dcode_home));
        match config_snapshot.sandbox_policy {
            SandboxPolicy::WorkspaceWrite { writable_roots, .. } => {
                assert!(
                    writable_roots
                        .iter()
                        .any(|root| root.as_path() == harness.config.dcode_home.as_path()),
                    "consolidation subagent should have dcode_home as writable root"
                );
            }
            other => panic!("unexpected sandbox policy: {other:?}"),
        }
        subagent.dcode.session.ensure_rollout_materialized().await;
        subagent.dcode.session.flush_rollout().await;
    ...
    async fn dispatch_with_empty_stage1_outputs_rebuilds_local_artifacts() {
        let harness = DispatchHarness::new().await;
        let root = memory_root(&harness.config.dcode_home);
    ...
    async fn dispatch_marks_job_for_retry_when_spawn_agent_fails() {
        ...
        let mut metadata_builder = ThreadMetadataBuilder::new(
            thread_id,
            config.dcode_home.join(format!("rollout-{thread_id}.jsonl")),
            Utc::now(),
            SessionSource::Cli,
        );
