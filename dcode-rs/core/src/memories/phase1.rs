...
use dcode_secrets::redact_secrets;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<dcode_state::Stage1JobClaim>> {
    ...
    match state_db
        .claim_stage1_jobs_for_startup(
            ...
            dcode_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    session: &Arc<Session>,
    claimed_candidates: Vec<dcode_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(in crate::memories) async fn run(
        session: &Session,
        claim: dcode_state::Stage1JobClaim,
        stage_one_context: &RequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        ...
        serde_json::to_string(&filtered).map_err(|err| {
            DcodeErr::InvalidRequest(format!("failed to serialize rollout memory: {err}"))
        })
    }
