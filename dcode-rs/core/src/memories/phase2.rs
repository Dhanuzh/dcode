...
pub(super) async fn run(session: &Arc<Session>, config: Arc<Config>) {
    ...
    let root = memory_root(&config.dcode_home);
...
fn artifact_memories_for_phase2(
    selection: &dcode_state::Phase2InputSelection,
) -> Vec<Stage1Output> {
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            dcode_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
            ...
            dcode_state::Phase2JobClaimOutcome::SkippedNotDirty => return Err("skipped_not_dirty"),
            dcode_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[dcode_state::Stage1Output],
        reason: &'static str,
    ) {
        session.services.session_telemetry.counter(
            metrics::MEMORY_PHASE_TWO_JOBS,
...
mod agent {
    ...
    pub(super) fn get_config(config: Arc<Config>) -> Option<Config> {
        ...
        match AbsolutePathBuf::from_absolute_path(agent_config.dcode_home.clone()) {
            Ok(dcode_home) => writable_roots.push(dcode_home),
            Err(err) => warn!(
                "memory phase-2 consolidation could not add dcode_home writable root {}: {err}",
                agent_config.dcode_home.display()
            ),
        }
        // The consolidation agent only needs local dcode_home write access and no network.
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<dcode_state::Stage1Output>,
        thread_id: ThreadId,
        phase_two_e2e_timer: Option<dcode_otel::Timer>,
    ) {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[dcode_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
