...
#[test]
fn dcode_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        CODEX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_dcode_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        CODEX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn dcode_apps_tool_question_without_elicitation_omits_always_allow() {
...
#[test]
fn dcode_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: CODEX_APPS_MCP_SERVER_NAME.to_string(),
...
#[tokio::test]
async fn dcode_apps_tool_call_request_meta_includes_turn_metadata_and_dcode_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = serde_json::from_str::<serde_json::Value>(
        &turn_context
            .turn_metadata_state
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        dcode_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_dcode_apps metadata should be an object"),
        ),
    };
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_dcode_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            CODEX_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_dcode_app_tool_approval_writes_tool_override() {
    let tmp = tempdir().expect("tempdir");

    persist_dcode_app_tool_approval(tmp.path(), "calendar", "calendar/list_events")
        .await
...
async fn approve_mode_blocks_when_arc_returns_interrupt_for_model() {
    ...
    turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
        crate::DcodeAuth::create_dummy_chatgpt_auth_for_testing(),
    ));
