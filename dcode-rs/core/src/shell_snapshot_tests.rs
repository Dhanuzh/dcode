...
fn bash_snapshot_filters_invalid_exports() -> Result<()> {
    let output = Command::new("/bin/bash")
        ...
        .env("VALID_NAME", "ok")
        .env("PWD", "/tmp/stale")
        .env("NEXTEST_BIN_EXE_dcode-write-config-schema", "/path/to/bin")
        .env("BAD-NAME", "broken")
        .output()?;
    ...
    assert!(!stdout.contains("NEXTEST_BIN_EXE_dcode-write-config-schema"));
...
async fn write_rollout_stub(dcode_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
    let dir = dcode_home
        .join("sessions")
        .join("2025")
        .join("01")
...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    let dir = tempdir()?;
    let dcode_home = dir.path();
    let snapshot_dir = dcode_home.join(SNAPSHOT_DIR);
...
async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(dcode_home, stale_session).await?;
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(dcode_home, active_session).await?;
    fs::write(&active_snapshot, "active").await?;

    set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

    cleanup_stale_snapshots(dcode_home, active_session).await?;
