...
This crate implements the business logic for Dcode. It is designed to be used by the various Dcode UIs written in Rust.

## Dependencies

Note that `dcode-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.dcode` read-only.
...
Expects the binary containing `dcode-core` to run the equivalent of `dcode sandbox linux` (legacy alias: `dcode debug landlock`) when `arg0` is `dcode-linux-sandbox`. See the `dcode-arg0` crate for details.
...
missing or too old to support the required flags, Dcode also surfaces a startup
...
Expects the binary containing `dcode-core` to simulate the virtual `apply_patch` CLI when `arg1` is `--dcode-run-as-apply-patch`. See the `dcode-arg0` crate for details.
