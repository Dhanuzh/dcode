...
async fn js_repl_is_not_advertised_when_startup_node_is_incompatible() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let warning = wait_for_event_match(&test.dcode, |event| match event {
...
async fn js_repl_keeps_function_to_string_stable() -> Result<()> {
    ...
    assert!(!output.contains("__dcodeInternalMarkCommittedBindings"));
...
async fn js_repl_can_invoke_builtin_tools() -> Result<()> {
    ...
    let mock = run_js_repl_turn(
        ...
        &[(
            "call-1",
            "const toolOut = await dcode.tool(\"list_mcp_resources\", {}); console.log(toolOut.type);",
        )],
    )
    .await?;
...
async fn js_repl_tool_call_rejects_recursive_js_repl_invocation() -> Result<()> {
    ...
    let mock = run_js_repl_turn(
        ...
        &[(
            ...
            r#"
try {
  await dcode.tool("js_repl", "console.log('recursive')");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn js_repl_exposes_dcode_path_helpers() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = responses::start_mock_server().await;
    let mock = run_js_repl_turn(
        &server,
        "check dcode path helpers",
        &[(
            "call-1",
            "console.log(`cwd:${typeof dcode.cwd}:${dcode.cwd.length > 0}`); console.log(`home:${dcode.homeDir === null || typeof dcode.homeDir === \"string\"}`);",
        )],
    )
    .await?;
