...
) -> Result<(TestDcode, ResponseMock)> {
    let mut builder = test_dcode()
        .with_model("test-gpt-5.1-dcode")
...
"#;
    ...
    let turn_id = wait_for_event_match(&test.dcode, |event| match event {
        EventMsg::TurnStarted(event) => Some(event.turn_id.clone()),
        _ => None,
    })
    .await;
    let request = wait_for_event_match(&test.dcode, |event| match event {
    ...
    test.dcode
        ...
        .await?;
    wait_for_event(&test.dcode, |event| match event {
