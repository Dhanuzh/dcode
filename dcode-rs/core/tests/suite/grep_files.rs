...
const MODEL_WITH_TOOL: &str = "test-gpt-5.1-dcode";
...
async fn grep_files_tool_collects_matches() -> Result<()> {
    ...
    let test = build_test_dcode(&server).await?;
...
async fn build_test_dcode(server: &wiremock::MockServer) -> Result<TestDcode> {
    let mut builder = test_dcode().with_model(MODEL_WITH_TOOL);
