...
use dcode_test_macros::large_stack_test;
...
pub async fn apply_patch_harness() -> Result<TestDcodeHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestDcodeBuilder) -> TestDcodeBuilder,
) -> Result<TestDcodeHarness> {
    let builder = configure(test_dcode()).with_config(|config| {
