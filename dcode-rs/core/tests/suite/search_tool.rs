...
const CALENDAR_CREATE_TOOL: &str = "mcp__dcode_apps__calendar_create_event";
const CALENDAR_LIST_TOOL: &str = "mcp__dcode_apps__calendar_list_events";
const SEARCH_CALENDAR_NAMESPACE: &str = "mcp__dcode_apps__calendar";
...
fn configure_apps(config: &mut Config, apps_base_url: &str) {
    ...
    config.model = Some("gpt-5-dcode".to_string());
    ...
    let model = model_catalog
        .models
        .iter_mut()
        .find(|model| model.slug == "gpt-5-dcode")
        .expect("gpt-5-dcode exists in bundled models.json");
    model.supports_search_tool = true;
    config.model_catalog = Some(model_catalog);
}

fn configured_builder(apps_base_url: String) -> TestDcodeBuilder {
    test_dcode()
...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    let EventMsg::McpToolCallEnd(end) = wait_for_event(&test.dcode, |event| {
    ...
    let apps_tool_call = server
        ...
        .find_map(|request| {
            ...
            (request.url.path() == "/api/dcode/apps"
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/_dcode_apps"),
        Some(&json!({
            "resource_uri": CALENDAR_CREATE_EVENT_RESOURCE_URI,
            "contains_mcp_source": true,
            "connector_id": "calendar",
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-dcode-turn-metadata/session_id"),
        Some(&json!(test.session_configured.session_id.to_string()))
    );
    assert!(
        apps_tool_call
            .pointer("/params/_meta/x-dcode-turn-metadata/turn_id")
            .and_then(Value::as_str)
