...
const REMOTE_MODEL_SLUG: &str = "dcode-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 Dcode".to_string(),
    ...
    let config = load_default_config_for_test(&dcode_home).await;
    ...
    let manager = dcode_core::test_support::models_manager_with_provider(
        dcode_home.path().to_path_buf(),
        dcode_core::test_support::auth_manager_from_auth(auth),
        provider,
    );

    manager.list_models(RefreshStrategy::OnlineIfUncached).await;

    let model_info = manager.get_model_info("gpt-5.3-dcode-test", &config).await;

    assert_eq!(model_info.slug, "gpt-5.3-dcode-test");
...
async fn remote_models_long_model_slug_is_sent_with_high_reasoning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-dcode-test";
    let prefix_model = "gpt-5.3-dcode";
...
async fn namespaced_model_slug_uses_catalog_metadata_without_fallback_warning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "custom/gpt-5.2-dcode";
    let response_mock = mount_sse_once(
        &server,
        sse(vec![ev_response_created("resp-1"), ev_completed("resp-1")]),
    )
    ...
    loop {
        let event = wait_for_event(&dcode, |_| true).await;
        match event {
...
async fn remote_models_remote_model_uses_unified_exec() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&dcode, |msg| match msg {
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "dcode-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "dcode-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let remote_model = test_remote_model("dcode-auto-balanced", ModelVisibility::Hide, 0);
    let models_mock = mount_models_once(
        &server,
        ModelsResponse {
            models: vec![remote_model],
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "dcode-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    dcode_core::test_support::all_model_presets()
        .iter()
