...
async fn undo_harness() -> Result<TestDcodeHarness> {
    let builder = test_dcode().with_model("gpt-5.1").with_config(|config| {
    ...
    TestDcodeHarness::with_builder(builder).await
}
...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "Dcode Tests"])?;
    git(path, &["config", "user.email", "dcode-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by Dcode.\n")?;
...
async fn invoke_undo(dcode: &Arc<DcodeThread>) -> Result<UndoCompletedEvent> {
    dcode.submit(Op::Undo).await?;
    let event = wait_for_event_match(dcode, |msg| match msg {
...
async fn expect_successful_undo(dcode: &Arc<DcodeThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(dcode).await?;
    assert!(
        event.success,
...
async fn expect_failed_undo(dcode: &Arc<DcodeThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(dcode).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let dcode = Arc::clone(&harness.test().dcode);
    let completed = expect_successful_undo(&dcode).await?;
...
async fn undo_sequential_turns_consumes_snapshots() -> Result<()> {
    ...
    let dcode = Arc::clone(&harness.test().dcode);
    expect_successful_undo(&dcode).await?;
    assert_eq!(fs::read_to_string(&story)?, "turn two\n");
    ...
    expect_failed_undo(&dcode).await?;
