...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    dcode
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _begin = wait_for_event_match(dcode, |ev| match ev {
    ...
    .await;
    dcode.submit(Op::Interrupt).await.unwrap();
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.dcode, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.dcode.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_dcode::test_dcode().with_config(move |config| {
        config
            .features
    ...
    let begin_event = wait_for_event_match(&test.dcode, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.dcode, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.dcode, |ev| match ev {
    ...
    let _ = wait_for_event(&test.dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_dcode::test_dcode().with_config(|config| {
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_dcode::test_dcode();
    let test = builder
        .with_config(|config| {
