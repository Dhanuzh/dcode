...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.dcode, |ev| {
    ...
    let end_event = wait_for_event(&fixture.dcode, |ev| {
    ...
    wait_for_event(&fixture.dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn stdio_image_responses_round_trip() -> anyhow::Result<()> {
    ...
    loop {
        fixture.dcode.submit(Op::ListMcpTools).await?;
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: dcode_protocol::openai_models::ReasoningEffort::Medium,
    ...
    wait_for_event(&fixture.dcode, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    wait_for_event(&fixture.dcode, |ev| {
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    let _dcode_home_guard = EnvVarGuard::set("DCODE_HOME", temp_home.path().as_os_str());
