...
async fn memories_startup_phase2_tracks_added_and_removed_inputs_across_runs() -> Result<()> {
    ...
    let first = build_test_dcode(&server, home.clone()).await?;
    ...
    shutdown_test_dcode(&first).await?;
    ...
    let second = build_test_dcode(&server, home.clone()).await?;
    ...
    shutdown_test_dcode(&second).await?;
    Ok(())
}
...
async fn web_search_pollution_moves_selected_thread_into_removed_phase2_inputs() -> Result<()> {
    ...
    let mut initial_builder = test_dcode().with_home(home.clone()).with_config(|config| {
        config
            .features
    ...
    shutdown_test_dcode(&initial).await?;
    ...
    let mut resumed_builder = test_dcode().with_home(home.clone()).with_config(|config| {
        config
            .features
            .enable(Feature::Sqlite)
            .expect("test config should allow feature update");
    ...
    shutdown_test_dcode(&resumed).await?;
    Ok(())
}

async fn build_test_dcode(server: &wiremock::MockServer, home: Arc<TempDir>) -> Result<TestDcode> {
    #[allow(clippy::expect_used)]
    let mut builder = test_dcode().with_home(home).with_config(|config| {
        config
            .features
            .enable(Feature::Sqlite)
            .expect("test config should allow feature update");
...
async fn init_state_db(home: &Arc<TempDir>) -> Result<Arc<dcode_state::StateRuntime>> {
    let db =
        dcode_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into()).await?;
    db.mark_backfill_complete(None).await?;
    Ok(db)
}

async fn seed_stage1_output(
    db: &dcode_state::StateRuntime,
    dcode_home: &Path,
    ...
    let mut metadata_builder = dcode_state::ThreadMetadataBuilder::new(
        thread_id,
        dcode_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = dcode_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        dcode_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_dcode(test: &TestDcode) -> Result<()> {
    test.dcode.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.dcode, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}
