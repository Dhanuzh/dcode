...
use dcode_core::auth::DCODE_API_KEY_ENV_VAR;
...
fn repo_root() -> std::path::PathBuf {
    ...
    dcode_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = dcode_utils_cargo_bin::cargo_bin("dcode").unwrap();
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Dcode to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `dcode exec --profile ...` preserves the active profile when it
...
/// 2. Configures dcode to read from this fixture via CODEX_RS_SSE_FIXTURE env var
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the dcode CLI and invoke `exec`, which is what records a session.
    let bin = dcode_utils_cargo_bin::cargo_bin("dcode").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        ...
        .arg(&prompt);
    cmd.env("DCODE_HOME", home.path())
        .env(DCODE_API_KEY_ENV_VAR, "dummy")
        .env("CODEX_RS_SSE_FIXTURE", &fixture);

    let output = cmd.output().unwrap();
    assert!(
        output.status.success(),
        "dcode-cli exec failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let bin2 = dcode_utils_cargo_bin::cargo_bin("dcode").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("DCODE_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("CODEX_RS_SSE_FIXTURE", &fixture);

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume dcode-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = dcode_core::git_info::collect_git_info(&git_repo).await;
