...
const REMOTE_MODEL: &str = "dcode-test-ttl";
const VERSIONED_MODEL: &str = "dcode-test-versioned";
const MISSING_VERSION_MODEL: &str = "dcode-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "dcode-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let dcode = Arc::clone(&test.dcode);
    ...
    let cache_path = config.dcode_home.join(CACHE_FILE);
    ...
    let _ = wait_for_event(&dcode, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    builder = builder
        .with_pre_build_hook(move |home| {
            let cache = ModelsCache {
                fetched_at: Utc::now(),
                etag: None,
                client_version: Some(dcode_core::models_manager::client_version_to_whole()),
...
async fn refreshes_when_cache_version_differs() -> Result<()> {
    ...
    builder = builder
        .with_pre_build_hook(move |home| {
            let client_version = dcode_core::models_manager::client_version_to_whole();
            let cache = ModelsCache {
                fetched_at: Utc::now(),
                etag: None,
