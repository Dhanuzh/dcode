...
use dcode_protocol::items::parse_hook_prompt_fragment;
...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let hook_inputs = read_stop_hook_inputs(test.dcode_home_path())?;
    ...
    let rollout_path = test.dcode.rollout_path().expect("rollout path");
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.dcode_home_path())?;
...
async fn resumed_thread_keeps_stop_continuation_prompt_in_history() -> Result<()> {
    ...
    let mut resume_builder = test_dcode().with_config(|config| {
        config
            .features
            .enable(Feature::DcodeHooks)
            .expect("test config should allow feature update");
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.dcode_home_path())?;
...
async fn pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.dcode_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn pre_tool_use_blocks_local_shell_before_execution() -> Result<()> {
    ...
    assert!(
        output.contains(&format!(
            "Command: {}",
            dcode_shell_command::parse_command::shlex_join(&command)
        )),
...
async fn pre_tool_use_does_not_fire_for_non_shell_tools() -> Result<()> {
    ...
    let hook_log_path = test.dcode_home_path().join("pre_tool_use_hook_log.jsonl");
    assert!(
