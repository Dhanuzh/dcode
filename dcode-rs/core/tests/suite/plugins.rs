...
fn write_sample_plugin_manifest_and_config(home: &TempDir) -> std::path::PathBuf {
    ...
    std::fs::create_dir_all(plugin_root.join(".dcode-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
async fn build_plugin_test_dcode(
    server: &MockServer,
    dcode_home: Arc<TempDir>,
) -> Result<Arc<dcode_core::DcodeThread>> {
...
async fn build_analytics_plugin_test_dcode(
    server: &MockServer,
    dcode_home: Arc<TempDir>,
) -> Result<Arc<dcode_core::DcodeThread>> {
...
async fn build_apps_enabled_plugin_test_dcode(
    server: &MockServer,
    dcode_home: Arc<TempDir>,
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let dcode_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(dcode_home.as_ref());
    write_plugin_app_plugin(dcode_home.as_ref());
    let dcode = build_apps_enabled_plugin_test_dcode(
        &server,
        Arc::clone(&dcode_home),
    ...
    dcode
        .submit(Op::UserInput {
            items: vec![dcode_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(dcode_home.as_ref());
    write_plugin_mcp_plugin(dcode_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(dcode_home.as_ref());

    let dcode =
        build_apps_enabled_plugin_test_dcode(&server, dcode_home, apps_server.chatgpt_base_url)
            .await?;

    dcode
        .submit(Op::UserInput {
            items: vec![dcode_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__dcode_apps__google_calendar_create_event"),
    ...
    let calendar_description = tool_description(
        &request_body,
        "mcp__dcode_apps__google_calendar_create_event",
    )
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let dcode_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(dcode_home.as_ref());
    let dcode = build_analytics_plugin_test_dcode(&server, dcode_home).await?;
    ...
    let analytics_request = loop {
        ...
        if let Some(request) = requests
            .into_iter()
            .find(|request| request.url.path() == "/dcode/analytics-events/events")
        {
            break request;
        }
    ...
    assert_eq!(event["event_type"], "dcode_plugin_used");
    ...
    assert_eq!(
        ...
        serde_json::json!(dcode_core::default_client::originator().value)
    );
...
async fn plugin_mcp_tools_are_listed() -> Result<()> {
    ...
    write_plugin_mcp_plugin(dcode_home.as_ref(), &rmcp_test_server_bin);
    let dcode = build_plugin_test_dcode(&server, dcode_home).await?;

    let tools_ready_deadline = Instant::now() + Duration::from_secs(30);
    loop {
        dcode.submit(Op::ListMcpTools).await?;
