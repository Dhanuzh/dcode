...
fn skill_script_command(test: &TestDcode, script_name: &str) -> Result<(String, String)> {
...
async fn wait_for_exec_approval_request(test: &TestDcode) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.dcode.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestDcode) {
    wait_for_event(test.dcode.as_ref(), |event| {
        ...
        Some(PermissionProfile {
            file_system: Some(FileSystemPermissions {
                read: Some(vec![absolute_path(
                    &test.dcode_home_path().join("skills/mbolin-test-skill/data"),
                )]),
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/dcode-zsh-fork-workspace-write-deny.txt";
    let workspace_write_policy = restrictive_workspace_write_policy();
    let _ = fs::remove_file(outside_path);
    let test = build_zsh_fork_test(
        &server,
