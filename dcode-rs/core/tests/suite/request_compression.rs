...
async fn request_body_is_zstd_compressed_for_dcode_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/dcode/v1", server.uri());
    ...
    let dcode = builder.build(&server).await?.dcode;
