...
use dcode_core::config::types::Personality;
...
async fn model_change_appends_model_instructions_developer_message() -> Result<()> {
    ...
    let mut builder = test_dcode().with_model("gpt-5.2-dcode");
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-dcode-personality";
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_dcode().build(&server).await?;
...
async fn model_change_from_image_to_text_strips_prior_image_content() -> Result<()> {
    ...
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &dcode_protocol::models::image_open_tag_text()),
        "second request should preserve the image open tag text"
    );
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &dcode_protocol::models::image_close_tag_text()),
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.dcode, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.dcode, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.dcode, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
