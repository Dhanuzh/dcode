...
use dcode_core::config::AgentRoleConfig;
...
const INHERITED_MODEL: &str = "gpt-5.2-dcode";
...
async fn wait_for_spawned_thread_id(test: &TestDcode) -> Result<String> {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_dcode::TestDcodeBuilder,
    ) -> core_test_support::test_dcode::TestDcodeBuilder,
) -> Result<(TestDcode, String)> {
    ...
    let mut builder = configure_test(test_dcode().with_config(|config| {
        config
            .features
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.dcode_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
...
async fn spawn_agent_tool_description_mentions_role_locked_settings() -> Result<()> {
    ...
    assert_eq!(
        ...
        "custom: {\nCustom role\n- This role's model is set to `gpt-5.1-dcode-max` and its reasoning effort is set to `high`. These settings cannot be changed.\n}"
    );
