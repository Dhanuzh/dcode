...
async fn submit_user_turn(
    test: &core_test_support::test_dcode::TestDcode,
    prompt: &str,
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    ...
    let mut builder = test_dcode().with_config(|config| {
        let policy_path = config.dcode_home.join("rules").join("policy.rules");
        fs::create_dir_all(
            policy_path
                .parent()
    ...
    let EventMsg::ExecCommandEnd(end) = wait_for_event(&test.dcode, |event| {
