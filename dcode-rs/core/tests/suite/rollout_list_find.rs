...
use chrono::Utc;
use dcode_core::EventPersistenceMode;
use dcode_core::RolloutRecorder;
use dcode_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(dcode_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = dcode_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(dcode_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(dcode_home, "sessions", id)
}

async fn upsert_thread_metadata(dcode_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(dcode_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    ...
    builder.cwd = dcode_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_dcode_home_directory() {
    let repo = TempDir::new().unwrap();
    let dcode_home = repo.path().join(".dcode");
    std::fs::create_dir_all(&dcode_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".dcode/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&dcode_home, id);

    let found = find_thread_path_by_id_str(&dcode_home, &id.to_string())
        .await
        .unwrap();
