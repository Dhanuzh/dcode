...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let mut builder = test_dcode().with_model("gpt-5").with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config
            .features
            .enable(Feature::UnifiedExec)
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&dcode, |msg| match msg {
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&dcode, |_| true).await;
        match msg {
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    loop {
        let event_msg = wait_for_event(&dcode, |_| true).await;
        match event_msg {
...
async fn unified_exec_keeps_long_running_session_after_turn_end() -> Result<()> {
    ...
    wait_for_event(&dcode, |event| matches!(event, EventMsg::ShutdownComplete)).await;
...
async fn unified_exec_interrupt_preserves_long_running_session() -> Result<()> {
    ...
    let _begin_event = wait_for_event_match(&dcode, |msg| match msg {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    ...
    dcode.submit(Op::Interrupt).await?;
    wait_for_event(&dcode, |event| matches!(event, EventMsg::TurnAborted(_))).await;
    ...
    dcode.submit(Op::CleanBackgroundTerminals).await?;
    wait_for_process_exit(&pid).await?;
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = dcode.next_event().await.expect("event");
