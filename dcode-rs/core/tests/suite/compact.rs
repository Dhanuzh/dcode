...
use dcode_core::compact::SUMMARIZATION_PROMPT;
...
async fn assert_compaction_uses_turn_lifecycle_id(dcode: &std::sync::Arc<dcode_core::DcodeThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = dcode.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Dcode.
    let model_provider = non_openai_model_provider(&server);
    let mut builder = test_dcode().with_config(move |config| {
    ...
    dcode.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Dcode to flush rollout entries before inspecting the file.
    dcode.submit(Op::Shutdown).await.unwrap();
...
async fn manual_compact_uses_custom_prompt() {
    ...
    dcode.submit(Op::Compact).await.expect("trigger compact");
    let warning_event = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::Warning(_))).await;
    let EventMsg::Warning(WarningEvent { message }) = warning_event else {
        panic!("expected warning event after compact");
    };
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let dcode = builder.build(&server).await.unwrap().dcode;
    ...
    let first = wait_for_event_match(&dcode, |ev| match ev {
    ...
    let last = wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        dcode_protocol::models::ResponseItem::Message {
            id: None,
        ...
        dcode_protocol::models::ResponseItem::Compaction {
    ...
    let mut resume_builder = test_dcode().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
    });
    ...
    wait_for_event(&resumed.dcode, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let previous_model = "gpt-5.2-dcode";
    let next_model = "gpt-5.1-dcode-max";
    ...
    let test = builder.build(&server).await.expect("build test dcode");
    ...
    test.dcode
        ...
        .await
        .expect("submit first user turn");
    wait_for_event(&test.dcode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.dcode).await;
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    let model_provider = non_openai_model_provider(&server);
    let mut initial_builder = test_dcode()
        .with_auth(DcodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test dcode");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    wait_for_event(&initial.dcode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let model_provider = non_openai_model_provider(&server);
    let mut resumed_builder = test_dcode()
        .with_auth(DcodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.dcode).await;
...
async fn manual_compact_retries_after_context_window_error() {
    ...
    let EventMsg::BackgroundEvent(event) =
        wait_for_event(&dcode, |ev| matches!(ev, EventMsg::BackgroundEvent(_))).await
    else {
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&dcode, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&dcode, |event| match event {
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let dcode = builder.build(&server).await.unwrap();

    dcode.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    dcode.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    dcode.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
