...
use dcode_core::config_loader::NetworkConstraints;
use dcode_core::config_loader::NetworkRequirementsToml;
use dcode_core::config_loader::RequirementSource;
use dcode_core::config_loader::Sourced;
use dcode_core::sandboxing::SandboxPermissions;
use dcode_features::Feature;
use dcode_protocol::approvals::NetworkApprovalProtocol;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestDcode) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestDcode, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.dcode, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestDcode) {
    let event = wait_for_event(&test.dcode, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestDcode) {
...
async fn wait_for_spawned_thread(test: &TestDcode) -> Result<Arc<DcodeThread>> {
...
fn scenarios() -> Vec<ScenarioSpec> {
    ...
    vec![
        ...
        ScenarioSpec {
            ...
            sandbox_permissions: SandboxPermissions::UseDefault,
            features: vec![],
            model_override: Some("gpt-5.1-dcode"),
            outcome: Outcome::Auto,
            expectation: Expectation::PatchApplied {
                target: TargetPath::Workspace("apply_patch_function.txt"),
                content: "function-apply-patch",
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let mut builder = test_dcode().with_model(model).with_config(move |config| {
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/dcode-fallback-rule-test.txt && echo hello > /tmp/dcode-fallback-rule-test.txt";
...
"#,
    ...
    let sandbox_policy_for_config = sandbox_policy.clone();
    let mut builder = test_dcode().with_home(home).with_config(move |config| {
        config.permissions.approval_policy = Constrained::allow_any(approval_policy);
        config.permissions.sandbox_policy = Constrained::allow_any(sandbox_policy_for_config);
        let layers = config
            .config_layer_stack
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://dcode-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
