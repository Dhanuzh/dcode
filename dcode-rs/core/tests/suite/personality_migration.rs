...
use dcode_core::SESSIONS_SUBDIR;
use dcode_core::config::ConfigToml;
use dcode_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use dcode_core::personality_migration::PersonalityMigrationStatus;
use dcode_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(dcode_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(dcode_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(dcode_home: &Path) -> io::Result<()> {
...
async fn write_archived_session_with_user_event(dcode_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = dcode_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(dcode_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = dcode_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
...
async fn no_marker_sessions_preserves_existing_config_fields() -> io::Result<()> {
    let temp = TempDir::new()?;
    write_session_with_user_event(temp.path()).await?;
    tokio::fs::write(temp.path().join("config.toml"), "model = \"gpt-5-dcode\"\n").await?;
    ...
    assert_eq!(status, PersonalityMigrationStatus::Applied);
    let persisted = read_config_toml(temp.path()).await?;
    assert_eq!(persisted.model, Some("gpt-5-dcode".to_string()));
    assert_eq!(persisted.personality, Some(Personality::Pragmatic));
    Ok(())
}
