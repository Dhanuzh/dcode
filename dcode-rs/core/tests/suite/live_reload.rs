...
fn enable_trusted_project(config: &mut dcode_core::config::Config) {
...
async fn submit_skill_turn(test: &TestDcode, skill_path: PathBuf, prompt: &str) -> Result<()> {
...
async fn live_skills_reload_refreshes_skill_cache_after_skill_change() -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(test.dcode_home_path().join("skills/demo/SKILL.md"))?;
    ...
    write_skill(test.dcode_home_path(), "demo", "demo skill", skill_v2);

    let saw_skills_update = timeout(Duration::from_secs(5), async {
        loop {
            match test.dcode.next_event().await {
                Ok(event) => {
