...
async fn dcode_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    dcode_returns_json_result("gpt-5.1".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn dcode_returns_json_result_for_gpt5_dcode() -> anyhow::Result<()> {
    dcode_returns_json_result("gpt-5.1-dcode".to_string()).await
}

async fn dcode_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("dcode_output_schema".into()))
    ...
    let TestDcode { dcode, cwd, .. } = test_dcode().build(&server).await?;
    ...
    let message = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
