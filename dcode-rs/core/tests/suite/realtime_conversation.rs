...
use dcode_core::auth::OPENAI_API_KEY_ENV_VAR;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Dcode.";
...
async fn seed_recent_thread(
    ...
    let db = test.dcode.state_db().context("state db enabled")?;
    let thread_id = ThreadId::new();
    let updated_at = Utc::now();
    let mut metadata_builder = dcode_state::ThreadMetadataBuilder::new(
        thread_id,
        test.dcode_home_path()
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.dcode, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.dcode, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.dcode, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.dcode.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.dcode, |msg| match msg {
...
async fn conversation_start_uses_openai_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    ...
    test.dcode.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.dcode, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.dcode, |msg| match msg {
        EventMsg::Error(err) => Some(err.clone()),
        _ => None,
    })
    .await;
    assert_eq!(err.dcode_error_info, Some(DcodeErrorInfo::BadRequest));
...
async fn conversation_start_preflight_failure_emits_realtime_error_only() -> Result<()> {
    ...
    let closed = timeout(Duration::from_millis(200), async {
        wait_for_event_match(&test.dcode, |msg| match msg {
            EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
            _ => None,
        })
        .await
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.dcode
        ...
        .await?;
    let _ = wait_for_event_match(&test.dcode, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_experimental_realtime_ws_base_url_override() -> Result<()> {
    ...
    let mut builder = test_dcode().with_config({
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("dcode-rs/core"))?;
    ...
    assert!(startup_context.contains("dcode-rs/"));
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_dcode().with_model("gpt-5.1").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
        }
