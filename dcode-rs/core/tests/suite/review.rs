...
use dcode_core::REVIEW_PROMPT;
use dcode_core::config::Config;
use dcode_core::review_format::render_review_output_text;
use dcode_protocol::models::ContentItem;
use dcode_protocol::models::ResponseItem;
use dcode_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use dcode_protocol::protocol::EventMsg;
use dcode_protocol::protocol::ExitedReviewModeEvent;
use dcode_protocol::protocol::Op;
use dcode_protocol::protocol::ReviewCodeLocation;
use dcode_protocol::protocol::ReviewFinding;
use dcode_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Dcode sandbox network restrictions.
    skip_if_no_network!();
    ...
    let dcode = new_conversation_for_server(&server, dcode_home.clone(), |_| {}).await;
    ...
    let _entered = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = dcode.rollout_path().expect("rollout path");
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&dcode, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let dcode = new_conversation_for_server(&server, dcode_home.clone(), |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&dcode, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = dcode_home.path().join("resume.jsonl");
    {
        ...
        let user = dcode_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = dcode_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let dcode =
        resume_conversation_for_server(&server, dcode_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let dcode = new_conversation_for_server(&server, dcode_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, dcode_home, resume_path)
        .await
