...
fn absolute_path(path: &Path) -> anyhow::Result<dcode_utils_absolute_path::AbsolutePathBuf> {
    dcode_utils_absolute_path::AbsolutePathBuf::try_from(path.to_path_buf())
...
async fn create_workspace_directory(test: &TestDcode, rel_path: &str) -> anyhow::Result<PathBuf> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn view_image_tool_can_preserve_original_resolution_when_requested_on_gpt5_3_dcode()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let mut builder = test_dcode()
        .with_model("gpt-5.3-dcode")
        .with_config(|config| {
            config
                .features
...
async fn view_image_tool_resizes_when_model_lacks_original_detail_support() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_dcode().with_model("gpt-5.2").with_config(|config| {
        config
            .features
            .enable(Feature::ImageDetailOriginal)
            .expect("test config should allow feature update");
...
const fs = await import("node:fs/promises");
const path = await import("node:path");
const imagePath = path.join(dcode.tmpDir, "js-repl-view-image-no-emit.png");
const png = Buffer.from(
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==",
  "base64"
);
...
async fn view_image_tool_returns_unsupported_message_for_text_only_model() -> anyhow::Result<()> {
    ...
    let test = builder.build_remote_aware(&server).await?;
    let TestDcode { dcode, config, .. } = &test;
