...
fn image_generation_artifact_path(dcode_home: &Path, session_id: &str, call_id: &str) -> PathBuf {
...
async fn user_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let TestDcode { dcode, .. } = test_dcode().build(&server).await?;
    ...
    let started_item = wait_for_event_match(&dcode, |ev| match ev {
    ...
    .await;
    let completed_item = wait_for_event_match(&dcode, |ev| match ev {
    ...
    let legacy_message = wait_for_event_match(&dcode, |ev| match ev {
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let started = wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
    ...
    .await;
    let completed = wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
            item: TurnItem::AgentMessage(item),
            ..
        }) => Some(item.clone()),
    ...
    let Some(dcode_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {
...
async fn web_search_item_is_emitted() -> anyhow::Result<()> {
    ...
    let begin = wait_for_event_match(&dcode, |ev| match ev {
...
async fn image_generation_call_event_is_emitted() -> anyhow::Result<()> {
    ...
    .await;
    let end = wait_for_event_match(&dcode, |ev| match ev {
...
async fn agent_message_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let (started_turn_id, started_item) = wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            turn_id,
            item: TurnItem::AgentMessage(item),
            ..
    ...
    let delta_event = wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::AgentMessageContentDelta(event) => Some(event.clone()),
        _ => None,
    })
    .await;
    let legacy_delta = wait_for_event_match(&dcode, |ev| match ev {
...
async fn plan_mode_emits_plan_item_from_proposed_plan_block() -> anyhow::Result<()> {
    ...
    dcode
        .submit(Op::UserTurn {
            ...
            approval_policy: dcode_protocol::protocol::AskForApproval::Never,
    ...
    let plan_delta = wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::PlanDelta(event) => Some(event.clone()),
        _ => None,
    })
    .await;

    let plan_completed = wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
...
async fn plan_mode_strips_plan_from_agent_messages() -> anyhow::Result<()> {
    ...
    while plan_delta.is_none() || agent_item.is_none() || plan_item.is_none() {
        let ev = wait_for_event(&dcode, |_| true).await;
        match ev {
...
async fn reasoning_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let reasoning_item = wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.clone()),
