...
use dcode_core::compact::SUMMARY_PREFIX;
...
use core_test_support::test_dcode::TestDcodeBuilder;
use core_test_support::test_dcode::TestDcodeHarness;
...
fn remote_realtime_test_dcode_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestDcodeBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_dcode()
        .with_auth(DcodeAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(dcode: &dcode_core::DcodeThread) -> Result<()> {
    dcode
    ...
    wait_for_event_match(dcode, |msg| match msg {
...
async fn close_realtime_conversation(dcode: &dcode_core::DcodeThread) -> Result<()> {
    dcode.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(dcode, |msg| match msg {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestDcodeHarness::with_builder(
        test_dcode().with_auth(DcodeAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let dcode = harness.test().dcode.clone();
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&dcode, |event| match event {
...
async fn remote_compact_trims_function_call_history_to_fit_context_window() -> Result<()> {
    ...
    let harness = TestDcodeHarness::with_builder(
        test_dcode()
            .with_auth(DcodeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&dcode, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestDcodeHarness::with_builder(
        test_dcode()
            .with_auth(DcodeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_dcode = baseline_harness.test().dcode.clone();
    ...
    baseline_dcode
        ...
        .await?;
    wait_for_event(&baseline_dcode, |event| {
    ...
    baseline_dcode.submit(Op::Compact).await?;
    wait_for_event(&baseline_dcode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestDcodeHarness::with_builder(
        test_dcode()
            .with_auth(DcodeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_dcode = override_harness.test().dcode.clone();
    ...
    override_dcode
        ...
        .await?;
    wait_for_event(&override_dcode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_dcode.submit(Op::Compact).await?;
    wait_for_event(&override_dcode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = dcode.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    dcode.submit(Op::Shutdown).await?;
    wait_for_event(&dcode, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.dcode.submit(Op::Compact).await?;
    wait_for_event(&initial.dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    initial.dcode.submit(Op::Shutdown).await?;
    wait_for_event(&initial.dcode, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    let mut builder = test_dcode().with_auth(DcodeAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    test.dcode
        ...
        .await?;
    wait_for_event(&test.dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    test.dcode.submit(Op::Compact).await?;
    wait_for_event(&test.dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_dcode_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.dcode.as_ref()).await?;
    ...
    close_realtime_conversation(test.dcode.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_dcode_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_dcode_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.dcode.as_ref()).await?;
    ...
    close_realtime_conversation(initial.dcode.as_ref()).await?;
...
-> Result<()> {
    ...
    let previous_model = "gpt-5.1-dcode-max";
    let next_model = "gpt-5.2-dcode";
    let harness = TestDcodeHarness::with_builder(
        test_dcode()
            .with_auth(DcodeAuth::create_dummy_chatgpt_auth_for_testing())
