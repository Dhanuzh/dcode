...
async fn dcode_delegate_forwards_exec_approval_and_proceeds_on_approval() {
    skip_if_no_network!();
    ...
    let mut builder = test_dcode().with_model("gpt-5.1").with_config(|config| {
    ...
    let approval_event = wait_for_event(&test.dcode, |ev| {
...
#[ignore = "TODO once we have a delegate that can ask for approvals"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn dcode_delegate_forwards_patch_approval_and_proceeds_on_decision() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn dcode_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.dcode, |_| true).await;
        match ev {
