...
use dcode_protocol::openai_models::ModelInstructionsVariables;
use dcode_protocol::openai_models::ModelMessages;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    ...
    let model_info = dcode_core::test_support::construct_model_info_offline("gpt-5.1", &config);
    assert_eq!(
...
async fn base_instructions_override_disables_personality_template() {
    ...
    let model_info =
        dcode_core::test_support::construct_model_info_offline("gpt-5.2-dcode", &config);
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    .await;
    let mut builder = test_dcode()
        .with_model("exp-dcode-personality")
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "dcode-remote-default-personality";
    ...
    let mut builder = test_dcode()
        .with_auth(dcode_core::DcodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "dcode-remote-personality";
    ...
    let mut builder = test_dcode()
        ...
        .with_config(|config| {
            config
                ...
                .enable(Feature::Personality)
                .expect("test config should allow feature update");
            config.model = Some("gpt-5.2-dcode".to_string());
        });
    let test = builder.build(&server).await?;
