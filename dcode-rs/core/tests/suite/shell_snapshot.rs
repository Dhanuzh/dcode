...
const POLICY_PATH_FOR_TEST: &str = "/dcode/policy/path";
const SNAPSHOT_PATH_FOR_TEST: &str = "/dcode/snapshot/path";
...
async fn wait_for_snapshot(dcode_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = dcode_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_dcode().with_config(move |config| {
    ...
    let harness = TestDcodeHarness::with_builder(builder).await?;
    let args = json!({
        "cmd": command,
        "yield_time_ms": 1000,
    });
    ...
    let dcode_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&dcode_home).await?;
...
async fn run_tool_turn_on_harness(
    ...
    wait_for_event_match(&dcode, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.dcode_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    let builder = test_dcode().with_config(|config| {
        config
            .features
            .enable(Feature::ShellSnapshot)
            .expect("test config should allow feature update");
    ...
    let harness = TestDcodeHarness::with_builder(builder).await?;
    let dcode_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(dcode_home));
...
async fn shell_command_snapshot_still_intercepts_apply_patch() -> Result<()> {
    ...
    let args = json!({
        "command": script,
        // The intercepted apply_patch path self-invokes dcode, which can take
    ...
    wait_for_event(&dcode, |ev| match ev {
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let dcode_home = home.path().to_path_buf();
