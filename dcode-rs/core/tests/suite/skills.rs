...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_dcode().with_pre_build_hook(|home| {
        write_skill(home, "demo", "demo skill", skill_body);
    });
    let test = builder.build(&server).await?;

    let skill_path = test.dcode_home_path().join("skills/demo/SKILL.md");
    ...
    core_test_support::wait_for_event(test.dcode.as_ref(), |event| {
        matches!(event, dcode_protocol::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn skill_load_errors_surface_in_session_configured() -> Result<()> {
    ...
    test.dcode
        ...
        .await?;
    let response =
        core_test_support::wait_for_event_match(test.dcode.as_ref(), |event| match event {
            dcode_protocol::protocol::EventMsg::ListSkillsResponse(response) => {
                Some(response.clone())
            }
            _ => None,
        })
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.dcode_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, dcode_protocol::protocol::SkillScope::System);
