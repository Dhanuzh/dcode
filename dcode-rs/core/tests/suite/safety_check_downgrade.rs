...
use dcode_protocol::protocol::ModelRerouteReason;
...
const REQUESTED_MODEL: &str = "gpt-5.3-dcode";
...
async fn openai_model_header_mismatch_emits_warning_event_and_warning_item() -> Result<()> {
    ...
    let mut builder = test_dcode().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.dcode, |event| {
    ...
    let warning = wait_for_event(&test.dcode, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let warning_item = wait_for_event(&test.dcode, |event| {
        matches!(
            event,
    ...
    let _ = wait_for_event(&test.dcode, |event| {
...
async fn response_model_field_mismatch_emits_warning_when_header_matches_requested() -> Result<()> {
    ...
    let warning = wait_for_event(&test.dcode, |event| {
        matches!(
            event,
...
async fn openai_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    ...
    loop {
        let event = wait_for_event(&test.dcode, |_| true).await;
        match event {
