...
async fn run_turn(test: &TestDcode, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestDcode, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_dcode_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestDcode> {
    let mut builder = test_dcode().with_model("test-gpt-5.1-dcode");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_dcode_with_test_tool(&server).await?;
