...
async fn websocket_test_dcode_shell_chain() -> Result<()> {
    ...
    let mut builder = test_dcode().with_windows_cmd_shell();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn websocket_v2_test_dcode_shell_chain() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut builder = test_dcode().with_windows_cmd_shell().with_config(|config| {
        config
            .features
