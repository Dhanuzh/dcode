...
//! request payload that Dcode would send to the model and assert that the
...
async fn compact_resume_and_fork_preserve_model_history_view() {
    ...
    let expected_model = "gpt-5.1-dcode";
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<DcodeThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.dcode)
}

async fn user_turn(conversation: &Arc<DcodeThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<DcodeThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<DcodeThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = dcode_core::test_support::auth_manager_from_auth(
        dcode_core::DcodeAuth::from_api_key("dummy"),
    );
