...
use dcode_apply_patch::APPLY_PATCH_TOOL_INSTRUCTIONS;
use dcode_core::shell::Shell;
use dcode_core::shell::default_user_shell;
...
async fn prompt_tools_are_consistent_across_requests() -> anyhow::Result<()> {
    ...
    let TestDcode {
        ...
        .with_config(|config| {
            ...
            config.model = Some("gpt-5.1-dcode-max".to_string());
...
-> anyhow::Result<()> {
    ...
    let TestDcode { dcode, config, .. } = test_dcode()
        .with_config(|config| {
            config.user_instructions = Some("be consistent and helpful".to_string());
            config
                .features
