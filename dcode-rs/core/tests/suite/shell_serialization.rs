...
use core_test_support::test_dcode::ApplyPatchModelOutput;
use core_test_support::test_dcode::ShellModelOutput;
...
fn configure_shell_model(
    builder: TestDcodeBuilder,
    output_type: ShellModelOutput,
    include_apply_patch_tool: bool,
) -> TestDcodeBuilder {
    let builder = match (output_type, include_apply_patch_tool) {
        (ShellModelOutput::ShellCommand, _) => builder.with_model("test-gpt-5-dcode"),
        (ShellModelOutput::LocalShell, true) => builder.with_model("gpt-5.1-dcode"),
        (ShellModelOutput::Shell, true) => builder.with_model("gpt-5.1-dcode"),
        (ShellModelOutput::LocalShell, false) => builder.with_model("dcode-mini-latest"),
...
async fn shell_output_stays_json_without_freeform_apply_patch(
    ...
    let mut builder = configure_shell_model(test_dcode(), output_type, false);
...
async fn shell_output_is_structured_with_freeform_apply_patch(
    ...
    let server = start_mock_server().await;
    let mut builder = configure_shell_model(test_dcode(), output_type, true);
    let test = builder.build(&server).await?;
...
async fn shell_output_reserializes_truncated_content(output_type: ShellModelOutput) -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder =
        configure_shell_model(test_dcode(), output_type, true).with_config(move |config| {
