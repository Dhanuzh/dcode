...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    let TestDcode { dcode, .. } = builder.build(&server).await?;

    let notice = wait_for_event_match(&dcode, |event| match event {
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable unified_exec` or `[features].unified_exec` in config.toml. See https://developers.openai.com/dcode/config-basic#feature-flags for details."
        ),
    );
