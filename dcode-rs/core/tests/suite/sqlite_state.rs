...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db_path = dcode_state::state_db_path(test.config.sqlite_home.as_path());
    ...
    let db = test.dcode.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_dcode()
        .with_pre_build_hook(move |dcode_home| {
            let rollout_path = dcode_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = dcode_state::state_db_path(test.config.sqlite_home.as_path());
    let rollout_path = test.config.dcode_home.join(&rollout_rel_path);
...
async fn mcp_call_marks_thread_memory_mode_polluted_when_configured() -> Result<()> {
    ...
    .await;
    wait_for_event_match(&test.dcode, |event| match event {
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let subscriber = tracing_subscriber::registry().with(dcode_state::log_db::start(db.clone()));
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = dcode_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
