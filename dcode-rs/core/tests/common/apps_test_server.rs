...
const SERVER_NAME: &str = "dcode-apps-test";
...
async fn mount_streamable_http_json_rpc(
    ...
    Mock::given(method("POST"))
        .and(path_regex("^/api/dcode/apps/?$"))
        .respond_with(DcodeAppsJsonRpcResponder {
            connector_name,
...
struct DcodeAppsJsonRpcResponder {
    connector_name: String,
    connector_description: String,
    searchable: bool,
}

impl Respond for DcodeAppsJsonRpcResponder {
        ...
        match method {
            ...
            "tools/call" => {
                ...
                let starts_at = body
                    ...
                    .and_then(Value::as_str)
                    .unwrap_or_default();
                let dcode_apps_meta = body.pointer("/params/_meta/_dcode_apps").cloned();
