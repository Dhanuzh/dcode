...
use anyhow::ensure;
use dcode_arg0::Arg0PathEntryGuard;
use dcode_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    dcode_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    dcode_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(dcode_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = dcode_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("dcode-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\dcode\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.dcode`.
pub async fn load_default_config_for_test(dcode_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        dcode_linux_sandbox_exe: Some(
            find_dcode_linux_sandbox_exe().expect("should find binary for dcode-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_dcode_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    if let Some(path) = TEST_ARG0_PATH_ENTRY
        ...
        .and_then(|path_entry| path_entry.paths().dcode_linux_sandbox_exe.clone())
    {
        return Ok(path);
    }

    dcode_utils_cargo_bin::cargo_bin("dcode-linux-sandbox")
}
...
pub async fn wait_for_event<F>(
    dcode: &DcodeThread,
    predicate: F,
) -> dcode_protocol::protocol::EventMsg
where
    F: FnMut(&dcode_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(dcode, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(dcode: &DcodeThread, matcher: F) -> T
where
    F: Fn(&dcode_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(dcode, |ev| matcher(ev).is_some()).await;
    matcher(&ev).unwrap()
}
    ...
    F: FnMut(&dcode_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), dcode.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    dcode_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    dcode_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    dcode_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    dcode_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    dcode_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! dcode_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_dcode_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("dcode-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
