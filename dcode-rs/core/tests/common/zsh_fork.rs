...
use crate::test_dcode::TestDcode;
use crate::test_dcode::test_dcode;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = dcode_utils_cargo_bin::cargo_bin("dcode-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `dcode-execve-wrapper` binary");
        return Ok(None);
    };
