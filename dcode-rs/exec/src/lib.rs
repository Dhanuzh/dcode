...
use dcode_app_server_client::InProcessServerEvent;
use dcode_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use dcode_core::LMSTUDIO_OSS_PROVIDER_ID;
use dcode_core::OLLAMA_OSS_PROVIDER_ID;
use dcode_core::auth::AuthConfig;
use dcode_core::auth::enforce_login_restrictions;
use dcode_core::check_execpolicy_for_warnings;
use dcode_core::config::Config;
use dcode_core::config::ConfigBuilder;
use dcode_core::config::ConfigOverrides;
use dcode_core::config::find_dcode_home;
use dcode_core::config::load_config_as_toml_with_cli_overrides;
use dcode_core::config::resolve_oss_provider;
use dcode_core::config_loader::ConfigLoadError;
use dcode_core::config_loader::LoaderOverrides;
use dcode_core::config_loader::format_config_error_with_source;
use dcode_core::format_exec_policy_error_with_source;
use dcode_core::git_info::get_git_repo_root;
use dcode_feedback::DcodeFeedback;
use dcode_otel::set_parent_from_context;
use dcode_otel::traceparent_context_from_env;
use dcode_protocol::account::PlanType as AccountPlanType;
use dcode_protocol::config_types::SandboxMode;
...
use dcode_utils_oss::ensure_oss_provider_ready;
use dcode_utils_oss::get_default_model_for_oss_provider;
...
use crate::event_processor::DcodeStatus;
use crate::event_processor::EventProcessor;
use dcode_core::default_client::set_default_client_residency_requirement;
use dcode_core::default_client::set_default_originator;
use dcode_core::find_thread_path_by_id_str;
use dcode_core::find_thread_path_by_name_str;
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("dcode_exec".to_string()) {
        tracing::warn!(?err, "Failed to set dcode exec originator override {err:?}");
    }
    ...
    #[allow(clippy::print_stderr)]
    let dcode_home = match find_dcode_home() {
        Ok(dcode_home) => dcode_home,
        Err(err) => {
    ...
    let cloud_requirements =
        cloud_requirements_loader(cloud_auth_manager, chatgpt_base_url, dcode_home.clone());
    ...
    let overrides = ConfigOverrides {
        ...
        service_tier: None,
        dcode_linux_sandbox_exe: arg0_paths.dcode_linux_sandbox_exe.clone(),
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        dcode_core::otel_init::build_provider(
            &config,
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        feedback: DcodeFeedback::new(),
        config_warnings,
        session_source: SessionSource::Exec,
        enable_dcode_api_key_env: true,
        client_name: "dcode-exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = dcode_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Dcode
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode && let Some(message) = dcode_core::config::system_bwrap_warning() {
        let _ = event_processor.process_event(Event {
            id: String::new(),
            msg: EventMsg::Warning(dcode_protocol::protocol::WarningEvent { message }),
        });
    }
                ...
                match &event.msg {
                    ...
                    EventMsg::McpStartupUpdate(update) => {
                        if required_mcp_servers.contains(&update.server)
                            && let dcode_protocol::protocol::McpStartupStatus::Failed { error } =
                                &update.status
                        {
                            error_seen = true;
                            eprintln!(
...
fn sandbox_mode_from_policy(
    ...
    match sandbox_policy {
        ...
        dcode_protocol::protocol::SandboxPolicy::ReadOnly { .. } => {
            Some(dcode_app_server_protocol::SandboxMode::ReadOnly)
        }
        dcode_protocol::protocol::SandboxPolicy::WorkspaceWrite { .. } => {
            Some(dcode_app_server_protocol::SandboxMode::WorkspaceWrite)
        }
        dcode_protocol::protocol::SandboxPolicy::ExternalSandbox { .. } => None,
    }
}
...
fn session_configured_from_thread_response(
    ...
    let session_id = dcode_protocol::ThreadId::from_string(thread_id)
...
fn normalize_legacy_notification_method(method: &str) -> &str {
    method.strip_prefix("dcode/event/").unwrap_or(method)
}
...
async fn resolve_resume_path(
    ...
    if args.last {
        ...
        match dcode_core::RolloutRecorder::find_latest_thread_path(
            config,
            /*page_size*/ 1,
            /*cursor*/ None,
            dcode_core::ThreadSortKey::UpdatedAt,
            &[],
        ...
        if Uuid::parse_str(id_str).is_ok() {
            let path = find_thread_path_by_id_str(&config.dcode_home, id_str).await?;
            Ok(path)
        } else {
            let path = find_thread_path_by_name_str(&config.dcode_home, id_str).await?;
            Ok(path)
        }
    } else {
        Ok(None)
...
mod tests {
    ...
    fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
        ...
        let tracer = provider.tracer("dcode-exec-tests");
    ...
    fn exec_root_span_can_be_parented_from_trace_context() {
        ...
        let parent = dcode_protocol::protocol::W3cTraceContext {
    ...
    fn decode_legacy_notification_preserves_conversation_id() {
        let decoded = decode_legacy_notification(JSONRPCNotification {
            method: "dcode/event/error".to_string(),
        ...
        assert!(matches!(
            decoded.event.msg,
            EventMsg::Error(dcode_protocol::protocol::ErrorEvent {
                message,
    ...
    async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
        let dcode_home = tempdir().expect("create temp dcode home");
        ...
        assert_eq!(
            ...
            Some(dcode_app_server_protocol::ApprovalsReviewer::User)
        );
    }
    ...
    async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
        ...
        let cwd = tempdir().expect("create temp cwd");
        std::fs::write(
            dcode_home.path().join("config.toml"),
        ...
        assert_eq!(
            params.approvals_reviewer,
            Some(dcode_app_server_protocol::ApprovalsReviewer::GuardianSubagent)
        );
    }
    ...
    fn session_configured_from_thread_response_uses_review_policy_from_response() {
        let response = ThreadStartResponse {
            thread: dcode_app_server_protocol::Thread {
                ...
                source: dcode_app_server_protocol::SessionSource::Cli,
                agent_nickname: None,
                agent_role: None,
                git_info: None,
            ...
            service_tier: None,
            cwd: PathBuf::from("/tmp"),
            approval_policy: dcode_app_server_protocol::AskForApproval::OnRequest,
            approvals_reviewer: dcode_app_server_protocol::ApprovalsReviewer::GuardianSubagent,
            sandbox: dcode_app_server_protocol::SandboxPolicy::WorkspaceWrite {
                writable_roots: vec![],
                read_only_access: dcode_app_server_protocol::ReadOnlyAccess::FullAccess,
