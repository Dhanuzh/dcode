use dcode_exec::event_processor_with_jsonl_output::EventProcessorWithJsonOutput;
use dcode_exec::exec_events::AgentMessageItem;
use dcode_exec::exec_events::CollabAgentState;
use dcode_exec::exec_events::CollabAgentStatus;
use dcode_exec::exec_events::CollabTool;
use dcode_exec::exec_events::CollabToolCallItem;
use dcode_exec::exec_events::CollabToolCallStatus;
use dcode_exec::exec_events::CommandExecutionItem;
use dcode_exec::exec_events::CommandExecutionStatus;
use dcode_exec::exec_events::ErrorItem;
use dcode_exec::exec_events::ItemCompletedEvent;
use dcode_exec::exec_events::ItemStartedEvent;
use dcode_exec::exec_events::ItemUpdatedEvent;
use dcode_exec::exec_events::McpToolCallItem;
use dcode_exec::exec_events::McpToolCallItemError;
use dcode_exec::exec_events::McpToolCallItemResult;
use dcode_exec::exec_events::McpToolCallStatus;
use dcode_exec::exec_events::PatchApplyStatus;
use dcode_exec::exec_events::PatchChangeKind;
use dcode_exec::exec_events::ReasoningItem;
use dcode_exec::exec_events::ThreadErrorEvent;
use dcode_exec::exec_events::ThreadEvent;
use dcode_exec::exec_events::ThreadItem;
use dcode_exec::exec_events::ThreadItemDetails;
use dcode_exec::exec_events::ThreadStartedEvent;
use dcode_exec::exec_events::TodoItem as ExecTodoItem;
use dcode_exec::exec_events::TodoListItem as ExecTodoListItem;
use dcode_exec::exec_events::TurnCompletedEvent;
use dcode_exec::exec_events::TurnFailedEvent;
use dcode_exec::exec_events::TurnStartedEvent;
use dcode_exec::exec_events::Usage;
use dcode_exec::exec_events::WebSearchItem;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_id =
        dcode_protocol::ThreadId::from_string("67e55044-10b1-426f-9247-bb680e5fe0c8").unwrap();
    ...
    let ev = event(
        ...
        EventMsg::SessionConfigured(SessionConfiguredEvent {
            ...
            forked_from_id: None,
            thread_name: None,
            model: "dcode-mini-latest".to_string(),
...
fn task_started_produces_turn_started_event() {
    ...
    let out = ep.collect_thread_events(&event(
        "t1",
        EventMsg::TurnStarted(dcode_protocol::protocol::TurnStartedEvent {
...
fn plan_update_emits_todo_list_started_updated_and_completed() {
    ...
    let complete = event(
        "p3",
        EventMsg::TurnComplete(dcode_protocol::protocol::TurnCompleteEvent {
            turn_id: "turn-1".to_string(),
...
fn stream_error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::StreamError(dcode_protocol::protocol::StreamErrorEvent {
...
fn task_complete_produces_turn_completed_with_usage() {
    ...
    let usage = dcode_protocol::protocol::TokenUsage {
        input_tokens: 1200,
    ...
    let info = dcode_protocol::protocol::TokenUsageInfo {
    ...
    let token_count_event = event(
        "e1",
        EventMsg::TokenCount(dcode_protocol::protocol::TokenCountEvent {
            info: Some(info),
            rate_limits: None,
        }),
    );
