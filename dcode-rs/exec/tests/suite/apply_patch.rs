...
/// While we may add an `apply-patch` subcommand to the `dcode` CLI multitool
/// at some point, we must ensure that the smaller `dcode-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(dcode_utils_cargo_bin::cargo_bin("dcode-exec")?)
