...
//!
//! The detailed explanation of what "hermetic" means here lives in `dcode_client::custom_ca`.
//! This binary exists so the tests can exercise
//! [`dcode_client::build_reqwest_client_for_subprocess_tests`] in a separate process without
//! duplicating client-construction logic.

use std::process;

fn main() {
    match dcode_client::build_reqwest_client_for_subprocess_tests(reqwest::Client::builder()) {
        Ok(_) => {
            println!("ok");
        }
        Err(error) => {
