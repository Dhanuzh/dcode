...
#[derive(Clone, Debug)]
pub struct DcodeHttpClient {
    inner: reqwest::Client,
}

impl DcodeHttpClient {
    pub fn new(inner: reqwest::Client) -> Self {
        Self { inner }
    }

    pub fn get<U>(&self, url: U) -> DcodeRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn post<U>(&self, url: U) -> DcodeRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn request<U>(&self, method: Method, url: U) -> DcodeRequestBuilder
    where
        U: IntoUrl,
    {
        let url_str = url.as_str().to_string();
        DcodeRequestBuilder::new(self.inner.request(method.clone(), url), method, url_str)
    }
}

#[must_use = "requests are not sent unless `send` is awaited"]
#[derive(Debug)]
pub struct DcodeRequestBuilder {
    builder: reqwest::RequestBuilder,
    method: Method,
    url: String,
}

impl DcodeRequestBuilder {
