//! Custom CA handling for Dcode outbound HTTP and websocket clients.
//!
//! Dcode constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!   because Dcode itself commonly runs spawned test processes under seatbelt, so this is not just
...
/// Builds a reqwest client that honors Dcode custom CA environment variables.
///
...
/// easy mistake to make when adding a new outbound Dcode HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Dcode custom CA bundle is configured.
///
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Dcode custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        dcode_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `CODEX_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Dcode-specific override.
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Dcode CA overrides.
    ///
    /// This accepts a few common real-world variants so Dcode behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Dcode only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_dcode_env() {
        let env = map_env(&[
            (CODEX_CA_CERT_ENV, "/tmp/dcode.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/dcode.pem"))
        );
    }
