...
//! orchestration and rollout scanning live in `dcode-core`.
...
pub const DB_ERROR_METRIC: &str = "dcode.db.error";
/// Metrics on backfill process. Tags: [status]
pub const DB_METRIC_BACKFILL: &str = "dcode.db.backfill";
/// Metrics on backfill duration. Tags: [status]
pub const DB_METRIC_BACKFILL_DURATION_MS: &str = "dcode.db.backfill.duration_ms";
