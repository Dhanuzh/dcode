...
mod tests {
    ...
    async fn insert_logs_use_dedicated_log_database() {
        ...
        let state_count = log_row_count(state_db_path(dcode_home.as_path()).as_path()).await;
        let logs_count = log_row_count(logs_db_path(dcode_home.as_path()).as_path()).await;
