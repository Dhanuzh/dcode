...
fn thread_spawn_parent_thread_id_from_source_str(source: &str) -> Option<ThreadId> {
    ...
    match parsed_source.ok() {
        Some(SessionSource::SubAgent(dcode_protocol::protocol::SubAgentSource::ThreadSpawn {
            parent_thread_id,
            ..
...
mod tests {
    ...
    use dcode_protocol::protocol::GitInfo;
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        ...
        let mut metadata = test_thread_metadata(&dcode_home, thread_id, dcode_home.clone());
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&dcode_home, thread_id, dcode_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        assert_eq!(
            ...
            Some("git@example.com:openai/dcode.git")
        );
    }
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:openai/dcode.git")),
            )
            .await
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&dcode_home, thread_id, dcode_home.clone());
        ...
        let mut fallback = test_thread_metadata(&dcode_home, thread_id, dcode_home.clone());
    ...
    async fn update_thread_git_info_can_clear_fields() {
        ...
        metadata.git_origin_url = Some("git@example.com:openai/dcode.git".to_string());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            dcode_protocol::protocol::TokenCountEvent {
                info: Some(dcode_protocol::protocol::TokenUsageInfo {
                    total_token_usage: dcode_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: dcode_protocol::protocol::TokenUsage::default(),
    ...
    async fn thread_spawn_edges_track_directional_status() {
        let dcode_home = unique_temp_dir();
        let runtime = StateRuntime::init(dcode_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
