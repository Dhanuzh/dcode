...
mod tests {
    ...
    async fn init_removes_legacy_state_db_files() {
        ...
        tokio::fs::create_dir_all(&dcode_home)
            .await
            .expect("create dcode_home");
        ...
        for suffix in ["", "-wal", "-shm", "-journal"] {
            let path = dcode_home.join(format!("{unversioned_name}{suffix}"));
            tokio::fs::write(path, b"legacy")
                .await
                .expect("write legacy");
            let old_version_path = dcode_home.join(format!(
        ...
        let unrelated_path = dcode_home.join("state.sqlite_backup");
        tokio::fs::write(&unrelated_path, b"keep")
            .await
            .expect("write unrelated");
        let numeric_path = dcode_home.join("123");
        tokio::fs::write(&numeric_path, b"keep")
            .await
            .expect("write numeric");

        let _runtime = StateRuntime::init(dcode_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");

        for suffix in ["", "-wal", "-shm", "-journal"] {
            let legacy_path = dcode_home.join(format!("{unversioned_name}{suffix}"));
            assert_eq!(
        ...
        assert_eq!(
            tokio::fs::try_exists(dcode_home.join(current_name))
                .await
