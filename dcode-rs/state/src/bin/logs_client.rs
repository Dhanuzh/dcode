...
use chrono::DateTime;
use clap::Parser;
use dcode_state::LogQuery;
use dcode_state::LogRow;
use dcode_state::StateRuntime;
use dirs::home_dir;
use owo_colors::OwoColorize;

#[derive(Debug, Parser)]
#[command(name = "dcode-state-logs")]
#[command(about = "Tail Dcode logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to CODEX_HOME. Defaults to $CODEX_HOME or ~/.dcode.
    #[arg(long, env = "DCODE_HOME")]
    dcode_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --dcode-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let dcode_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(dcode_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let dcode_home = args.dcode_home.clone().unwrap_or_else(default_dcode_home);
    Ok(dcode_state::logs_db_path(dcode_home.as_path()))
}

fn default_dcode_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".dcode");
    }
    PathBuf::from(".dcode")
}
