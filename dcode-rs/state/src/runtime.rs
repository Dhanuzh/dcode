...
impl StateRuntime {
    /// Initialize the state runtime using the provided Dcode home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `dcode_home`,
    /// keeping logs in a dedicated file to reduce lock contention with the
    /// rest of the state store.
    pub async fn init(dcode_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
        ...
        .await;
        let state_path = state_db_path(dcode_home.as_path());
        let logs_path = logs_db_path(dcode_home.as_path());
    ...
    /// Return the configured Dcode home directory for this runtime.
    pub fn dcode_home(&self) -> &Path {
        self.dcode_home.as_path()
    }
}
...
pub fn state_db_path(dcode_home: &Path) -> PathBuf {
    dcode_home.join(state_db_filename())
}
...
pub fn logs_db_path(dcode_home: &Path) -> PathBuf {
    dcode_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(dcode_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read dcode_home for {db_label} db cleanup {}: {err}",
                dcode_home.display(),
            );
            return;
        }
