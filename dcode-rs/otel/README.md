...
`dcode-otel` is the OpenTelemetry integration crate for Dcode. It provides:

- Provider wiring for log/trace/metric exporters (`dcode_otel::OtelProvider`
  and `dcode_otel::provider`).
- Session-scoped business event emission via `dcode_otel::SessionTelemetry`.
- Low-level metrics APIs via `dcode_otel::metrics`.
- Trace-context helpers via `dcode_otel::trace_context` and crate-root re-exports.
...
```rust
use dcode_otel::config::OtelExporter;
use dcode_otel::config::OtelHttpProtocol;
use dcode_otel::config::OtelSettings;
use dcode_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "dcode-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    dcode_home: std::path::PathBuf::from("/tmp"),
...
Dcode-specific session events. Rich session/business events should go through
...
`dcode-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Dcode-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use dcode_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("dcode.session_started", 1, &[("source", "tui")])?;
metrics.histogram("dcode.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("dcode.turns", 1, &[("model", "gpt-5.1")])?;
