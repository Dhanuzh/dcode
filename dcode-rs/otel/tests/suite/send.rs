...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "dcode-cli"), ("env", "prod")])?;

    metrics.counter("dcode.turns", 1, &[("model", "gpt-5.1"), ("env", "dev")])?;
    metrics.histogram("dcode.tool_latency", 25, &[("tool", "shell")])?;
    metrics.shutdown()?;

    let resource_metrics = latest_metrics(&exporter);

    let counter = find_metric(&resource_metrics, "dcode.turns").expect("counter metric missing");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "dcode-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "dcode.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "dcode.tool_latency").and_then(|metric| {
            match metric.data() {
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    metrics.counter("dcode.alpha", 1, &[("env", "dev"), ("component", "alpha")])?;
    metrics.counter(
        "dcode.beta",
        2,
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "dcode.alpha").expect("dcode.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "dcode.beta").expect("dcode.beta metric missing");
