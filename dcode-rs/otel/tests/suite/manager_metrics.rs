...
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "dcode-cli")])?;
    ...
    manager.counter("dcode.session_started", 1, &[("source", "tui")]);
    manager.shutdown_metrics()?;

    let resource_metrics = latest_metrics(&exporter);
    let metric =
        find_metric(&resource_metrics, "dcode.session_started").expect("counter metric missing");
...
fn manager_attaches_optional_service_name_tag() -> Result<()> {
    ...
    manager.counter("dcode.session_started", 1, &[]);
    manager.shutdown_metrics()?;
