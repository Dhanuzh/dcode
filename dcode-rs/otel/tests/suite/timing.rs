...
fn record_duration_records_histogram() -> Result<()> {
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "dcode.request_latency");
    assert!(!bounds.is_empty());
    assert_eq!(bucket_counts.iter().sum::<u64>(), 1);
    assert_eq!(sum, 15.0);
    assert_eq!(count, 1);
    let metric = crate::harness::find_metric(&resource_metrics, "dcode.request_latency")
        .unwrap_or_else(|| panic!("metric dcode.request_latency missing"));
...
fn timer_result_records_success() -> Result<()> {
    ...
    {
        let timer = metrics.start_timer("dcode.request_latency", &[("route", "chat")]);
        assert!(timer.is_ok());
    }
    ...
    let attrs = attributes_to_map(
        match crate::harness::find_metric(&resource_metrics, "dcode.request_latency").and_then(
