...
impl OtelProvider {
    ...
    pub fn dcode_export_filter(meta: &tracing::Metadata<'_>) -> bool {
...
mod tests {
    ...
    fn log_export_target_excludes_trace_safe_events() {
        assert!(is_log_export_target("dcode_otel.log_only"));
        assert!(is_log_export_target("dcode_otel.network_proxy"));
        assert!(!is_log_export_target("dcode_otel.trace_safe"));
        assert!(!is_log_export_target("dcode_otel.trace_safe.debug"));
    }

    #[test]
    fn trace_export_target_only_includes_trace_safe_prefix() {
        assert!(is_trace_safe_target("dcode_otel.trace_safe"));
        assert!(is_trace_safe_target("dcode_otel.trace_safe.summary"));
        assert!(!is_trace_safe_target("dcode_otel.log_only"));
        assert!(!is_trace_safe_target("dcode_otel.network_proxy"));
    }

    fn test_otel_settings() -> OtelSettings {
        OtelSettings {
            environment: "test".to_string(),
            service_name: "dcode-test".to_string(),
            service_version: "0.0.0".to_string(),
            dcode_home: PathBuf::from("."),
