...
pub use dcode_utils_string::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on dcode-core.
...
impl From<dcode_app_server_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: dcode_app_server_protocol::AuthMode) -> Self {
        match mode {
            dcode_app_server_protocol::AuthMode::ApiKey => Self::ApiKey,
            dcode_app_server_protocol::AuthMode::Chatgpt
            | dcode_app_server_protocol::AuthMode::ChatgptAuthTokens => Self::Chatgpt,
        }
    }
}
