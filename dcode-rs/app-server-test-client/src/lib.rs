...
use dcode_app_server_protocol::ClientInfo;
...
use dcode_app_server_protocol::DynamicToolSpec;
...
use dcode_app_server_protocol::InitializeCapabilities;
use dcode_app_server_protocol::InitializeParams;
use dcode_app_server_protocol::InitializeResponse;
use dcode_app_server_protocol::JSONRPCMessage;
use dcode_app_server_protocol::JSONRPCNotification;
use dcode_app_server_protocol::JSONRPCRequest;
...
use dcode_app_server_protocol::ReadOnlyAccess;
use dcode_app_server_protocol::RequestId;
use dcode_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "dcode-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $CODEX_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Dcode app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Dcode", version, about = "Bootstrap Dcode app-server", long_about = None)]
struct Cli {
    /// Path to the `dcode` CLI binary. When set, requests use stdio by
    /// spawning `dcode app-server` as a child process.
    #[arg(long, env = "CODEX_BIN", global = true)]
    dcode_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--dcode-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "CODEX_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `dcode` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `dcode app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/dcode-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `dcode app-server --listen`.
    ...
    /// Send a user message through the Dcode app-server.
    SendMessage {
        /// User message to send to Dcode.
        user_message: String,
    },
    ...
    TestLogin,
    /// Fetch the current account rate limits from the Dcode app-server.
    GetAccountRateLimits,
    /// List the available models from the Dcode app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Dcode app-server.
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let dcode_bin = dcode_bin.unwrap_or_else(|| PathBuf::from("dcode"));
            serve(&dcode_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(dcode_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(dcode_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(dcode_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(dcode_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if dcode_bin.is_some() && url.is_some() {
        bail!("--dcode-bin and --url are mutually exclusive");
    }
    if let Some(dcode_bin) = dcode_bin {
        return Ok(Endpoint::SpawnDcode(dcode_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if dcode_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --dcode-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(dcode_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(dcode_bin);
        if let Some(dcode_bin_parent) = dcode_bin.parent() {
            let mut path = OsString::from(dcode_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", dcode_bin.display()))?;
...
fn serve(dcode_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/dcode-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,dcode_=trace {}",
        shell_quote(&dcode_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started dcode app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnDcode(dcode_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut DcodeClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = DcodeClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = DcodeClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (dcode_bin, url) {
        (Some(_), Some(_)) => bail!("--dcode-bin and --url are mutually exclusive"),
        (Some(dcode_bin), None) => {
            let server = BackgroundAppServer::spawn(&dcode_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve dcode-app-server-test-client binary path")?;
...
impl DcodeClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnDcode(dcode_bin) => Self::spawn_stdio(dcode_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(dcode_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let dcode_bin_display = dcode_bin.display();
        let mut cmd = Command::new(dcode_bin);
        if let Some(dcode_bin_parent) = dcode_bin.parent() {
            let mut path = OsString::from(dcode_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut dcode_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{dcode_bin_display}` app-server"))?;

        let stdin = dcode_app_server
            .stdin
            .take()
            .context("dcode app-server stdin unavailable")?;
        let stdout = dcode_app_server
            .stdout
            .take()
            .context("dcode app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: dcode_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `dcode-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "dcode-toy-app-server".to_string(),
                    title: Some("Dcode Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: dcode_app_server_protocol::LoginAccountParams::Chatgpt,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to dcode app-server")?;
                    return Ok(());
                }
                bail!("dcode app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from dcode app-server")?;
                if bytes == 0 {
                    bail!("dcode app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = dcode_core::otel_init::build_provider(
            &config,
...
impl Drop for DcodeClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[dcode app-server exited: {status}]");
            return;
        }
